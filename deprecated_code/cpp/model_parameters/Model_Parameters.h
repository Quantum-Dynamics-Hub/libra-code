...
public:
  ...
  HF_integrals(const HF_integrals &ob) { data = ob.data; }
  ...
  friend bool operator==(const HF_integrals &m1, const HF_integrals &m2) {
    ...
    if (m1.data.size() != m2.data.size()) {
      ...
      for (auto i = 0u; i < m1.data.size(); i++) {
...
class EHT_K {
  /**
    ...
    string type, so very often access to these data elements in actual
    calculations may notably slow down the overall calculations, so we only use
    this class for reading in the parameters from the input file
  */
  ...
  int find_data(std::string, std::string, std::string,
                std::string);        // in PP_data
...
public:
  ...
  void set_K_value(int k_indx, std::string elt1, std::string orb_type1,
                   std::string elt2, std::string orb_type2, double K);
  double get_K_value(int k_indx, std::string elt1, std::string orb_type1,
                     std::string elt2, std::string orb_type2);

  void set_C_value(int, std::string, std::string, std::string, std::string,
                   double);
  ...
  friend bool operator==(const EHT_K &m1, const EHT_K &m2) {
    ...
    if (m1.pp_data.size() != m2.pp_data.size()) {
      res = 0;
    } else {
      for (auto i = 0u; i < m1.pp_data.size(); i++) {
    ...
    if (m1.psps_data.size() != m2.psps_data.size()) {
      res = 0;
    } else {
      for (auto i = 0u; i < m1.psps_data.size(); i++) {
...
class mEHT_K {
  /**
    This is an efficient version of the EHT_K class - tuned for actual
    calculations. The efficiency is gained due to 1-integer index access, which
    is much faster than the one using 4 strings. The drawback is that the
    storage requirements are increased - about O(N^2) with N being the number of
    orbitals. The data for all orbital pairs will be stored.
  */

public:
  int size; ///< is the size of the matrix (N below - number of AOs in the
            ///< basis)
  vector<vector<double>>
      eht_K; ///< eht_K[k_indx][I*N+J] = is the k_indx-type parameter for pair
             ///< of orbitals I and J - each of special type and on special atom
  vector<vector<double>>
      eht_C; ///< eht_C[c_indx][I*N+J] = is the parameter for pair of orbitals I
             ///< and J - each of special type and on special atom
  ...
  inline double get_K_value(int indx, int I, int J) {
    return eht_K[indx][I * size + J];
  }
  inline double get_C_value(int indx, int I, int J) {
    return eht_C[indx][I * size + J];
  }
...
public:
  ...
  map<std::string, double> IP; ///< Ionization potential for a given shell
                               ///< (first = 1s, 2s, 2p, ... 3d, so on)
  map<std::string, double> EA; ///< Electron affinity for a given shell (first =
                               ///< 1s, 2s, 2p, ... 3d, so on)

  map<std::string, int> Nquant; ///< principal quantum number for each shell
  map<std::string, int>
      Nzeta; ///< number of different zetas (exponents for given shell)
  ...
  map<std::string, double>
      J_param3; ///< in .../sqrt(r^2 + f(J_param1))  - for el-nucl interaction
  map<std::string, double>
      J_param4; ///< in f'(J_param2)/sqrt(...)       - for el-nucl interaction

  // CNDO, CNDO/2, INDO
  map<std::string, double> G1, F2; ///< Slater-Condon factors (for INDO)
  map<std::string, double>
      beta0;   ///< proportionality constants for off-diagonal matrix elements
  ...
  pElement(std::string en, int z, int nv, map<std::string, double> ip) {
    elt_name = en;
    Z = z;
    Nval = nv;
    IP = ip;
  ...
  void _set(std::string en, int z) {
    elt_name = en;
    Z = z;
  }
  void _set(std::string en, int z, int nv) {
    elt_name = en;
    Z = z;
    Nval = nv;
  }
  void _set(std::string en, int z, int nv, double zeff) {
    elt_name = en;
    Z = z;
    Nval = nv;
    Zeff = zeff;
...
class OrbParams {
  /**
    The class that keeps the parameters for a given orbital type (e.g. H(1s),
    Si(3p), etc.)
  */
...
class Model_Parameters {
  /**
    Class that stores the parameters of the atomistic quantal Hamiltonians
...
public:
  ...
  // Atomic parameters
  map<std::string, pElement>
      PT; ///< General and specific (for a given method) atomic parameters
  ...
  //  vector<double> eri;    // precomputed electron repulsion integrals - for
  //  all atoms vector<> V_A

  // For INDO/CNDO/CNDO2
  int indo_opt;       ///< 1 = INDO, 0 = CNDO/CNDO2
  vector<double> eri; ///< precomputed electron repulsion integrals (only
                      ///< (ss|ss) type between all pairs of atoms)
  vector<double>
      V_AB; ///< precomputed core-core repulsion terms for all pairs of atoms
...
void set_parameters_eht_mapping(Model_Parameters &modprms,
                                const vector<AO> &basis_ao);
void set_parameters_eht_mapping1(Model_Parameters &modprms, int nat,
