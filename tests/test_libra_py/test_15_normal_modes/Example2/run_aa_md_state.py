...
def main():
    ...
    syst.init_atom_velocities(10.0, rnd)  # must be this !!!
#    syst.init_fragment_velocities(300.0, rnd)
    ...
    ndof = 3 * syst.Number_of_atoms
    nsteps = 500

    M = MATRIX(ndof, 1)
    R = MATRIX(ndof, nsteps)
    V = MATRIX(ndof, nsteps)
    A = MATRIX(ndof, nsteps)    
    E = [] 
    ...
    for i in xrange(nsteps):
        ...
        for a in xrange(syst.Number_of_atoms):
            if i==0:
                E.append(syst.Atoms[a].Atom_element)
 
            m = syst.Atoms[a].Atom_RB.rb_mass
            M.set(3*a + 0, 0, m)
            M.set(3*a + 1, 0, m)
            M.set(3*a + 2, 0, m)

            R.set(3*a + 0,  i,  syst.Atoms[a].Atom_RB.rb_cm.x)
            R.set(3*a + 1,  i,  syst.Atoms[a].Atom_RB.rb_cm.y)
            R.set(3*a + 2,  i,  syst.Atoms[a].Atom_RB.rb_cm.z)
                        
            V.set(3*a + 0,  i,  syst.Atoms[a].Atom_RB.rb_p.x/m )
            V.set(3*a + 1,  i,  syst.Atoms[a].Atom_RB.rb_p.y/m)
            V.set(3*a + 2,  i,  syst.Atoms[a].Atom_RB.rb_p.z/m)

            A.set(3*a + 0,  i,  syst.Atoms[a].Atom_RB.rb_force.x/m)
            A.set(3*a + 1,  i,  syst.Atoms[a].Atom_RB.rb_force.y/m)
            A.set(3*a + 2,  i,  syst.Atoms[a].Atom_RB.rb_force.z/m)
...
#    params = {"nsteps":100, "nperiods":1, "scale":5.0, "print_modes":range(280, 299) }
#    nmvc_1.get_normal_modes2(R, V, A, M, E, params)
