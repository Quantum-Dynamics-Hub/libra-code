...
def md( params ):
    ...
    acffile1 = params["acf_filename_mu"]
    specfile1 = params["spectrum_filename_mu"]
    acffile2 = params["acf_filename_vel"]
    specfile2 = params["spectrum_filename_vel"]
    ...
    for j in xrange(Nsteps):
        ...
        vel_t = MATRIX(3*N, 1)
        for i in xrange(N):
            vel_t.set(3*i,   0, syst.Atoms[i].Atom_RB.rb_p.x / mass)
            vel_t.set(3*i+1, 0, syst.Atoms[i].Atom_RB.rb_p.y / mass)
            vel_t.set(3*i+2, 0, syst.Atoms[i].Atom_RB.rb_p.z / mass)
        vel.append(vel_t)
    ...
    acf_vector.recipe1(mu, dt*0.02419, 5000.0, 1.0, acffile1, specfile1, 0)
    acf_matrix.recipe1(vel, dt*0.02419, 5000.0, 1.0, acffile2, specfile2, 0)
...
if prefix == "test1":
    ## 2-particle system, without periodicity

    params = {"N":2, "dist_init":2.1, "scl":VECTOR(0.0, 0.0, 0.0), "mass":2000.0,
              "K":0.1, "dist_eq":2.0, "is_periodic":0,         
              "dt":20.0, "Nsteps":500, 
              "trajectory_filename":prefix+"/chain.xyz", "energy_filename":prefix+"/energy.txt",
              "acf_filename_mu":prefix+"/acf_mu.txt", "spectrum_filename_mu":prefix+"/spectrum_mu.txt",
              "acf_filename_vel":prefix+"/acf_vel.txt", "spectrum_filename_vel":prefix+"/spectrum_vel.txt"
             }
    md(params)

    params["dist_init"] = 2.0
    params["Hess_filename"] = prefix+"/Hessian.txt"
    params["normal_modes_filename"] = prefix+"/Modes.txt"
    ...
    ## 2-particle system, with periodicity

    params = {"N":2, "dist_init":2.1, "scl":VECTOR(0.0, 0.0, 0.0), "mass":2000.0,
              "K":0.1, "dist_eq":2.0, "is_periodic":1,        
              "dt":20.0, "Nsteps":500, 
              "trajectory_filename":prefix+"/chain.xyz", "energy_filename":prefix+"/energy.txt",
              "acf_filename_mu":prefix+"/acf_mu.txt", "spectrum_filename_mu":prefix+"/spectrum_mu.txt",
              "acf_filename_vel":prefix+"/acf_vel.txt", "spectrum_filename_vel":prefix+"/spectrum_vel.txt"
    ...
    ## 6-particle system, without periodicity

    params = {"N":6, "dist_init":2.1, "scl":VECTOR(0.0, 0.0, 0.0), "mass":2000.0,
              "K":0.1, "dist_eq":2.0, "is_periodic":0,        
              "dt":20.0, "Nsteps":500, 
              "trajectory_filename":prefix+"/chain.xyz", "energy_filename":prefix+"/energy.txt",
              "acf_filename_mu":prefix+"/acf_mu.txt", "spectrum_filename_mu":prefix+"/spectrum_mu.txt",
    ...
    ## 6-particle system, without periodicity, but with non-zero velocities

    params = {"N":6, "dist_init":2.1, "scl":VECTOR(0.0, 1.0, 0.0), "mass":2000.0,
              "K":0.1, "dist_eq":2.0, "is_periodic":0,        
              "dt":20.0, "Nsteps":2000, 
              "trajectory_filename":prefix+"/chain.xyz", "energy_filename":prefix+"/energy.txt",
              "acf_filename_mu":prefix+"/acf_mu.txt", "spectrum_filename_mu":prefix+"/spectrum_mu.txt",
