...
"""
  Lets try to run TSH for some Holstein models
"""
...
def compute_model(q, params, full_id):
    ...
    if model==1:
        res = models_Holstein.Holstein_uncoupled(q.col(indx), params)
...
def run_test(dt, md_run, ndia, nadi, nnucl, ntraj, _q, _p, _Cdia, _Cadi, _iM, model, rep, params1, rnd, _states):
    ...
    Ekin, Epot, Etot, dEkin, dEpot, dEtot = tsh.compute_etot_tsh(ham, p, None, Cadi, states, iM, rep) 
    ...
    for i in xrange(md_run):
        ...
        print i, Ekin, Epot, Etot, dEkin, dEpot, dEtot
...
def run_model(N):
    ...
    model = 1
    ndia, nadi, nnucl, ntraj = N, N, N, 100
