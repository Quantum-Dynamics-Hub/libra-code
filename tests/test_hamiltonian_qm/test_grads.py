...
#    F_x =       (  el.get_P_alp() * (dHao_dx + dFao_alp_dx) ).tr() #+ dP_alp_dx * (Hao + el.get_Fao_alp()) ).tr()   #
#    F_x = F_x + (  el.get_P_bet() * (dHao_dx + dFao_bet_dx) ).tr() #+ dP_bet_dx * (Hao + el.get_Fao_bet()) ).tr()   #
#     F_x = -0.5 * F_x                                                                                               
                                                                                                                    
#    F_y =       (  el.get_P_alp() * (dHao_dy + dFao_alp_dy) ).tr() #+ dP_alp_dy * (Hao + el.get_Fao_alp()) ).tr()   #
#    F_y = F_y + (  el.get_P_bet() * (dHao_dy + dFao_bet_dy) ).tr() #+ dP_bet_dy * (Hao + el.get_Fao_bet()) ).tr()   #
#     F_y = -0.5 * F_y                                                                                               
                                                                                                                    
#    F_z =       (  el.get_P_alp() * (dHao_dz + dFao_alp_dz) ).tr() #+ dP_alp_dz * (Hao + el.get_Fao_alp()) ).tr()   #
#    F_z = F_z + (  el.get_P_bet() * (dHao_dz + dFao_bet_dz) ).tr() #+ dP_bet_dz * (Hao + el.get_Fao_bet()) ).tr()   #
#    F_z = -0.5 * F_z
...
def energy_and_forces(syst, basis_ao, Nelec, Norb, atom_to_ao_map, ao_to_atom_map):
    ...
    for i in xrange(syst.Number_of_atoms):
        ...
        syst.Atoms[i].Atom_RB.rb_force.x = F[i][0]
        syst.Atoms[i].Atom_RB.rb_force.y = F[i][1]
        syst.Atoms[i].Atom_RB.rb_force.z = F[i][2]
   

    return E, F


def run_test(basis_ao, I, J):

    eri = electron_repulsion_integral(basis_ao[I],basis_ao[I],basis_ao[J],basis_ao[J],1,1)
    print "Analytic derivatives for dERI[I,I,J,J]/dR_I", 2.0*eri[1].x, 2.0*eri[1].y, 2.0*eri[1].z


    dx = 0.01
    dr = VECTOR(dx,0.0,0.0)

    basis_ao[I].shift_position(dr)
    eri_r = electron_repulsion_integral(basis_ao[I],basis_ao[I],basis_ao[J],basis_ao[J])

    basis_ao[I].shift_position(-2.0*dr)
    eri_l = electron_repulsion_integral(basis_ao[I],basis_ao[I],basis_ao[J],basis_ao[J])

    basis_ao[I].shift_position(dr)
    grad_x = ( eri_r - eri_l ) / (2.0*dx)




    dr = VECTOR(0.0,dx,0.0)
    basis_ao[I].shift_position(dr)
    eri_r = electron_repulsion_integral(basis_ao[I],basis_ao[I],basis_ao[J],basis_ao[J])
    ...
    basis_ao[I].shift_position(dr)
    grad_y = ( eri_r - eri_l ) / (2.0*dx)


    dr = VECTOR(0.0,0.0,dx)
    basis_ao[I].shift_position(dr)
    eri_r = electron_repulsion_integral(basis_ao[I],basis_ao[I],basis_ao[J],basis_ao[J])
    ...
    basis_ao[I].shift_position(dr)
    grad_z = ( eri_r - eri_l ) / (2.0*dx)


    print "Numeric gradient dERI[4,5]/dR[4]= ",  grad_x, grad_y, grad_z
