...
#    syst.show_molecules()
    ...
    md = MD({"integrator":"DLML", "dt":1.0,"ensemble":"NVT","max_step":10})
    therm = Thermostat({"thermostat_type":"Nose-Hoover", "NHC_size":2, "nu_therm":0.01, "Q":100.0, "Temperature":300.0})
    ...
    f = open("_en_cooling1.txt","w")
    f.close()


    for i in xrange(100):
        syst.set_atomic_q(mol.q)
        syst.print_xyz("_mol_cooling1.xyz",i)

        st.run_md(mol, el, ham)
        syst.cool()

        f = open("_en_cooling1.txt","a")
        f.write("i= %3i ekin= %8.5f  epot= %8.5f  etot= %8.5f\n" % (i, st.E_kin, st.E_pot, st.E_tot))
        f.close()

    ########################## Cooling # 2 #################################
    md.dt = 20.0
    f = open("_en_cooling2.txt","w")
    f.close()


    for i in xrange(100):
        syst.set_atomic_q(mol.q)
        syst.print_xyz("_mol_cooling2.xyz",i)

        st.run_md(mol, el, ham)
        syst.cool()

        f = open("_en_cooling2.txt","a")
        f.write("i= %3i ekin= %8.5f  epot= %8.5f  etot= %8.5f\n" % (i, st.E_kin, st.E_pot, st.E_tot))
        f.close()
    ...
    for i in xrange(100):
        ...
        f = open("_en_md.txt","a")
        f.write("i= %3i ekin= %8.5f  epot= %8.5f  etot= %8.5f eext= %8.5f currT= %8.5f\n" % (i, st.E_kin, st.E_pot, st.E_tot, st.H_NP, st.curr_T))
        f.close()
