...
for ie in rang: 
    ...
    for i in xrange(ntraj):
        mol = Nuclear(nnucl)
        mol.mass[0] = mass
        mol.q[0], mol.p[0], mol.f[0] = 0.0, 5.0+boltz(), 0.0
        MOL.append(mol)
        mollist.append(mol)
    ...
    for i in xrange(ntraj):
        ...
        ham.set_v( [MOL[i].p[0]/MOL[i].mass[0] ])
        ham.set_q( [MOL[i].q[0]] )
        HAM.append(ham) 
        hamlist.append(ham)
...
#        epot = compute_potential_energy(MOL[i], EL[i], HAM[i], 1)  # 0 - MF forces, 1 - FSSH (state-resolved) forces
    ...
    for snap in xrange(nsnap):
        ...
        f.write("%10.8f   %10.8f   %10.8f  %10.8f  %10.8f  %10.8f  %10.8f  %10.8f  %10.8f  %10.8f  %10.8f  %10.8f  %10.8f  %10.8f  %10.8f  %10.8f\n" % (t, pops[0], pops[1], x, sigmax_, e_kin, e_pot, e_tot, e_ext, pops_se[0], pops_se[1], p, sigmap_, pops_se[2], pops_se[3], pop0_ex))    
        f.close()
