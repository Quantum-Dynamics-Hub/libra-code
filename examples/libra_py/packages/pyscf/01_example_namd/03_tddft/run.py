"""Mixed-multiplicity TDDFT/FSSH dynamics for an Al3 cluster."""
...
from libra_py.packages.pyscf.implementations.tddft import TDDFT
...
q_init = [
    [q[idof] + 0.001 * q_fluct[idof, itraj] for itraj in range(ntraj)]
    for idof in range(ndof)
]
p_init = [
    [p[idof] + 0.001 * p_fluct[idof, itraj] for itraj in range(ntraj)]
    for idof in range(ndof)
]
...
# open-shell TDDFT analytic NAC response is not available, while
