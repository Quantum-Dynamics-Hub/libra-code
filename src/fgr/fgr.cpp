/*********************************************************************************
 * Copyright (C) 2019 Alexey V. Akimov, Xiang Sun
 *
...
namespace liblibra{
    ...
    namespace libfgr{
        ...
        complex<double> Integrand_NE_exact(double tp, double tau, double omega_DA, double omega, double req, double shift, double beta){
            ...
            im = -pref * sin(wt) - omega*req*shift * (sin(wtp) - sin(wtp - wt));//omega_DA*tau
        ...
        complex<double> ACF_NE_exact(double tp, double tau, double omega_DA, double V,
            ...
            complex<double> wdat(0.0, omega_DA*tau);
        ...
        complex<double> ACF_NE_Marcus(double tp, double tau, double omega_DA, double V,
            ...
            argg = wdat;
            for(int w=0; w<n_omega; w++){
                argg += Integrand_NE_Marcus(tp, tau, omega_DA, omega_nm[w], req_nm[w], shift_NE[w], beta);
                
                if(type==1){
                    ampl += Linear_NE_Marcus(tp, tau, gamma_nm[w], omega_nm[w], req_nm[w], shift_NE[w], beta);
                }
            }
        ...
        double NEFGRL_rate(double tp, double omega_DA, double V,
            ...
            /**
             ...
             k(t') = 2/(hbar^2) * Re { int_0^t' { dtau * C(t',tau) }  }
            ...
            for(int n=0; n<N; n++){ //tau index
                ...
                if(tau<=tp){
                    ...
                    switch(method){
                        ...
                        case 5: {  C = ACF_NE_Marcus(tp, tau, omega_DA, V, omega_nm, gamma_nm, req_nm, shift_NE, beta, type);    } break;
                        default: {
