...
void State::init_md(Electronic &el, Hamiltonian &ham, Random &rnd) {
  ...
  int Nf_b = 0;
  if (!is_system) {
    std::cout << "Fatal error: No system defined - nothing to simulate\n "
                 "Exiting now...\n";
    exit(1);
  }
  ...
  if (md->ensemble == "NVT" || md->ensemble == "NPT") {
    if (!is_thermostat) {
      std::cout << "Error: No Thermostat object were set up. Can not run "
                   "simulations in "
                << md->ensemble << " ensemble\n";
      exit(1);
    }
  }
  if (md->ensemble == "NPT" || md->ensemble == "NPH") {
    Nf_b = 1;
    if (!is_barostat) {
      std::cout << "Error: No Barostat object were set up. Can not run "
                   "simulations in "
                << md->ensemble << " ensemble\n";
      exit(1);
    }
  ...
  if (is_barostat && is_thermostat) {
    if (thermostat->thermostat_type != "Nose-Hoover" && md->ensemble == "NPT") {
      std::cout << "Error: Can not use thermostat other than Nose-Hoover for "
  ...
  if (is_barostat) {
    ...
    if (is_thermostat) {
      barostat->init(thermostat->Temperature);
    }
  }
  ...
  //!!!!!!!!!!  E_pot = system->energy(); !!!!!!!!!!!!!!!!!
  Nuclear mol(3 * syst->Number_of_atoms);
...
void State::run_md(Electronic &el, Hamiltonian &ham) {
  int i;
  if (md == NULL) {
    std::cout << "Error: MD parameters have not been defined\n";
    exit(1);
  }
  if (!is_md_initialized) {
    std::cout << "Error: Need to call init_md() first. MD is not initialized\n";
    exit(2);
  }

  int is_thermostat, is_barostat;
  is_thermostat = ((thermostat != NULL) &&
                   ((md->ensemble == "NVT") || (md->ensemble == "NPT") ||
                    (md->ensemble == "NPT_FLEX")));
  is_barostat = ((barostat != NULL) &&
                 ((md->ensemble == "NPT") || (md->ensemble == "NPT_FLEX") ||
                  (md->ensemble == "NPH") || (md->ensemble == "NPH_FLEX")));
  ...
  while (md->curr_step < md->max_step) {
    ...
    if (is_thermostat) {
      ...
      if (is_barostat) {
        ekin_baro = barostat->ekin_baro();
      }
      thermostat->update_thermostat_forces(syst->ekin_tr(), syst->ekin_rot(),
                                           ekin_baro);
      thermostat->propagate_nhc(dt_half, syst->ekin_tr(), syst->ekin_rot(),
                                ekin_baro);
    }

    if (is_thermostat) {
      thermostat->propagate_sPs(dt_half);
    }
    ...
    if (is_barostat) {
      if (md->ensemble == "NPT" || md->ensemble == "NPH") {
        barostat->update_barostat_forces(syst->ekin_tr(), syst->ekin_rot(),
                                         curr_V, curr_P);
      } else if (md->ensemble == "NPT_FLEX" || md->ensemble == "NPH_FLEX") {
        barostat->update_barostat_forces(syst->ekin_tr(), syst->ekin_rot(),
                                         curr_V, curr_P_tens);
      }
      scl = 0.0;
      if (is_thermostat) {
        scl = thermostat->get_ksi_b();
      }
      barostat->propagate_velocity(dt_half, scl);
    }
    // bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb

    double s_var, dt_over_s, dt_over_s2;
    s_var = 1.0;
    dt_over_s = dt;
    dt_over_s2 = dt;

    if (md->ensemble == "NVT" || md->ensemble == "NPT" ||
        md->ensemble == "NPT_FLEX" || md->ensemble == "NPH" ||
        md->ensemble == "NPH_FLEX") {
      if (is_thermostat) {
    ...
    if (is_barostat) {
      if (Nf_b == 9) {
        S = barostat->ksi_eps +
            (barostat->ksi_eps.tr() /
             (barostat->get_Nf_t() /*+barostat->get_Nf_r()*/)) *
                I;
      } else if (Nf_b == 1) {
        S = barostat->ksi_eps_iso * I +
            (3.0 * barostat->ksi_eps_iso /
             (barostat->get_Nf_t() /*+barostat->get_Nf_r()*/)) *
                I;
      }
    }
    if (is_thermostat) {
      S = S + thermostat->get_ksi_t() * I;
    }
    ...
    // ccccccccccccccccccccccccccccccc Core part
    ...
    for (i = 0; i < syst->Number_of_fragments; i++) {
      RigidBody &top = syst->Fragments[i].Group_RB;
      if (is_thermostat) {
        thermostat->propagate_Ps(0.5 * dt_over_s2 *
                                 (top.ekin_rot() + top.ekin_tr()));
      }
      double Ps;
      s_var = 1.0;
      if (is_thermostat) {
        s_var = thermostat->s_var;
      }
      Ps = 0.0;
      if (md->integrator == "Jacobi") {
        top.propagate_exact_rb(dt_over_s);
      } else if (md->integrator == "DLML") {
        top.propagate_dlml(dt_over_s, Ps);
      } else if (md->integrator == "Terec") {
        top.propagate_terec(dt_over_s);
      } else if (md->integrator == "qTerec") {
        top.propagate_qterec(dt_over_s);
      } else if (md->integrator == "NO_SQUISH") {
        top.propagate_no_squish(dt_over_s);
      } else if (md->integrator == "KLN") {
        top.propagate_kln(dt_over_s);
      } else if (md->integrator == "Omelyan") {
        top.propagate_omelyan(dt_over_s);
      }
    ...
    if (is_thermostat) {
      thermostat->propagate_Ps(dt *
                               (H0 - Nf * boltzmann * thermostat->Temperature *
                                         (log(thermostat->s_var) + 1.0)));
    }
    ...
    if (is_barostat) {
      if (syst->is_Box) {
        ...
        /*
                ...
                double max_n,min_n,max_o,min_o; // maximal and minimal heights
           new and old
                ...
                max_n = (h1_n>=h2_n)?h1_n:h2_n;      min_n =
           (h1_n<=h2_n)?h1_n:h2_n; max_n = (max_n>=h3_n)?max_n:h3_n;    min_n =
           (min_n<=h3_n)?min_n:h3_n;

                S.cross(t2_o,t3_o); h1_o = V_o/fabs(S.length());
                S.cross(t3_o,t1_o); h2_o = V_o/fabs(S.length());
                S.cross(t1_o,t2_o); h3_o = V_o/fabs(S.length());
                max_o = (h1_o>=h2_o)?h1_o:h2_o;      min_o =
           (h1_o<=h2_o)?h1_o:h2_o; max_o = (max_o>=h3_o)?max_o:h3_o;    min_o =
           (min_o<=h3_o)?min_o:h3_o;
    ...
    for (i = 0; i < syst->Number_of_fragments; i++) {
      syst->update_atoms_for_fragment(i);
    }
    syst->zero_forces_and_torques();
    ...
    Nuclear mol(3 * syst->Number_of_atoms);
    syst->extract_atomic_q(mol.q); // syst -> mol
    //    E_pot = compute_potential_energy(mol, el, ham, 1); //  # 1 - FSSH
    //    forces
    ...
    if (is_thermostat) {
      E_kin /= (thermostat->s_var * thermostat->s_var);
    }
    ...
    if (md->ensemble == "NVE") {
      ...
      if (is_thermostat) {
        if (!is_H0) {
          H0 = E_tot + thermostat->energy();
          is_H0 = 1;
        }
        if (thermostat->thermostat_type == "Nose-Poincare") {
          H_NP = thermostat->s_var * (E_tot + thermostat->energy() - H0);
        } else if (thermostat->thermostat_type == "Nose-Hoover") {
          H_NP = E_tot + thermostat->energy();
        }
      }
    } else if (md->ensemble == "NPH" || md->ensemble == "NPH_FLEX") {
      if (is_barostat) {
        H_NP = E_tot + barostat->ekin_baro() + curr_V * barostat->Pressure;
      }
