...
void update_Mull_orb_pop(MATRIX *P, MATRIX *S,
  ...
  /**
    ...
    \param[in,out] Mull_orb_pop_gross The vector which will collect
    orbital-resolved Mulliken gross populations
    \param[in,out] Mull_orb_pop_net The vector which will collect
    orbital-resolved Mulliken net populations
  */
  ...
  /*
    ...
    int Norb = Mull_orb_pop_gross.size(); // is assumed be the same as for the
    other array
...
} // void update_Mull_orb_pop(MATRIX* P, MATRIX* S, vector<double>&
  // Mull_orb_pop_gross,vector<double>& Mull_orb_pop_net)
...
void update_Mull_charges(vector<int> &fragment, vector<int> &basis_fo,
                         vector<vector<int>> &at_orbitals, vector<double> &Zeff,
                         vector<double> &Mull_orb_pop_gross,
                         vector<double> &Mull_orb_pop_net,
  ...
  /**
    ...
    \param[in] fragment  Contains indices of all nuclei, for which the Mull
    charges should be updated. So that fragment[a] - is the global index of the
    a-th atom of the fragment.
    \param[in]  basis_fo  Contains the global indices of the basis functions
    centered on the chosen (by the "fragment" variable) fragment
    \param[in] at_orbitals Mapping bewteen the local indices of the basis
    functions in the atom set and the global indices of all basis functions. For
    instance at_orbitals[n][i] is the global index of the i-th basis fucntion of
    n-th atoms.
  ...
  */
  ...
  for (int a = 0; a < Nat_frag; a++) { // O(Nfrag)
    ...
    for (auto i = 0u; i < at_orbitals[n].size(); i++) { // O(Nnucl)
      int j = at_orbitals[n][i];

      for (auto k = 0u; k < basis_fo.size(); k++) { // O(Nbas)
        if (basis_fo[k] == j) {
...
} // void update_Mull_charges(vector<double>& Mull_orb_pop_gross,
  // vector<double>& Mull_orb_pop_net, ...
...
void update_Mull_charges(vector<int> &ao_to_atom_map, vector<double> &Zeff,
  ...
  /**
    ...
    \param[in] ao_to_atom_map Mapping from the grobal indices of orbitals to the
    global indices nuclei: ao_to_atom_map[i] - is the index of the atom on which
    i-th AO is localized.
    \param[in] Zeff Effective charges of all nuclei
    \param[in] Mull_orb_pop_gross Mulliken gross populations on all orbitals
    \param[in] Mull_orb_pop_net Mulliken net populations on all orbitals
    \param[out] Mull_charges_gross Mulliken gross charges on all atoms
  ...
  */
  ...
  if (static_cast<int>(Mull_charges_gross.size()) != Nat) {
    cout << "Error in update_Mull_charges: The size of Mull_charges_gross is "
            "inconsistent with the size of Zeff\n";
    exit(0);
  }
  if (static_cast<int>(Mull_charges_net.size()) != Nat) {
    cout << "Error in update_Mull_charges: The size of Mull_charges_gross is "
            "inconsistent with the size of Zeff\n";
    exit(0);
  }

  for (auto a = 0; a < Nat; a++) { // all atoms
  ...
  for (auto i = 0u; i < ao_to_atom_map.size(); i++) { // O(AOs)

    const auto a =
        ao_to_atom_map[i]; // index of atom on which i-th AO is located
...
boost::python::list update_Mull_charges(vector<int> &ao_to_atom_map,
  ...
  update_Mull_charges(ao_to_atom_map, Zeff, Mull_orb_pop_gross,
                      Mull_orb_pop_net, Mull_charges_gross, Mull_charges_net);
