...
/**
  \file Interactions.h
  \brief The file describes functions and classes for molecular-mechanical
...
template <class X> vector<X> value_subset(vector<X> &r_in, vector<int> &indxs) {
  /**
    This is an auxiliary function that sets the values in r_out to the values of
    a sub-set of pointers in r_in. The subset is determined by the vector of
    indices "indxs"
  ...
  */
  ...
  for (int i = 0; i < sz_sub; i++) {
    ...
    if (indxs[i] >= sz_in) {
      cout << "Error: Sub-set indxed " << indxs[i]
           << " is beyond the size of the set" << sz_in << endl;
      exit(0);
    } // if
...
void address_subset(vector<X *> &r_out, vector<X *> &r_in, vector<int> &indxs) {
  ...
  */
  ...
  if (static_cast<std::size_t>(sz_out) != indxs.size()) {
    cout << "The size of the output vector " << sz_out
         << " is not equal to the size to the subset " << indxs.size() << endl;
    cout << "Are you sure you have allocated memory for the r_out?\n";
    exit(0);
  }

  for (auto i = 0; i < sz_out; i++) {
...
void address_subset(vector<X *> &r_out, vector<X> &r_in, vector<int> &indxs) {
  /**
    This is an auxiliary function that sets the pointers in r_out to the
    addresses of a a sub-set of VECTORS in r_in. The subset is determined by the
    vector of indices "indxs"
...
// void address_subset(vector<VECTOR*>& r_out, vector<VECTOR*>& r_in,
// vector<int>& indxs); void address_subset(vector<VECTOR*>& r_out,
// vector<VECTOR>& r_in, vector<int>& indxs);
...
public:
  ...
  vector<VECTOR *> t; ///< list of the pointers to the vectors representing
                      ///< periodic translations of the actoms
