...
double Elec_Ewald3D(vector<VECTOR *> &r, vector<double> &q,
                    ...
                    vector<VECTOR *> &excl_vector2, vector<double> &excl_scales,
                    int rec_deg, int pbc_deg, double etha, double R_on,
  ...
  for (int na = -pbc_deg; na <= pbc_deg; na++) {
    for (int nb = -pbc_deg; nb <= pbc_deg; nb++) {
      for (int nc = -pbc_deg; nc <= pbc_deg; nc++) {
        ...
        //  Note: It is very important that the translation vectors are
        //  symmetric, leading to: summ (over tv)  is equivalent to summ (over
        //  -tv)
          ...
          for (int j = 0; j < sz; j++) {
            ...
            if (na == 0 && nb == 0 && nc == 0 && i == j) {
              ...
              if (SW > 0.0) {
                ...
                s1mod = etha_13 * SW * (Qij / (a * a)) *
                        (erfc1 + const2 * exp(-a * a));
                f_mod = 0.5 * (s1mod * rij - (erfc1 / etha) * Qij * dSW);
                *f[i] += f_mod;
                *f[j] -= f_mod;

                tp.tensor_product(rij, f_mod);
                at_stress += tp;
  ...
  for (int e = 0; e < nexcl; e++) {
    ...
    VECTOR rij;
    rij = (*r[i] + *excl_vector1[e]) - (*r[j] + *excl_vector2[e]);
    ...
    if (i != j) {
      ...
      // Scaling works differently: we already have included the full
      // interaction in the reciprocal space, so we are going to exclude the
      // entire one.. The scaling returns part of it. E.g. if scaling is 0.2 we
      // are going to add overall -0.8
      E1_excl += (scl - 1.0) * en;
      *f[i] += (scl - 1.0) * fi;
      *f[j] -= (scl - 1.0) * fi;

      tp.tensor_product(rij, fi);
      at_stress -= (scl - 1.0) * tp;
    }
  ...
  for (Lx = -rec_deg; Lx <= rec_deg; Lx++) {
    for (Ly = -rec_deg; Ly <= rec_deg; Ly++) {
      for (Lz = -rec_deg; Lz <= rec_deg; Lz++) {
        ...
        //  Note: It is very important that the translation vectors are
        //  symmetric, leading to: summ (over h)  is equivalent to summ (over
        //  -h)
        ...
        if ((Lx == 0) && (Ly == 0) && (Lz == 0)) {
          ...
          tp.tensor_product(h, h);
          tp = sum3 * const1 * (I - 2.0 * ((1.0 + b * b) / (h * h)) * tp);
          at_stress += tp;
...
double Elec_Ewald3D(vector<VECTOR> &r, vector<double> &q, MATRIX3x3 &box,
                    double epsilon,                          /* Inputs */
                    vector<VECTOR> &f, MATRIX3x3 &at_stress, /* Outputs*/
                    int rec_deg, int pbc_deg, double etha, double R_on,
                    double R_off /* Parameters */
) {
  /**
    ...
    This function takes coordinates in a.u. (Bohrs) and returns the energy in
    a.u. (Hatree)
  */
            ...
            if (na == 0 && nb == 0 && nc == 0 && i == j) {
              ...
              if (SW > 0.0) {
                ...
                */
                ...
                /**
                       dE/dr_i = (0.5/etha) * (q[i]*q[j]/epsilon) * (ERFC(a)/a)
                * dSW/dr_i + SW *  q[i]*q[j]/epsilon * (0.5/etha) * d
                (ERFC(a)/a)/da * da/dr_i

                       d(ERFC(a)/a)/da =   ((-2*exp(-a**2) / sqrt(pi) ) * a -
                ERFC(a)  )/a**2 =

                       = -(  (2*exp(-a**2)/ sqrt(pi) )  + (ERFC(a)/a)  ) /a

                       da/dr_i  = (rij / sqrt(rijt) ) / etha = rij / (sqrt(rijt)
                * etha) = rij / (a * etha**2)

                      So, the last term together:

                 - SW *  q[i]*q[j]/epsilon * (0.5/etha**3) * rij * (
                (2*exp(-a**2)/ sqrt(pi) )  + (ERFC(a)/a)  ) /a**2
                ...
                */
                ...
                VECTOR derfc1_dri =
                    -((epsilon * const2 * exp(-a * a) + erfc1) / rijt) * rij;
                VECTOR dEdri = const3 * Qij * (derfc1_dri * SW + erfc1 * dSW);

                f[i] -= dEdri;
                f[j] += dEdri;

                tp.tensor_product(rij, dEdri);
                at_stress -= tp;
      ...
      for (int Lz = -rec_deg; Lz <= rec_deg; Lz++) {
        ...
        if ((Lx == 0) && (Ly == 0) && (Lz == 0)) {
          ...
          for (auto i = 0; i < sz; i++) {
            ...
            f_mod *=
                const1 * 2.0 * fact * q[i] *
                (sin(h * r[i]) * sum1 - cos(h * r[i]) * sum2); // -dsum3/dr_i
            f[i] += f_mod;
          }
...
double Elec_Ewald3D(VECTOR *r, /* Inputs */
                    VECTOR *g, VECTOR *m, VECTOR *f, MATRIX3x3 &at_stress,
                    MATRIX3x3 &fr_stress, MATRIX3x3 &ml_stress, /* Outputs*/
                    int sz, double *q, int nexcl, int *excl1, int *excl2,
                    double *scale, MATRIX3x3 *box, int rec_deg, int pbc_deg,
                    double etha, int is_cutoff, double R_on, double R_off,
                    int &time, vector<vector<triple>> &images,
                    vector<triple> &central_translation, double *dr2,
                    double dT_2, int &update_displ2 /* Parameters */
) {
  //********************* double ELECTROSTATIC *****************************
  //* 3D Ewald summation given by:                                         *
  //*  Karasawa, N.; Goddard III,W. A. "Acceleration of Convergence for    *
  ...
  /*
    ...
    //  Note: It is very important that the translation vectors are symmetric,
    leading to:
  ...
  */
  ...
  double scl =
      2.0; // use 1 if full i,j loops are used: (i in [1,sz]) and (j in [1,sz])
      ...
      for (int im = 0; im < n_images; im++) {
        ...
        if ((na == xshift) && (nb == yshift) && (nc == zshift) && (i == j)) {
          ...
          if (SW > 0.0) {
            ...
            s1mod =
                etha_13 * SW * (Qij / (a * a)) * (erfc1 + const2 * exp(-a * a));
            f_mod = 0.5 * (s1mod * rij - (erfc1 / etha) * Qij * dSW);
            f_mod *= scl;
            f[i] += f_mod;
            f[j] -= f_mod;
            ...
            tp.tensor_product(rij, f_mod);
            at_stress += tp;
            tp.tensor_product(gij, f_mod);
            fr_stress += tp;
            tp.tensor_product(mij, f_mod);
            ml_stress += tp;
      ...
      for (Lz = -rec_deg; Lz <= rec_deg; Lz++) {
        ...
        if ((Lx == 0) && (Ly == 0) && (Lz == 0)) {
          ...
          for (i = 0; i < sz; i++) {
            ...
            tp.tensor_product((r[i] - g[i]), -f_mod);
            fr_stress += tp;
            tp.tensor_product((r[i] - m[i]), -f_mod);
            ml_stress += tp;
          }
