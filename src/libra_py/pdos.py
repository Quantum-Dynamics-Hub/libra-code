    ...
    for proj in projections:  # loop over all projection
        ...
        for a in atoms:  # open files for atoms with given indices (indexing from 1)
            for symb in ang_mom:  # for given angular momentum labels
                for wfc in range(
                        0, 5):  # Specify max wfc type index - usually no more than 3, 5 - should be more than enough
                    for Elt in elements:  # for given atom names

                        if nspin == 4:
                            for k in [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0]:  # total angular momentum label
                                filename = prefix + str(a) + "(" + Elt + ")_wfc#" + str(wfc) + \
                                    "(" + symb + "_j" + str(k) + ")"
        ...
        for proj in projections:  # loop over all projection
            ...
            for Elt in elements:  # for given atom names
                ...
                if os.path.exists(filename):
                    ...
                    # the second to the last entry of the first line contains Fermi energy in a.u.
                    Ef = float(check[-2]) * units.au2ev
                    # convert to eV
                    # For debug
...
def convolve_cp2k_pdos(params: dict):
    ...
    for j in range(1, len(angular_momentum_cols) + 1):
        ...
        for i in range(0, num_levels):
            ...
            gaussian_fun = pre_factor * \
                (np.exp(-0.5 * np.power(((energy_grid - float(pdos_sum[i][0]) * energy_conversion) / sigma), 2)))
