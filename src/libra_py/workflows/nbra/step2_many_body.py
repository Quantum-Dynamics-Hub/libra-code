...
def reindex_cp2k_sd_states(
        ks_orbital_homo_index,
        ks_orbital_indicies,
        sd_basis_states,
        sd_format=2,
        ks_beta_homo_index=0,
...
def run_step2_many_body(params):
    ...
    * Molecular orbital visualization using VMD
    ...
    if es_software == "cp2k":
        ...
        os.system("mpirun -np %d %s -i %s-%i.inp -o logfiles/step_%d.log " %
                  (nprocs, cp2k_exe, project_name, curr_step, curr_step))
        ...
        Gaussian_methods.cube_generator_gaussian(project_name,
                                                 curr_step,
                                                 ks_orbital_indicies[0],
                                                 ks_orbital_indicies[-1],
                                                 nprocs,
                                                 '../../sample_cube_file.cube',
                                                 int(params["isUKS"]))
        ...
        DFTB_methods.cube_generator_dftbplus(project_name,
                                             curr_step,
                                             ks_orbital_indicies[0],
                                             ks_orbital_indicies[-1],
                                             waveplot_exe,
    ...
    for step in range(nsteps_this_job - 1):
        ...
        np.savetxt("%s/S_ks_%d_re" %
                   (res_dir, int(params["istep"]) +
                    step), S_ks_job[step], fmt='%.16e', delimiter=" ")
        np.savetxt("%s/E_ks_%d_re" %
                   (res_dir, int(params["istep"]) +
                    step), E_ks_job[step], fmt='%.16e', delimiter=" ")
        np.savetxt("%s/St_ks_%d_re" %
                   (res_dir, int(params["istep"]) +
                    step), St_ks_job[step], fmt='%.16e', delimiter=" ")

        curr_step += 1

    # Print out the KS Overlap and Energy matricies for the last step in this job batch
    np.savetxt("%s/S_ks_%d_re" % (res_dir, int(params["istep"]) +
               step + 1), S_ks_job[step + 1], fmt='%.16e', delimiter=" ")
    np.savetxt("%s/E_ks_%d_re" % (res_dir, int(params["istep"]) +
               step + 1), E_ks_job[step + 1], fmt='%.16e', delimiter=" ")
