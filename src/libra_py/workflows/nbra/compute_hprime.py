...
def compute_hprime_dia(es, info, filename):
    ...
    Hprime[0].real().show_matrix("%sx_re" % (filename))
    Hprime[0].imag().show_matrix("%sx_im" % (filename))
    Hprime[1].real().show_matrix("%sy_re" % (filename))
    Hprime[1].imag().show_matrix("%sy_im" % (filename))
    Hprime[2].real().show_matrix("%sz_re" % (filename))
    Hprime[2].imag().show_matrix("%sz_im" % (filename))
...
def hprime_py(es, info, filename):
            ...
            for j in range(0, sz):
                ...
                if (is_compl == 1):
                    if (g == 0):
                        hx = tmp.real * gx
                        hy = tmp.real * gy
                        hz = tmp.real * gz  # This should give zero!
    ...
    Hx.real().show_matrix("%sx_re" % (filename))
    Hx.imag().show_matrix("%sx_im" % (filename))
    Hy.real().show_matrix("%sy_re" % (filename))
    Hy.imag().show_matrix("%sy_im" % (filename))
    Hz.real().show_matrix("%sz_re" % (filename))
    Hz.imag().show_matrix("%sz_im" % (filename))
