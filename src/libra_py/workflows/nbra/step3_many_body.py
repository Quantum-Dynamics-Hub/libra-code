...
def get_step2_mb_sp_properties(_params):
    ...
    excitation_energies, ci_basis_raw, ci_coefficients_raw_unnorm, spin_components = step2_many_body.get_excitation_analysis_output(
        params)
    ...
    # return S_sd_job, St_sd_job, sd_basis_states_unique, ci_basis_states_job,
...
def make_T_matrices(ci_coefficients, ci_basis_states, spin_components, sd_states_unique_sorted, _params):
    ...
    for step in range(fstep - istep):
        ...
        for i in range(number_of_states):
            ...
            if verbosity >= 3:
                print(
                    F"      handling CI {i}, number of configurations = {nconf},  ci_coeffs",
                    ci_coefficients[step][i])
            ...
            for j in range(nSDs - 1):
                ...
                if sd_states_unique_sorted[step][j] in tmp_ci_basis_state_and_spin:
                    ...
                    if verbosity >= 4:
                        print(
                            F"      found match: j = {j}, what = { sd_states_unique_sorted[step][j]}, its index = {item_index} ")
                        print(F" {val} goes to matrix element {i+1}, {j+1}")

                    # ci_coefficients_libra[step][i+1][j+1] =
                    # float(ci_coefficients[step][i][item_index])  # deprecate - AVA 7/8
...
def run(_params):
    ...
    # 2.2. Reindex the single-particle excitations into a format expected by
    # Libra: keeping in mind Slater Determinants notation
    sd_states_reindexed = step2_many_body.reindex_cp2k_sd_states(
        homo_index, orbital_indices, sd_basis_states_unique, sd_format=2)
