...
def traj_statistics(i, Coeff, istate, Hvib, itimes):
    ...
    # ================ Dimensions ==================
    # total number of trajectory = data set size x number of init times x number of SH trajectories
    Ntraj = len(Coeff)
            ...
            for it_indx in range(0, nitimes):  # over all initial times
                ...
                for tr in range(0, ntraj):  # over all stochastic trajectories
                    ...
                    if params["decoherence_method"] in [0, 1, 2]:
                        ...
                        istate[Tr], Coeff[Tr] = tsh.hopping(
                            Coeff[Tr], Heff, istate[Tr], params["sh_method"], do_collapse, ksi, ksi2, dt, T, bolt_opt)

                    elif params["decoherence_method"] in [3]:  # DISH

                        tau_m[Tr] = coherence_intervals(Coeff[Tr], dephasing_rates)
                        istate[Tr] = tsh.dish_py(
                            Coeff[Tr], istate[Tr], t_m[Tr], tau_m[Tr], Heff, bolt_opt, T, ksi, ksi2)
                        t_m[Tr] += dt
    ...
    for istate in init_states:  # initial states
        ...
        for method in tsh_methods:  # decoherence method: FSSH, IDA, mSDM, DISH
            ...
            if n > 0:
                ...
                rat_stat = DATA(rat)
                rat_stat.Calculate_Estimators()
                rat2_stat = DATA(rat2)
                rat2_stat.Calculate_Estimators()
                tau_stat = DATA(tau)
                tau_stat.Calculate_Estimators()
                tau2_stat = DATA(tau2)
                tau2_stat.Calculate_Estimators()
                ...
                if istate in [-1] and method in [0]:
                    ...
                    # tau_stat.sd #( tau_stat.ave / rat_stat.ave) *rat_stat.sd
                    ERR = prefactor * 0.5 * TAU**3 * rat2_stat.sd
...
def get_Hvib_scipy(params):
    ...
    for i in range(len(params['data_set_paths'])):
        ...
        for step in range(params['nfiles']):
            ...
            try:
                file_name = params['data_set_paths'][0] + params['Hvib_re_prefix'] + \
                    str(step) + params['Hvib_re_suffix'] + '.npz'
            ...
            try:
                file_name = params['data_set_paths'][0] + params['Hvib_im_prefix'] + \
                    str(step) + params['Hvib_im_suffix'] + '.npz'
