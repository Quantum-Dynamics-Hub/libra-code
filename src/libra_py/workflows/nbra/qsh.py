...
def compute_freqs(H_vib, params):
    ...
    freqs, T, norm_acf, raw_acf, W, J, J2 = influence_spectrum.compute_all(
        H_vib, params)  # T in fs, W and freqs in cm^-1
...
def run(H_vib, params):
    ...
    for idata in range(0, ndata):   # over all MD trajectories (data sets)
        ...
        for i in range(0, nsteps):
            ...
            if params["do_QSH_output"]:
                # ============= Output the resulting QSH Hamiltonians ===========================
                re_filename = params["output_set_paths"][idata] + \
                    params["qsh_Hvib_re_prefix"] + str(i) + params["qsh_Hvib_re_suffix"]
                im_filename = params["output_set_paths"][idata] + \
                    params["qsh_Hvib_im_prefix"] + str(i) + params["qsh_Hvib_im_suffix"]
