                        ...
                        if params["Nstart"] < i:
                            for k in xrange(el_mts):
                                if params["smat_inc"] == 1:
                                    el[cnt_inc_el].propagate_electronic(
                                        0.5 * dt_elec, ham[cnt], smat)  # el propagate using S-matrix
                                else:
...
# well, actually, it seems the code works fine even without the above instruction
                        ...
                        if params["interface"] == "GAMESS":
                            ...
                            E[cnt], E_SD, nac, sd_basis[cnt], all_grads, mu[cnt] = gamess_to_libra(
                                params, ao[cnt], E[cnt], sd_basis[cnt], active_space, str(ij))  # E_mol_red -> E_SD
                            ...
                            # update AO, MO, and gradients. Note: add 0 index on sd_basis[cnt] here.
                            E[cnt], E_SD, nac, sd_basis[cnt], all_grads, mu[cnt] = g09_to_libra(
                                params, ao[cnt], E[cnt], sd_basis[cnt], active_space, str(ij))  # E_mol_red -> E_SD
                            # tot_ene.append(tot_ene0); mu.append(mu0); # store total energy and dipole moment
                            ...
                            if params["non-orth"] == 1:
                                ...
                                grads_non_orth = []
                                E_SD, nac, smat, E[cnt], sd_basis[cnt], grads_non_orth = qe_to_libra(
                                    params, E[cnt], sd_basis[cnt], label[cnt], mol[cnt], str(ij), active_space)
                            else:
                                E_SD, nac, smat, E[cnt], sd_basis[cnt], all_grads = qe_to_libra(
                                    params, E[cnt], sd_basis[cnt], label[cnt], mol[cnt], str(ij), active_space)
                        ...
                        if params["non-orth"] == 1:
                            cmt = vibronic_hamiltonian_non_orth(
                                ham_adi[cnt], ham_vib[cnt], params, E_SD_old, E_SD, nac, smat_old, smat, str(ij))
                        ...
                        if params["is_MM"] == 1:
                            ...
                            for k in xrange(syst[cnt].Number_of_atoms):
                                for st in xrange(nstates):
                                    d1ham_adi[cnt][3 * k + 0].set(st, st, qm_frac *
                                                                  all_grads[st][k].x - mm_frac * mol[cnt].f[3 * k + 0])
                                    d1ham_adi[cnt][3 * k + 1].set(st, st, qm_frac *
                                                                  all_grads[st][k].y - mm_frac * mol[cnt].f[3 * k + 1])
                                    d1ham_adi[cnt][3 * k + 2].set(st, st, qm_frac *
                                                                  all_grads[st][k].z - mm_frac * mol[cnt].f[3 * k + 2])
                        else:
                            for k in xrange(syst[cnt].Number_of_atoms):
                                for st in xrange(nstates):
                        ...
                        epot[cnt] = compute_forces(mol[cnt], el[cnt_inc_el], ham[cnt],
                                                   f_pot)  # f_pot = 0 - Ehrenfest, 1 - TSH
                        # epot[cnt] = qm_frac*epot[cnt] + mm_frac*epot_mm[cnt]            # Note:
                        # epot is evaluated @ t+dt/2 while epot_mm is @ t+dt
                        # the above conflict is repaired.
                        epot[cnt] = qm_frac * E[cnt].get(el[cnt_inc_el].istate,
                                                         el[cnt_inc_el].istate) + mm_frac * epot_mm[cnt]
                        # for propagating thermostat variables.
            ...
            if SH_type >= 1 and params["Nstart"] < i:
                ...
                if params["interface"] == "GAMESS":
                    if params["do_rescaling"] == 1:  # default
                        ...
                        # velocity rescaling is not done; Electronic back reaction is neglected.
                        tsh.surface_hopping_cpa(mol, el, ham, rnd, params)
                ...
                for iconf in xrange(nconfig):
                    for i_ex in xrange(nstates_init):
                        ...
                        for itraj in xrange(num_SH_traj):
                            ...
                            el[cnt_inc_el].istate, el[cnt_inc_el] = tsh.ida_py(
                                el[cnt_inc_el], old_st[cnt_inc_el], new_st, E_old, E_new, params["Temperature"], ksi, params["do_collapse"])
            ################### END of TSH ##########################
            print "Finished TSH"
            t.stop()
