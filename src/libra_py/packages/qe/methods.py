...
def read_qe_index(filename, orb_list, verbose=0):
    ...
    info["nspin"] = int(float(ctx.get("Eigenvalues/<xmlattr>/nspin", "-1.0")))
...
def read_qe_wfc_info(filename, verbose=0):
    ...
    res = {}
    # the number of plane waves needed to represent the orbital
    res["ngw"] = int(float(ctx.get("Info/<xmlattr>/ngw", "-1.0")))
    # the number of the G points = plane waves needed to
    res["igwx"] = int(float(ctx.get("Info/<xmlattr>/igwx", "-1.0")))
    ...
    # 1 - unpolarized, 2 - polarized, 4 - non-collinear
    res["nspin"] = int(float(ctx.get("Info/<xmlattr>/nspin", "-1.0")))
    # T - use the Gamma-point storae trick, T - do not use it
    res["gamma_only"] = ctx.get("Info/<xmlattr>/gamma_only", "F")
...
def read_md_data_cell(filename):
    ...
    for t in range(0, nsteps):
        ...
        C.set(0, t, float(a1_str[0]))
        C.set(1, t, float(a1_str[1]))
        C.set(2, t, float(a1_str[2]))
        C.set(3, t, float(a2_str[0]))
        C.set(4, t, float(a2_str[1]))
        C.set(5, t, float(a2_str[2]))
        C.set(6, t, float(a3_str[0]))
        C.set(7, t, float(a3_str[1]))
        C.set(8, t, float(a3_str[2]))
    ...
    for a in f:
        if (start == 1 or start == 2) and t < T and nframe == dt:
            ...
            if is_alat == 0:
                ...
                if a.find("ATOMIC_POSITIONS") != -1:
                    # Set flag to write positions
                    # print "Atomic_positions record is found! nframe=%d t=%d start=%d" %
                    # (nframe,t,start)
                    if t < T:
                        if nframe == dt:
