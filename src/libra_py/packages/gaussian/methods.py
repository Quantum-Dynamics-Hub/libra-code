...
def gaussian_distribute(istep, fstep, nsteps_this_job, trajectory_xyz_file, gaussian_input, curr_job_number):
    ...
    for step in range(nsteps_this_job):
        ...
        sz = len(A)
        tmp.close()
        tmp2 = open("step_%d" % curr_step + ".gjf", "w")
        tmp2.close()
        for i in range(sz):
...
def cube_generator_gaussian(project_name, time_step, min_band, max_band, nprocs, sample_cube_file, isUKS):
    ...
    if isUKS == 1:
        # Generate the names and cube files for each state. Here we use the cube
        # names by CP2K. This will increase the speed of our work.
        for state in range(min_band, max_band + 1):
            ...
            os.system('cubegen %d AMO=%d %s-%d.fchk %s -1 h %s' %
                      (nprocs, state, project_name, time_step, cube_name, sample_cube_file))
            print('cubegen %d AMO=%d %s-%d.fchk %s -1 h %s' %
                  (nprocs, state, project_name, time_step, cube_name, sample_cube_file))
            cube_name = '%s-%d-WFN_%s_2-1_0.cube' % (project_name, time_step, state_name)
            print('Generating cube for state %d' % state)
            # Generate cube files for beta spin using the 'cubegen'
            os.system('cubegen %d BMO=%d %s-%d.fchk %s -1 h %s' %
                      (nprocs, state, project_name, time_step, cube_name, sample_cube_file))
            print('cubegen %d BMO=%d %s-%d.fchk %s -1 h %s' %
                  (nprocs, state, project_name, time_step, cube_name, sample_cube_file))
    # Spin restricted case
        ...
        for state in range(min_band, max_band + 1):
            ...
            os.system('cubegen %d MO=%d %s-%d.fchk %s -1 h %s' %
                      (nprocs, state, project_name, time_step, cube_name, sample_cube_file))
            print('cubegen %d MO=%d %s-%d.fchk %s -1 h %s' %
                  (nprocs, state, project_name, time_step, cube_name, sample_cube_file))
