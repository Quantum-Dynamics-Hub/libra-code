...
def overlap(st_mo, data1, data2, params):
    ...
    This routine:
      ...
      3. Computes spin-adapted SD and CSF overlap matrices
    ...
    params : dict
        Dictionary of required parameters:
            ...
            spin : int or float, optional
                Total spin S of the excited-state multiplet (default: 0).
            spin_projection : int or float, optional
                Spin projection Ms (default: 0).
            reference_det : iterable of int, optional
                Explicit open-shell reference determinant in signed
                spin-orbital notation. If omitted, a closed-shell reference
                is generated by the overlap interface.
    ...
    Notes
    -----
    - Without ``reference_det``, the reference is a closed-shell singlet.
      An explicit determinant enables open-shell spin manifolds.
    ...
    spin_projection = params.get("spin_projection", 0)
    reference_det = params.get("reference_det", None)
    reference_det_eff = reference_det
    ...
    #-------------------------------------------------------------------
    nelec_eff = 0
    if reference_det is not None:
        if active_space is None:
            nelec_eff = len(reference_det)
        else:
            reference_det_eff = [
                orb for orb in reference_det if abs(orb) in active_space
            ]
            nelec_eff = len(reference_det_eff)
    ...
    # ------------------------------------------------------------------
    # SD and spin-adapted CSF overlap matrices
    # ------------------------------------------------------------------
    ...
    csf_ovlp, sd_ovlp = interfaces.sd_and_csf_overlaps(
        st_mo,
        lowest_orbital,
        highest_orbital,
        nelec_eff,
        ...
        S=spin,
        Ms=spin_projection,
        reference_det=reference_det_eff,
    )
