...
def Pollien_Arribas_Agostini(q, _params, full_id=None):
    ...
    x = math.tanh((r - d12) / beta12)
    dx_dr = (1.0 - x**2) / beta12
    lambda_12_r = 0.5 * lambda_12_max * (1 - x)
    ...
    x = math.tanh((r - d23) / beta23)
    dx_dr = (1.0 - x**2) / beta23
    lambda_23_r = 0.5 * lambda_23_max * (1 - x)
    ...
    dV11_dr = dv10_dr + dv11_dr * (1 - cs2)
    dV11_dtheta = 2.0 * v11 * si2
    dV22_dr = dv20_dr + dv21_dr * (1 - cs2) + dv22_dr * (1 - cs2)**2
    dV22_dtheta = 2.0 * v21 * si2 + 4.0 * v22 * (1 - cs2) * si2
    dV33_dr = dv30_dr + dv31_dr * (1 - cs2)
    dV33_dtheta = 2.0 * v31 * si2

    obj = tmp()
    obj.ham_dia = CMATRIX(3, 3)
    obj.ovlp_dia = CMATRIX(3, 3)
    ...
    # Hamiltonian matrix
    obj.ham_dia.set(0, 0, V11 * (1.0 + 0.0j))
    obj.ham_dia.set(0, 1, V12 * (1.0 + 0.0j))
    obj.ham_dia.set(0, 2, V13 * (1.0 + 0.0j))
    obj.ham_dia.set(1, 0, V12 * (1.0 + 0.0j))
    obj.ham_dia.set(1, 1, V22 * (1.0 + 0.0j))
    obj.ham_dia.set(1, 2, V23 * (1.0 + 0.0j))
    obj.ham_dia.set(2, 0, V13 * (1.0 + 0.0j))
    obj.ham_dia.set(2, 1, V23 * (1.0 + 0.0j))
    obj.ham_dia.set(2, 2, V33 * (1.0 + 0.0j))
