...
def sync(univ,mss,mom_calc,props,model_params,qpas1,c1_new,qpas2,c2_new,norm2,beta):
	"""Returns the values for the new basis parameter matrices on surfaces 1 (*qpas1n*) and 2 (*qpas2n*), as well as their corresponding projection vectors *b1* and *b2*, where the motion of both sets of functions are synced to the lower energetic surface while the density on the upper surface is less than a threshold value specified by the *decpl* parameter. Also necessary are the functions for calculating momentum (*mom_calc*) and basis updates (*props*).
	...
	decpl=mss['decpl'];mirror=mss['mirror']
	...
	if norm2 < decpl and mirror=='true':
		qpas2n=[MATRIX(qvalsn),MATRIX(pvalsn),MATRIX(avalsn),MATRIX(svalsn)]
	elif norm2 < decpl and mirror=='false':
		qpas2n=[MATRIX(qvalsn),MATRIX(qpas2[1]),MATRIX(qpas2[2]),MATRIX(qpas2[3])]
	else:
		...
		for i in range(ndof):
			qn2=qprop(univ,i,qvals.row(i),mom2.row(i),mss);pn2=pprop(mom2.row(i));an2=aprop(univ,i,avals.row(i),gmom2.row(i),mss);sn2=sprop(univ,i,svals.row(i))
			for j in range(ntraj):
				qvalsn.set(i,j,qn2.get(j))
				pvalsn.set(i,j,pn2.get(j))
				avalsn.set(i,j,an2.get(j))
				svalsn.set(i,j,sn2.get(j))
...
def two_surf(univ,mss,mom_calc,props,model_params,qpas1,c1_new,qpas2,c2_new,norm2,beta):
	"""Returns the values for the new basis parameter matrices on surfaces 1 (*qpas1n*) and 2 (*qpas2n*), as well as their corresponding projection vectors *b1* and *b2*, where the basis parameters are calculated in pairs, although the surface assignment alternates with each function (i.e. even basis are ground surface, odd basis are excited surface). Also necessary are the functions for calculating momentum (*mom_calc*, although specifically mom_avg here) and basis updates (*props*).
