    ...
    if mom_calc_type == 1:
        for dof in range(ndof):
            ...
            for i in range(ntraj_on_surf):
                ...
                mom.set(dof, i, aa)
                r.set(dof, i, bb)
                gmom.set(dof, i, x.get(1, 0))
                gr.set(dof, i, x.get(1, 1))
...
def propagate_basis(_q, _p, _alp, _s, _states, coeff, dyn_params, surf_pops):
    ...
    # 0 - use the current value, 1 - use the value from the most populated surface
    q_sync_method = params["q_sync_method"]
    # 0 - use the current value, 1 - use the value from the most populated surface
    p_sync_method = params["p_sync_method"]
    # 0 - use the current value, 1 - use the value from the most populated surface
    a_sync_method = params["a_sync_method"]
    # 0 - use the current value, 1 - use the value from the most populated surface
    s_sync_method = params["s_sync_method"]
...
#            qpas_on_surf = [q_on_surf, p_on_surf, a_on_surf, s_on_surf] # variables for the TBFs on the surface n
            ...
            # gr - MATRIX(ndof, ntraj_on_surf) - gradient of the fit of r ~
            # nubla_{\alp} Re( nubla_{\alp} \psi / \psi)  - to update alphas
