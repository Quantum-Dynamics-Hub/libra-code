...
def model_ho_diab_nD(ndof,q,nsurf,model_params):
...
def model_t1_diab_nD(ndof,q,nsurf,model_params):
...
#	d1=0.005;d2=1.0;d3=0.0
	...
	fx=0.0;dfx=[];d2fx=[]
	if (nsurf==1):
		fx=model_params['a'][0]*(1.0+np.tanh(model_params['b'][0]*x))
		dfx.append(model_params['a'][0]*model_params['b'][0]*(1.0-np.tanh(model_params['b'][0]*x)**2))
		d2fx.append(-2.0*model_params['a'][0]*model_params['b'][0]**2* \
		np.tanh(model_params['b'][0]*x)*(1.0-np.tanh(model_params['b'][0]*x)**2))
	elif (nsurf==2):
		fx=model_params['a'][0]*(1.0-np.tanh(model_params['b'][0]*x))
		dfx.append(-model_params['a'][0]*model_params['b'][0]*(1.0-np.tanh(model_params['b'][0]*x)**2))
		d2fx.append(2.0*model_params['a'][0]*model_params['b'][0]**2* \
		...
		fx=model_params['d1'][0]*np.exp(-model_params['d2'][0]*(x-model_params['d3'][0])**2)

		dfx.append(-2.0*model_params['d1'][0]*model_params['d2'][0]*(x-model_params['d3'][0])* \
		np.exp(-model_params['d2'][0]*(x-model_params['d3'][0])**2))

		d2fx.append(-2.0*model_params['d1'][0]*model_params['d2'][0]*np.exp(-model_params['d2'][0]* \
		(x-model_params['d3'][0])**2)-dfx[0]*2.0*model_params['d2'][0]*(x-model_params['d3'][0]))

	return(fx,dfx,d2fx)

#def model_t1_adiab_nD(ndof,q,nsurf,model_params):
#	x=q.get(0)

#	aa=model_params['a'][0];bb=model_params['b'][0];cc=model_params['d1'][0];dd=model_params['d2'][0]
#	fx=0.0;dfx=[];d2fx=[]
#	if (nsurf==1):
#		fx=(aa*exp(2.0*bb*x)+aa-np.sqrt(cc**2*exp(-2.0*dd*x**2+4.0*bb*x)+2.0*cc**2*exp(2.0*x*(bb-dd*x))+ \
#		cc**2*exp(-2.0*dd*x**2)-2.0*(exp(2*bb*x)-exp(4.0*bb*x)/2.0-0.5)*aa**2))/(exp(2.0*bb*x)+1.0)
...
def model_t2_diab_nD(ndof,q,nsurf,model_params):
...
def model_t3_adiab_nD(ndof,q,nsurf,model_params):
