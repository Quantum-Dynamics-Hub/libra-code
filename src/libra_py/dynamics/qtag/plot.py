...
def plot_wf_1D(_plt_params, data):
    ...
    for i in range(len(snaps)):
        ...
        for j in range(len(which_states)):
            ...
            ax.legend(
                loc=(
                    plt_params['legend_loc'][0],
                    plt_params['legend_loc'][1]),
                prop={
                    'size': plt_params['legend_size']})
...
def trajectories(_plt_params):
    ...
    elif isinstance(which_states, list):
    ...
    if isinstance(which_traj, str):
        which_traj = which_traj.lower()
    elif isinstance(which_traj, list):
