...
def run_relaxation(_params, _potential, model_params):
    ...
    if "custom_pops" not in params.keys():
        ...
        if params["custom_pops"] is not None:
    ...
    if hdf5_output_level > 0:
        ...
        if ncustom_pops > 0:
            # boxed populations on adiabatic/diabatic states
            save.exact_init_custom_hdf5(hdf5_saver, nsteps, ncustom_pops, nstates)
    ...
    if mem_output_level > 0:
        ...
        if ncustom_pops > 0:
            # boxed populations on adiabatic/diabatic states
            save.exact_init_custom_hdf5(mem_saver, nsteps, ncustom_pops, nstates)
