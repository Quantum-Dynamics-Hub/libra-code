...
def set_method(params, ham_rep=0, is_nbra=0, method=0):
    ...
    if is_nbra == -1:  # Custom
        ...
        # state-specific forces, compute them in the adiabatic rep
        params.update({"force_method": 1, "rep_force": 1})
        ...
        # Tully-style velocity rescaling and frustr. hops
        params.update({"hop_acceptance_algo": 20, "momenta_rescaling_algo": 201})
        ...
        # Boltzmann-type acceptance, no velocity rescaling
        params.update({"hop_acceptance_algo": 31, "momenta_rescaling_algo": 0})

    elif is_nbra == 2:  # on-the-fly NBRA
        params.update({"force_method": 1, "rep_force": 1})   # state-specific forces
        # but make the nuclear dynamics follow the forces of a specific state (to be specified by user)
        params.update({"enforce_state_following": 0})
        params.update({"nac_update_method": 1})              # update NACs based on derivative couplings and momenta
        params.update({"time_overlap_method": 0})            # state-tracking based on the on-the-fly time-overlaps
        # Boltzmann-type acceptance, no velocity rescaling
        params.update({"hop_acceptance_algo": 31, "momenta_rescaling_algo": 0})
    ...
    if method == -1:   # Custom
        ...
        # FSSH, no decoherence, no deph-informed
        params.update({"tsh_method": 0, "decoherence_algo": -1, "dephasing_informed": 0})

    elif method == 1:  # IDA
        # FSSH, ID decoherence, no deph-informed
        params.update({"tsh_method": 0, "decoherence_algo": 1, "dephasing_informed": 0})

    elif method == 2:  # mSDM
        # FSSH, mSDM decoherence, no deph-informed
        params.update({"tsh_method": 0, "decoherence_algo": 0, "dephasing_informed": 0})

    elif method == 3:  # DISH
        # DISH, no other decoherence, no deph-informed
        params.update({"tsh_method": 3, "decoherence_algo": -1, "dephasing_informed": 0})

    elif method == 21:  # mSDM
        # FSSH, mSDM decoherence, yes deph-informed
        params.update({"tsh_method": 0, "decoherence_algo": 0, "dephasing_informed": 1})

    elif method == 31:  # DISH
        # DISH, no other decoherence, yes deph-informed
        params.update({"tsh_method": 3, "decoherence_algo": -1, "dephasing_informed": 1})

    elif method == 4:  # AFSSH
        params.update({"tsh_method": 0, "decoherence_algo": 2, "dephasing_informed": 0})   # AFSSH

    elif method == 5:  # BC-FSSH
        # BC-FSSH, no decoherence, no deph-informed
        params.update({"tsh_method": 0, "decoherence_algo": 3, "dephasing_informed": 0})
