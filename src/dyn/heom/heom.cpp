...
/**
  ...
  Based on the Fortran 90 code of Amber Jain & Joe Subotnik:
...
int compute_nn_tot(int d, int max_tier) {
  /**
    ...
    \param[in] nquant (often denoted M) the number of system-bath coupling
    terms, in the case where each state is coupled to the bath intependently,
    this number is also equal to the number of quantum state, hence the current
    name
  ...
  */
  ...
  for (int i = 0; i < num_parents; i++) {
    for (int j = 0; j < num_children; j++) {
      ...
      if (!is_included(child, next_level)) {
...
void gen_hierarchy(int d, int max_tier, int verbosity,
  ...
  vec_plus : vec_plus[n][k] - index of the vector for which k-th element is +1
  of that of the n-th vector

  vec_minus : vec_minus[n][k] - index of the vector for which k-th element is -1
  of that of the n-th vector

  The vec_minus and vec_plus contain -1 for the situation when there is no such
  a vector included in the current hierarchy structure
  ...
  */
  ...
  if (verbosity > 0) {
    ...
    cout << "Tier nums: \n";
    for (auto i = 0u; i < tier_nums.size(); i++) {
  ...
  if (verbosity > 1) {
    for (auto i = 0; i < num_nodes; i++) {
      ...
      cout << " vector=[";
      for (j = 0; j < d; j++) {
        cout << all_vectors[i][j] << ",";
      }
      cout << "] ";
      cout << " coordinates=[" << all_coordinates[i][0] << ","
           << all_coordinates[i][1] << "] ";
      cout << " vec_plus=[";
      for (j = 0; j < d; j++) {
        cout << vec_plus[i][j] << ",";
...
vector<int> filter(vector<CMATRIX> &rho, vector<int> &adm_list,
                   double tolerance, int do_zeroing) {
  /**
     Gives the list of indices for the density matrices which are assumed to
     be non-zero, also returns the list of flags for whether a given ADM is
     non-zero
...
CMATRIX compute_deriv_n(int n, vector<CMATRIX> &rho, CMATRIX &Ham,
                        vector<CMATRIX> &el_phon_coupl, double eta,
                        double temperature, vector<double> &gamma_matsubara,
                        vector<complex<double>> &c_matsubara,
                        int truncation_scheme,
                        complex<double> truncation_prefactor, int do_scale,
                        vector<int> &nonzero, vector<vector<int>> &nvectors,
                        vector<vector<int>> &vec_plus,
                        vector<vector<int>> &vec_minus) {
  /**
     ...
     Parameters:
      ...
      projectors : the matrices that describe how each electronic phonon is
     coupled to various electronic states in the simplest picture, when a phonon
     k is coupled to electronic state m, the matrix projectors[k] will
         contain 1.0 at the element (m,m) and zeroes everywhere else. You can of
     course define more general situations when one phonon is coupled to many
     states (and perhaps to their coherences) eta : reorganization energy [a.u.]
      temperature : bath temperature [ K ]
      gamma_matsubara : Matsubara frequencies
      c_matsubara : expansion coefficients of the autocorrelation function in
     the Matsubara terms do_truncate : a flag to control the Ihizaki-Tanimura
     scheme for truncation -  JPSJ 74 3131, 2005 0 - don't truncate 1 - do
     truncate do_scale : a flag to control the scaled HEOM approach from JCP
     130, 084105 (2009) 0 - don't use scaling 1 - do use scaling

      adm_list : the list of the indices of all non-zero auxiliary density
     matrices (ADMs) nvectors : the multi-index vectors vec_plus : mapping of
     the rho_n+ matrices vec_minus : mapping of the rho_n- matrices
  ...
  */
  ...
  if (nonzero[n] == 1) {
    ...
    if (truncation_scheme == 1 || truncation_scheme == 2 ||
      ...
      // Taking the real part in the line below is really important! otherwise,
      // the result is quite erratic
      // pref -= std::real( compute_matsubara_sum(gamma_matsubara, c_matsubara,
      // KK) );
  ...
  for (m = 0; m < nquant; m++) {
    ...
    for (k = 0; k <= KK; k++) {
      ...
      if (nplus != -1) {
        if (nonzero[nplus] == 1) {
          ...
          scaling_factor = 1.0;
          if (do_scale ==
              1) { // Scaled HEOM approach from JCP 130, 084105 (2009)
            scaling_factor = sqrt((nvectors[n][m * (KK + 1) + k] + 1.0) *
                                  std::abs(c_matsubara[k]));
          }
    ...
    drho_n_dt -= iota * (el_phon_coupl[m] * sum_rho_over_k -
                         sum_rho_over_k * el_phon_coupl[m]);
    ...
    for (k = 0; k <= KK; k++) {
      ...
      if (nminus != -1) {
        if (nonzero[nminus] == 1) {
          ...
          sum2_c_rho_over_k +=
              scaling_factor * std::conj(c_matsubara[k]) * rho[nminus];
        } // not filtered out
      } // a valid entry

    } // for k
    drho_n_dt -= iota * (el_phon_coupl[m] * sum1_c_rho_over_k -
                         sum2_c_rho_over_k * el_phon_coupl[m]);
...
CMATRIX compute_heom_derivatives(CMATRIX &RHO, bp::dict prms) {
  ...
  RHO - CMATRIX((nn_tot)*nquant, nquant) - all the density matrices stacked
  together in a super-column prms - Python dict - control parameters
  ...
  */
  ...
  for (i = 0; i < len(prms.values()); i++) {
    ...
    if (key == "KK") {
      KK = extract<int>(prms.values()[i]);
    }
    if (key == "Ham") {
      Ham = extract<CMATRIX>(prms.values()[i]);
    }
    if (key == "eta") {
      eta = extract<double>(prms.values()[i]);
    }
    if (key == "gamma") {
      gamma = extract<double>(prms.values()[i]);
    }
    if (key == "temperature") {
      temperature = extract<double>(prms.values()[i]);
    }
    if (key == "gamma_matsubara") {
      gamma_matsubara = extract<doubleList>(prms.values()[i]);
    }
    if (key == "c_matsubara") {
      c_matsubara = extract<complexList>(prms.values()[i]);
    }
    if (key == "truncation_scheme") {
      truncation_scheme = extract<int>(prms.values()[i]);
    }
    if (key == "do_scale") {
      do_scale = extract<int>(prms.values()[i]);
    }
    ...
    if (key == "nvec") {
      nvec = extract<intList2>(prms.values()[i]);
    }
    if (key == "nvec_plus") {
      nvec_plus = extract<intList2>(prms.values()[i]);
    }
    if (key == "nvec_minus") {
      nvec_minus = extract<intList2>(prms.values()[i]);
    }
    if (key == "nonzero") {
      nonzero = extract<intList>(prms.values()[i]);
    }
    if (key == "adm_list") {
      adm_list = extract<intList>(prms.values()[i]);
    }

    if (key == "num_threads") {
      n_threads = extract<int>(prms.values()[i]);
    }
  }
  ...
  if (truncation_scheme == 1 || truncation_scheme == 2) {
    ...
    truncation_prefactor =
        complex<double>(eta * kB * temperature / gamma_matsubara[0], 0.0);
    ...
    if (truncation_scheme == 1) {
      ...
      // the result is quite erratic
      truncation_prefactor -=
          std::real(compute_matsubara_sum(gamma_matsubara, c_matsubara, KK));
    }
    ...
    if (truncation_scheme == 2) {
      ...
      truncation_prefactor -=
          compute_matsubara_sum(gamma_matsubara, c_matsubara, KK);
    }
  }
  ...
  if (truncation_scheme == 3 || truncation_scheme == 4) {
    ...
    setup_bath(KK_ext, eta, gamma, temperature, gamma_matsubara_ext,
               c_matsubara_ext);

    complex<double> sum_KK;
    sum_KK = compute_matsubara_sum(gamma_matsubara, c_matsubara, KK);
    complex<double> sum_KK_ext;
    sum_KK_ext =
        compute_matsubara_sum(gamma_matsubara_ext, c_matsubara_ext, KK + more);
...
#pragma omp for
    for (int n1 = 0; n1 < nn_nonzero; n1++) {
      drho_unpacked[adm_list[n1]] = compute_deriv_n(
          adm_list[n1], rho_unpacked, Ham, el_phon_couplings, eta, temperature,
          gamma_matsubara, c_matsubara, truncation_scheme, truncation_prefactor,
          do_scale, nonzero, nvec, nvec_plus, nvec_minus);
    }
  }
...
vector<CMATRIX> initialize_el_phonon_couplings(int nquant) {
  /**
    Compute the matrices  Q_m = |m><m| - the simplest type of the coupling of
    electrons to phonons
...
complex<double> compute_matsubara_sum(vector<double> &gamma_matsubara,
                                      vector<complex<double>> &c_matsubara,
                                      int KK) {
  /**
...
void setup_bath(int KK, double eta, double gamma, double temperature,
  ...
  */
  ...
  c_matsubara[0] =
      0.5 * eta * gamma * (1.0 / tan(0.5 * gamma / kT) * one - iota);
...
void unpack_mtx(vector<CMATRIX> &rho_unpacked, CMATRIX &RHO) {
  ...
  */
  ...
  if (static_cast<int>(rho_unpacked.size()) != num_adm) {
    cout << "ERROR in unpack_rho: \n";
    exit(0);
  }
...
void scale_rho(vector<CMATRIX> &rho, vector<CMATRIX> &rho_scaled,
               bp::dict prms) {
...
void inv_scale_rho(vector<CMATRIX> &rho, vector<CMATRIX> &rho_scaled,
                   bp::dict prms) {
