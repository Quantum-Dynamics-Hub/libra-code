...
/**
  \file gwp_dipole.cpp
  \brief The file implements the transition dipole integrals needed for Gaussian Wavepacket propagation
...
/**
  This function computes a transition dipole matrix element of two moving 1D Gaussians  <G_1|r|G_2>, where:
  ...
  The function returns the value of the transition dipole moment - a complex number
*/
  ...
  double q_weighted = (alp2 * q2 + alp1 * q1 )/alp; 
  double dp = p2 - p1;
   
  complex<double> ovlp = gwp_overlap(q1, p1, gamma1, alp1, q2, p2, gamma2, alp2);
  return complex<double>(q_weighted, 0.5*dp/alp ) * ovlp;
...
/**
  This function computes a transition dipole moment matrix elements of two moving N-dimensional Gaussians  <G_1|d/dr|G_2>, where:
...
*/
  ...
  int Ndof = check_dimensions("libgwp::gwp_dipole", q1, p1, q2, p2);
  ...
  for(int  i=0; i<Ndof; i++){
    ...
    double q_weighted = (alp2.get(i,0) * q2.get(i,0) + alp1.get(i,0) * q1.get(i,0) )/alp; 
    double dp = p2.get(i,0) - p1.get(i,0);
   
    res.set(i, 0, q_weighted, 0.5*dp/alp );
