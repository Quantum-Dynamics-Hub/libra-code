...
int compute_imapping(vector<int> &inp, vector<int> &npts) {
  ...
  */
  ...
  if (inp.size() != npts.size()) {
    cout << "ERROR in compute_imapping: inp.size()= " << inp.size()
         << " but npts.size()= " << npts.size() << endl;
    cout << "Exiting now...\n";
    exit(0);
  }
...
vector<int> compute_hyperplane(vector<int> &npts, int idim_const,
                               int ipt_const) {
  /**
...
vector<vector<int>> compute_mapping(vector<vector<int>> &inp,
                                    vector<int> &npts) {
  /**
  ...
  */
  ...
  if (sz == 0) { // starting with an empty container
    ...
    if (static_cast<std::size_t>(lvl) < npts.size()) {
      ...
      for (i = 0; i < sz; i++) { // take each initial input vectors
        ...
        for (ipt = 0; ipt < npts[lvl];
             ipt++) { // all points along a given dimension
          res_i[lvl] = ipt;
          res.push_back(res_i);
        } // for ipt
...
CMATRIX init_grid(double xmin, double xmax, double dx) {
  /**
    ...
    will be generated as a Nx x 1 complex-valued matrix (elements are actually
    real-valued)
  ...
  */
  ...
  CMATRIX x(sz, 1);
  for (int i = 0; i < sz; i++) {
    x.M[i] = complex<double>(xmin + i * dx, 0.0);
  }
  return x;
}
...
void init_gauss_1D(vector<CMATRIX> &wfc, CMATRIX &X, double x_, double px_,
                   double dx, int nstates, int occ_state, complex<double> scl) {
  /**
    \brief Initialize a Gaussian wavepacket on a 1D grid
    \param[out] wfc Is a list of complex matrices (vectors), each containing
    numerical wavefunction for different electronic state
    ...
    \param[in] dx Spread (distribution width) of the spatial component of the
    Gaussian wavepacket
    \param[in] nstates The number of electronic states for which to initialize
    the wavefunction
    \param[in] occ_state Index of the occupied electronic state on which the
    wavepacket is initialized
    ...
    That is, this wavepacket would correspond to the probability density
    (classical coordinates) that are generated like this:

    x_i = x_ * dx * rnd.normal()


    Other connections:
    2*a = 1/2*dx^2 =>  a = 1/(2*dx)^2 , where a is such that:  alpha/2 = a +
    i*b, where a and b are defined in:

    (1) Heller, E. J. Guided Gaussian Wave Packets. Acc. Chem. Res. 2006, 39,
    127–134. (2) Akimov, A. V.; Prezhdo, O. V. Formulation of Quantized
    Hamiltonian Dynamics in Terms of Natural Variables. J. Chem. Phys. 2012,
    137, 224115.
...
void init_gauss_1D(vector<CMATRIX> &wfc, CMATRIX &X, double x_, double px_,
                   double dx, int nstates, int occ_state) {

  complex<double> one(1.0, 0.0);
  init_gauss_1D(wfc, X, x_, px_, dx, nstates, occ_state, one);
}

void add_gauss_1D(vector<CMATRIX> &wfc, CMATRIX &X, double x_, double px_,
                  double dx, int nstates, int occ_state,
  ...
  for (int nx = 0; nx < Nx; nx++) {
    ...
    for (int st = 0; st < nstates; st++) {
      if (st == occ_state) {
        ...
        wfc[st].M[nx * 1 + 0] +=
            weight * nrm * exp(c1) * (cos(c2) + one * sin(c2));
      } else {
        wfc[st].M[nx * 1 + 0] += 0.0;
      }
...
void add_ho_1D(vector<CMATRIX> &wfc, CMATRIX &X, int nu, double x_, double px_,
               complex<double> weight, int occ_state, int alpha) {
  /**
    \brief Adds a moving! Harmonic Oscillator (HO) with a given weight to a 1D
    grid wavefunction
    ...
    grid
    \param[in] occ_state Index of the electronic state to which the wavepacket
    is added
    ...
    H_nu(ksi):  H_{nu+1}(ksi) - 2 ksi*H_{nu}(ksi) + 2*nu *H_{nu-1}(ksi) = 0  -
    Hermite polynomial
  ...
  */
  ...
  // Constants
  const double nrm =
      (1.0 / sqrt(pow(2.0, nu) * FACTORIAL(nu))) * pow((alpha / M_PI), 0.25);
  const complex<double> one(0.0, 1.0);
  ...
  for (int nx = 0; nx < Nx; nx++) {
    ...
    wfc[occ_state].M[nx] +=
        weight * nrm * H * exp(-0.5 * ksi * ksi) * (cos(c2) + one * sin(c2));
...
void print_1D(CMATRIX &X, vector<CMATRIX> &PSI, string prefix, int frame) {
  /**
    \brief Printing populations of all states in different files (one state per
    file)
    \param[in] X The grid points (Nx x 1 matrix)
    \param[in] PSI The wavefunctions for all states to be printed out. This is a
    vector of Nx x 1 matrices
    \param[in] prefix The common part of the names of the files to which the wfc
    will be printed out
    \param[in] frame The integer index to be added into the file name - for
    instance, when printing snapshots of dynamical simulations

    All files have a common prefix, which also includes indexing via frame
    parameter, but all files have different suffixes - depending on the
    electronic state
  */
...
void print_1D(CMATRIX &X, vector<CMATRIX> &PSI, string filename) {
  /**
    ...
    \param[in] PSI The wavefunctions for all states to be printed out. This is a
    vector of Nx x 1 matrices
    \param[in] filename The common part of the names of the files to which the
    wfc will be printed out
  ...
  */
  ...
  for (int st = 0; st < nstates; st++) {
    ...
    for (int nx = 0; nx < Nx; nx++) {
      out << real(X.M[nx]) << "  "
          << real(std::conj(PSI[st].M[nx]) * PSI[st].M[nx]) << endl;
    } // for nx
...
void ft_1D(vector<CMATRIX> &psi, vector<CMATRIX> &reci_psi, int opt,
  ...
  /**
    \brief Do 1D cfft (complex fast Fourier transform) for each state
    independently
    \param[in,out] psi  Reals space wavefunctions: nstates x Nx x 1 - that is
    nstates complex-valued matrices Nx x 1 each
    \param[in,out] reci_psi  Reciprocal space wavefunctions: nstates x Nx x 1 -
    that is nstates complex-valued matrices Nx x 1 each
    \param[in] opt option to control the direction of transformation:  opt = 1:
    psi ---> reci_psi;  opt = 2:  psi <--- reci_psi
  ...
  */
  ...
  for (int i = 0; i < nstates; i++) {
    ...
    in = psi[i];
    if (opt == 1) {
      cfft1(in, out, xmin, kxmin, dx);
    } else if (opt == 2) {
      inv_cfft1(in, out, xmin, kxmin, dx);
    }
...
void init_gauss_2D(vector<CMATRIX> &wfc, CMATRIX &X, double x_, double px_,
                   double dx, CMATRIX &Y, double y_, double py_, double dy,
                   int nstates, int occ_state) {
  /**
    \brief Initialize a Gaussian wavepacket on a 2D grid
    \param[out] wfc Is a list of complex matrices (vectors), each containing
    numerical wavefunction for different electronic state
    \param[in] X is the complex matrix(vector) containing the grid points along
    X direction
    \param[in] x_ Position of the center of the Gaussian wavepacket in X
    direction
    \param[in] px_ Momentum of the Gaussian wavepacket in X direction
    \param[in] dx Spread (distribution width) of the spatial component of the
    Gaussian wavepacket along the axis X
    \param[in] Y is the complex matrix(vector) containing the grid points along
    Y direction
    \param[in] y_ Position of the center of the Gaussian wavepacket in Y
    direction
    \param[in] py_ Momentum of the Gaussian wavepacket in Y direction
    \param[in] dy Spread (distribution width) of the spatial component of the
    Gaussian wavepacket along the axis Y
    \param[in] nstates The number of electronic states for which to initialize
    the wavefunction
    \param[in] occ_state Index of the occupied electronic state on which the
    wavepacket is initialized
...
void print_2D(CMATRIX &X, CMATRIX &Y, vector<CMATRIX> &PSI, string prefix,
              int frame) {
  /**
    \brief Printing populations of all states in different files (one state per
    file)
    ...
    for (int nx = 0; nx < Nx; nx++) {
      for (int ny = 0; ny < Ny; ny++) {
        out << real(X.M[nx]) << "  " << real(Y.M[ny]) << "  "
            << real(std::conj(PSI[st].M[nx * Ny + ny]) *
                    PSI[st].M[nx * Ny + ny])
            << endl;
      } // for ny
      out << "\n";
...
void ft_2D(vector<CMATRIX> &psi, vector<CMATRIX> &reci_psi, int opt,
           double xmin, double ymin, double kxmin, double kymin, double dx,
           double dy) {
  /**
    \brief Do 2D cfft (complex fast Fourier transform) for each state
    independently
    \param[in,out] psi  Reals space wavefunctions: nstates x Nx x Ny - that is
    nstates complex-valued matrices Nx x Ny each
    \param[in,out] reci_psi  Reciprocal space wavefunctions: nstates x Nx x Ny -
    that is nstates complex-valued matrices Nx x Ny each
    \param[in] opt option to control the direction of transformation:  opt = 1:
    psi ---> reci_psi;  opt = 2:  psi <--- reci_psi
    \param[in] xmin The lower boundary of the real-space grid
    \param[in] kxmin The lower boundary of the reciprocal-space grid
  ...
  */
  ...
  for (int i = 0; i < nstates; i++) {
    ...
    in = psi[i];
    if (opt == 1) {
      cfft1_2D(in, out, xmin, ymin, kxmin, kymin, dx, dy);
    } else if (opt == 2) {
      inv_cfft1_2D(in, out, xmin, ymin, kxmin, kymin, dx, dy);
    }
