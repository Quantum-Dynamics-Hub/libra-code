...
/**
  \file Wfcgrid.cpp
  \brief The file implements some basic methods of the Wfcgrid class:
  initialization, printing, memory allocation, etc.
...
void Wfcgrid::init_numbers(double minx_, double maxx_, double dx_,
                           int nstates_) {
  /**
    ...
    Computes the number of points in X dimension so that the boundaries are
    satisfied and the number of point is the lowest power of 2 needed to enclose
    the interval. Also sets other class variables.
  */
...
void Wfcgrid::init_numbers(double minx_, double maxx_, double dx_, double miny_,
                           double maxy_, double dy_, int nstates_) {
  /**
    ...
    Computes the number of points in X and Y dimension so that the boundaries
    are satisfied and the number of point is the lowest power of 2 needed to
    enclose the interval. Also sets other class variables.
  */
...
void Wfcgrid::allocate_1D() {
  /**
    ...
    all numbers are assumed to be defined by this time, so the init_numbers()
  ...
  */
  ...
  // Allocate arrays
  CMATRIX psi(Nx, Ny);
  psi = 0.0; // is a matrix placeholder

  PSI = vector<CMATRIX>(nstates, psi); // wavefunction  nstates x Nx x Ny
  reciPSI = vector<CMATRIX>(
      nstates,
      psi); // same as PSI but in Fourier (reciprocal) space with 1.0*kmin
  DtreciPSI = vector<CMATRIX>(
      nstates,
      psi); // d/dt * reciPSI - time derivative of PSI in reciprocal space
  DxPSI =
      vector<CMATRIX>(nstates, psi); // d/dx * PSI in real space (up to factor)
  KxreciPSI = vector<CMATRIX>(
      nstates, psi); // Kx * reciPSI  -  d/dx * PSI in k space (up to factor)

  // Potential-related variables
  H = vector<vector<CMATRIX>>(nstates,
                              vector<CMATRIX>(nstates, psi)); // Hamiltonian
  Dx = vector<vector<CMATRIX>>(
      nstates,
      vector<CMATRIX>(nstates, psi)); // for each i,j pair - a separate x
                                      // projection for all r-points of the grid
...
void Wfcgrid::allocate_2D() {
  ...
  */
  ...
  KxreciPSI = vector<CMATRIX>(
      nstates, psi); // Kx * reciPSI  -  d/dx * PSI in k space (up to factor)
  KyreciPSI = vector<CMATRIX>(
      nstates, psi); // Ky * reciPSI  -  d/dy * PSI in k space (up to factor)
  ...
  Dx = vector<vector<CMATRIX>>(
      ...
      vector<CMATRIX>(nstates, psi)); // for each i,j pair - a separate x
                                      // projection for all r-points of the grid
  Dy = vector<vector<CMATRIX>>(
      nstates,
      vector<CMATRIX>(nstates, psi)); // for each i,j pair - a separate y
                                      // projection for all r-points of the grid
  expH = vector<vector<CMATRIX>>(nstates, vector<CMATRIX>(nstates, psi));
  expK = vector<CMATRIX>(nstates, psi);
...
void Wfcgrid::init_grid_1D() {
  /**
    ...
    memory is assumed to be allocated by this time. If not, call allocate_1D()
    function first
  */
...
void Wfcgrid::init_grid_2D() {
  /**
    ...
    memory is assumed to be allocated by this time. If not, call allocate_2D()
    function first
  */
...
Wfcgrid::Wfcgrid(double minx_, double maxx_, double dx_, int nstates_) {
  /**
    ...
    This constructor will: 1) initialize numbers, 2) allocate memory; 3)
    initialize grids
  */
...
// 2D Constructor
Wfcgrid::Wfcgrid(double minx_, double maxx_, double dx_, double miny_,
                 double maxy_, double dy_, int nstates_) {
  /**
...
void Wfcgrid::init_wfc_1D(double x0, double px0, double dx0, int init_state) {
  /**
    ...
    \param[in] dx0 Spread (distribution width) of the spatial component of the
    Gaussian wavepacket
    \param[in] init_state Index of the electronic state on which the wavepacket
    is initialized

     G(x) = [ (1/(2.0*pi*dx0^2))^(1/4) ] * exp(-((x-x0)/(2*dx0))^2 +
    i*(x-x0)*px0)
...
void Wfcgrid::init_wfc_1D(vector<double> &x0, vector<double> &px0,
                          vector<double> &dx0, vector<int> &init_state,
  ...
  */
  ...
  int sz = x0.size();
  //  init_gauss_1D(PSI, *X, x0[0], px0[0], dx0[0], nstates, init_state[0],
  //  weights[0]);

  // PSI should be already allocated by this point
  for (int i = 0; i < sz; i++) {
    add_gauss_1D(PSI, *X, x0[i], px0[i], dx0[i], nstates, init_state[i],
                 weights[i]);
  }
...
void Wfcgrid::init_wfc_1D_HO(vector<int> &init_state, vector<int> &nu,
                             vector<complex<double>> &weights,
                             vector<double> &x0, vector<double> &px0,
                             vector<double> &alpha) {
  /**
    \brief Initialize 1D wavefunction as a superposition of moving Harmonic
    oscillator eigenstates

    \param[in] init_state Index of the electronic state to which the wavepacket
    is added
    \param[in] nu Vibrational quantum number of the HO basis function
    \param[in] weight The weight with which the basis function is added to the
    grid
    \param[in] x0 Position of the center of the HO basis function
    \param[in] px0 Momentum of the HO basis wavepacket
    \param[in] alpha The parameter related to the reference HO Hamiltonian:
    alpha = sqrt(k*mu/hbar^2) Where:  H = -hbar^2 /(2*mu) d^2/dx^2  + 1/2 * k *
    (x-x_)^2

  */

  int sz = x0.size();
  cout << "Initialize the wfc as a superposition of " << sz
  ...
  for (int i = 0; i < sz; i++) {
    ...
    add_ho_1D(PSI, *X, nu[i], x0[i], px0[i], weights[i], init_state[i],
              alpha[i]);
  }
...
void Wfcgrid::init_wfc_1D_ARB(bp::object py_funct, bp::object params) {
  /**
    ...
    \param[in] py_funct - the name of the Python-defined function. Expectations
    is that the function returns a complex-values result for different states
...
void Wfcgrid::init_wfc_2D(double x0, double y0, double px0, double py0,
                          double dx0, double dy0, int init_state) {
  /**
    \brief Initialize 2D wavefunction - taken as moving Gaussian wavepacket
    \param[in] x0 Position of the center of the Gaussian wavepacket in X
    dimension
    \param[in] y0 Position of the center of the Gaussian wavepacket in Y
    dimension
    ...
    \param[in] dx0 Spread (distribution width) of the spatial component of the
    Gaussian wavepacket in X dimension
    \param[in] dy0 Spread (distribution width) of the spatial component of the
    Gaussian wavepacket in Y dimension
    \param[in] init_state Index of the electronic state on which the wavepacket
    is initialized
    ...
    where
    G(x) = [ (1/(2.0*pi*dx0^2))^(1/4) ] * exp(-((x-x0)/(2*dx0))^2 +
    i*(x-x0)*px0) G(y) = [ (1/(2.0*pi*dy0^2))^(1/4) ] * exp(-((y-y0)/(2*dy0))^2
    + i*(y-y0)*py0)
...
void Wfcgrid::print_wfc_1D(std::string prefix, int snap, int state) {
  /**
    ...
    \param[in] prefix The prefix of the filenames to which the wfc will be
    printed out
    \param[in] snap This is the integer indexing the printed out wavefunction -
    e.g. convenient for printing wfc at different times
    \param[in] state The wavefunction projection on this state will be printed
    out
  ...
  */
  ...
  for (int nx = 0; nx < Nx; nx++) {
    ...
    out << real(X->M[nx]) << "  "
        << real(std::conj(PSI[state].M[nx]) * PSI[state].M[nx]) << endl;
  ...
  */
  ...
  for (int nx = 0; nx < Nx; nx++) {
    for (int ny = 0; ny < Ny; ny++) {
      ...
      out << real(X->M[nx]) << "  " << real(Y->M[ny]) << "  "
          << real(std::conj(PSI[state].M[nx * Ny + ny]) *
                  PSI[state].M[nx * Ny + ny])
          << endl;
...
void Wfcgrid::print_reci_wfc_1D(std::string prefix, int snap, int state) {
  /**
    \biref Print 1D wavefunction (in k-representation, reciprocal space) into a
    file
    \param[in] prefix The prefix of the filenames to which the wfc will be
    printed out
    \param[in] snap This is the integer indexing the printed out wavefunction -
  ...
  */
  ...
  for (int nx = 0; nx < Nx; nx++) {
    ...
    out << real(2.0 * M_PI * Kx->M[nx]) << "  "
        << real(std::conj(reciPSI[state].M[nx]) * reciPSI[state].M[nx]) << endl;
...
void Wfcgrid::print_reci_wfc_2D(std::string prefix, int snap, int state) {
  /**
    \brief Prints 2D wavefunction (in k-representation, reciprocal space) into a
    file
    \param[in] prefix The prefix of the filenames to which the wfc will be
    printed out
    \param[in] snap This is the integer indexing the printed out wavefunction -
  ...
  */
  ...
  for (int nx = 0; nx < Nx; nx++) {
    for (int ny = 0; ny < Ny; ny++) {
      ...
      out << real(2.0 * M_PI * Kx->M[nx]) << "  "
          << real(2.0 * M_PI * Ky->M[ny]) << "  "
          << real(std::conj(reciPSI[state].M[nx * Ny + ny]) *
                  reciPSI[state].M[nx * Ny + ny])
          << endl;
...
void Wfcgrid::print_complex_matrix_1D(CMATRIX &CM, std::string filename) {
  /**
    ...
    \param[in] filename The name of the file to which the matrix will be printed
    out
  */
  ...
  for (int nx = 0; nx < Nx; nx++) {
    out << real(X->M[nx]) << "  " << real(2.0 * M_PI * Kx->M[nx]) << "  "
        << CM.M[nx].real() << "   " << CM.M[nx].imag() << endl;
  } // for nx
...
void Wfcgrid::print_ham_1D(std::string prefix, int i, int j) {
  /**
    ...
    \param[in] prefix The prefix of the file to which the info will be printed
    out
...
void Wfcgrid::print_expH_1D(std::string prefix, int i, int j) {
  /**
    \brief Printing of the exponential of the Hamiltonian of 1D wavefunctions
    (component of propagator)
    \param[in] prefix The prefix of the file to which the info will be printed
    out
    \param[in] i The index of one of the states
    \param[in] j The index of the other state
...
void Wfcgrid::print_expK_1D(std::string prefix, int i) {
  /**
    \brief Printing of the exponential of the kinetic energy operator (in
    reciprocal space) for 1D wavefunctions
    \param[in] prefix The prefix of the file to which the info will be printed
    out
    \param[in] i The index of the state - the operator is diagonal, so only need
    this one index
...
double Wfcgrid::print_populations_1D(string filename, int snap) {
  /**
    ...
    \param[in] filename The name of the file to which the wavefunction will be
    printed out
    \param[in] snap This is the integer indexing the printed out wavefunction -
    e.g. convenient for printing wfc at different times
  ...
  */
  ...
  out << "nsnap= " << snap;
  // Probabilities of the wavepackets that still remain in active calculation
  // area
  for (int nst = 0; nst < nstates; nst++) {
  ...
  for (int nx = 0; nx < Nx; nx++) {
    for (int ny = 0; ny < Ny; ny++) {
      for (int nst = 0; nst < nstates; nst++) {
        ...
        double pii = real(std::conj(PSI[nst].M[nx * Ny + ny]) *
                          PSI[nst].M[nx * Ny + ny]);
...
void Wfcgrid::flux_1D(double xf, vector<double> &res, double m0) {
  /**
    ...
    \param[out] res The collector of the fluxes (for each electronic state
    projection of the wfc)
    \param[in] m0 The effective mass of the quantum particle (DOF)
  */

  if (res.size() < static_cast<std::size_t>(nstates)) {
    res = vector<double>(nstates, 0.0);
  }
  ...
  for (int nst = 0; nst < nstates; nst++) {
    /*
        // Original expression
        res[nst]
       = 2.0*M_PI*(0.5*hbar/m0)*imag(std::conj(PSI[nst].M[i])*one*DxPSI[nst].M[i]
       - std::conj(one*DxPSI[nst].M[i])*PSI[nst].M[i]
                                              );
        // Simplified 1
        res[nst]
       = 4.0*M_PI*(0.5*hbar/m0)*imag(std::conj(PSI[nst].M[i])*one*DxPSI[nst].M[i]
       );
    */
    // Simplified 2
    res[nst] =
        (2.0 * M_PI / m0) * real(std::conj(PSI[nst].M[i]) * DxPSI[nst].M[i]);
  }
...
double Wfcgrid::get_x_1D() {
  /**
    Compute expectation value of coordinate of a 1D wavefunction: <psi|x|psi> /
    <psi|psi>
  */
...
double Wfcgrid::get_pow_x_1D(double n) {
  /**
    Compute expectation value of a power of coordinate x^n a 1D wavefunction:
    <psi|x^n|psi> / <psi|psi>
  */
...
double Wfcgrid::get_px_1D() {
  /**
    Compute the expectation value of momentum of a 1D wavefunction:
    <psi|-i*hbar*d/dx |psi> / <psi|psi>
  */
  ...
  for (int nst = 0; nst < nstates; nst++) {
    for (int nst1 = 0; nst1 < nstates; nst1++) {
      ...
      for (int nx = 0; nx < Nx; nx++) {
        ...
        res += real(Kx->M[nx]) *
               real(std::conj(reciPSI[nst].M[nx]) * reciPSI[nst1].M[nx]);
...
double Wfcgrid::get_pow_px_1D(int n) {
  /**
    Compute the expectation value of power of momentum of a 1D wavefunction:
    <psi|(-i*hbar*d/dx)^n|psi> / <psi|psi>
  */
...
double Wfcgrid::get_x_2D() {
  /**
    Compute expectation value of x coordinate of a 2D wavefunction: <psi|x|psi>
    / <psi|psi>
  */
    ...
    for (int nst1 = 0; nst1 < nstates; nst1++) {
      ...
      for (int nx = 0; nx < Nx; nx++) {
        for (int ny = 0; ny < Ny; ny++) {
          ...
          double d = real(std::conj(PSI[nst].M[nx * Ny + ny]) *
                          PSI[nst1].M[nx * Ny + ny]);
...
double Wfcgrid::get_y_2D() {
  /**
    Compute expectation value of y coordinate of a 2D wavefunction: <psi|y|psi>
    / <psi|psi>
  */
...
double Wfcgrid::get_px_2D() {
  /**
    Compute the expectation value of x component of momentum of a 2D
    wavefunction: <psi|-i*hbar*d/dx |psi> / <psi|psi>
  */
    ...
    for (int nst1 = 0; nst1 < nstates; nst1++) {
      ...
      for (int nx = 0; nx < Nx; nx++) {
        for (int ny = 0; nx < Ny; ny++) {
          ...
          norm += real(std::conj(PSI[nst].M[nx * Ny + ny]) *
                       PSI[nst1].M[nx * Ny + ny]);
          res +=
              real(Kx->M[nx]) * real(std::conj(reciPSI[nst].M[nx * Ny + ny]) *
                                     reciPSI[nst1].M[nx * Ny + ny]);
...
double Wfcgrid::get_py_2D() {
  /**
    Compute the expectation value of y component of momentum of a 2D
    wavefunction: <psi|-i*hbar*d/dy |psi> / <psi|psi>
  */
      ...
      for (int nx = 0; nx < Nx; nx++) {
        for (int ny = 0; nx < Ny; ny++) {
          ...
          norm += real(std::conj(PSI[nst].M[nx * Ny + ny]) *
                       PSI[nst1].M[nx * Ny + ny]);
          res +=
              real(Ky->M[ny]) * real(std::conj(reciPSI[nst].M[nx * Ny + ny]) *
                                     reciPSI[nst1].M[nx * Ny + ny]);
  ...
  for (int nst = 0; nst < nstates; nst++) {
    for (int nst1 = 0; nst1 < nstates; nst1++) {
      ...
      for (int nx = 0; nx < Nx; nx++) {
        ...
        norm += real(std::conj(PSI[nst].M[nx]) * PSI[nst1].M[nx]);
        res += real(std::conj(PSI[nst].M[nx]) * H[nst][nst1].M[nx] *
                    PSI[nst1].M[nx]);
    ...
    for (int nst1 = 0; nst1 < nstates; nst1++) {
      ...
      for (int nx = 0; nx < Nx; nx++) {
        for (int ny = 0; ny < Ny; ny++) {
          ...
          tmp = real(std::conj(reciPSI[nst].M[nx * Ny + ny]) *
                     reciPSI[nst1].M[nx * Ny + ny]);
    ...
    for (int nst1 = 0; nst1 < nstates; nst1++) {
      ...
      for (int nx = 0; nx < Nx; nx++) {
        for (int ny = 0; ny < Ny; ny++) {
          ...
          norm += real(std::conj(PSI[nst].M[nx * Ny + ny]) *
                       PSI[nst1].M[nx * Ny + ny]);
          res += real(std::conj(PSI[nst].M[nx * Ny + ny]) *
                      H[nst][nst1].M[nx * Ny + ny] * PSI[nst1].M[nx * Ny + ny]);
