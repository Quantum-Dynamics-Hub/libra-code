...
CMATRIX sdm(CMATRIX &Coeff, double dt, int act_st, MATRIX &decoh_rates,
  ...
  \param[in]       Coeff [ CMATRIX(nadi, 1) ] An object containig electronic
  DOFs.
  \param[in]          dt [ float ] The integration timestep. Units = a.u. of
  time
  \param[in]      act_st [ integer ] The active state index
  \param[in]      decoh_rates [ MATRIX ] The matrix of decoherence (pure
  dephasing) rates between all pairs of states
  \param[in]         tol [double] The maximal acceptable deviation of the
  p_aa_old from 1. If the p_aa_old < 1.0 + tol, then renormalize it to 1.0 C [
  CMATRIX ] - the updated state of the electronic DOF, in the same data type as
  the input
  ...
  */
  ...
  if (p_aa_old > 1.0 + tol) {
    ...
    // for some algorithms
    cout << "=== Place 1 =====\n";
    cout << "WARNING in CMATRIX sdm(CMATRIX& Coeff, double dt, int act_st, "
            "MATRIX& decoh_rates):\n";
    cout << "The population of the active state is larger than 1: p_aa_old = "
         << p_aa_old << endl;
    cout << "C = \n";
    C.show_matrix();
  ...
  if (p_aa_old > 0.0) {
    ...
    double inact_st_pop =
        0.0; // population of the inactive states after rescaling
    ...
    if (inact_st_pop > 1.0) {
      cout << "=== Place 2 =====\n";
      cout << "Error in CMATRIX sdm(CMATRIX& Coeff, double dt, int act_st, "
              "MATRIX& decoh_rates):\n";
      cout << "Total population of inactive states after rescaling is larger "
              "than 1: inact_st_pop = "
           << inact_st_pop << endl;
      cout << "C = \n";
      C.show_matrix();
    ...
    if (p_aa_new < 0.0) {
      ...
      cout << "Error in CMATRIX sdm(CMATRIX& Coeff, double dt, int act_st, "
              "MATRIX& decoh_rates):\n";
      cout << "new population of the active state " << p_aa_new
           << " is negative...\n";
  ...
  if (fabs(new_norm - 1.0) > 0.1) {
    ...
    cout << "Error in CMATRIX sdm(CMATRIX& Coeff, double dt, int act_st, "
            "MATRIX& decoh_rates):\n";
    //  cout<<"new population of the active state "<< p_aa_new <<" is
    //  negative...\n"; cout<<"inact_st_pop = "<<inact_st_pop<<endl;
    cout << "p_aa_old = " << p_aa_old << endl;
    cout << "C = \n";
    C.show_matrix();
...
CMATRIX sdm(CMATRIX &Coeff, double dt, vector<int> &act_st,
            vector<MATRIX> &decoh_rates, double tol, int isNBRA) {
  /**
  ...
  \param[in]       Coeff [ CMATRIX(nadi, ntraj) ] An object containig electronic
  DOFs.
  \param[in]          dt [ float ] The integration timestep. Units = a.u. of
  time
  \param[in]      act_st [ integer ] The active state index
  ...
  dephasing) rates between all pairs of states
  \param[in]         tol [double] The maximal acceptable deviation of the
  p_aa_old from 1. If the p_aa_old < 1.0 + tol, then renormalize it to 1.0
  \param[in]      isNBRA [integer] If this flag is set to 1, then the
  Hamiltonian related properties are only computed for one of the trajectories.

  The function returns:
  # C [ CMATRIX ] - the updated state of the electronic DOF, in the same data
  type as the input
...
CMATRIX sdm(CMATRIX &Coeff, double dt, vector<int> &act_st,
            vector<MATRIX> &decoh_rates, double tol) {
  int is_nbra = 0;
...
void project_out(CMATRIX &Coeff, int traj, int i) {
  ...
  Args:
      ...
      traj ( int ): The index of the trajectory (column) on which we will
  operate i ( int ): The index of the state to be projected out
...
void collapse(CMATRIX &Coeff, int traj, int i, int collapse_option) {
  ...
  Args:
      ...
      traj ( int ): The index of the trajectory (column) on which we will
  operate i ( int ): The index of the state onto which the supeposition will be
  collapsed collapse_option ( int ): the way to collapse onto given state:

        - 0: by rescaling the magnitude of the amplitude vector elements, but
  preserving "phase"
...
void collapse_dm(CMATRIX *dm, int i) {
  ...
  Args:
      ...
      i ( int ): The index of the state onto which the density matrix will be
  collapsed
...
void instantaneous_decoherence(CMATRIX &Coeff, vector<int> &accepted_states,
  ...
  /**
    This function implements the instantaneous decoherence approach of Nelson et
    al.:

    Nelson, T.; Fernandez-Alberti, S.; Roitberg, A. E.; Tretiak, S. J. Chem.
    Phys. 2013, 138, 224111

    Two options of the algorithm are available:

    ID-S: wavefunction amplitudes are collapsed only during the successful hops
    (onto the new state)
    ...
    There collapsing options are controlled by the parameter
    instantaneous_decoherence_variant

     0 - ID-S
     1 - ID-A
     2 - ID-C - consistent ID - an experimental algo

     In the "consistent" version, we lift the condition that the
    accepted/proposed states must be different from the starting state - this
    option addresses a philosophical question - what if we say that no hops is
    equivalent to the hop onto the current state? why should the "hopping" into
    the original state by treated differently from the "actual hopping" to
    another state? So, in this version we collapse onto the accpeted states, no
    matter if they are the result of a successfull or frustrated hop.

    The mechanism of the collapse event itself is controlled by the
    collapse_option parameter
  */
  int traj;
  int ntraj = Coeff.n_cols;

  if (accepted_states.size() != static_cast<std::size_t>(ntraj)) {
    cout << "ERROR in ids: the sizes of the input variables Coeff and "
            "accepted_states are inconsistent\n";
    cout << "Coeff.num_of_cols = = " << ntraj << "\n";
    cout << "accepted_states.size() = " << accepted_states.size() << "\n";
    cout << "exiting...\n";
    exit(0);
  }
  if (proposed_states.size() != static_cast<std::size_t>(ntraj)) {
    cout << "ERROR in ids: the sizes of the input variables Coeff and "
            "proposed_states are inconsistent\n";
    cout << "Coeff.num_of_cols = = " << ntraj << "\n";
    cout << "proposed_states.size() = " << proposed_states.size() << "\n";
    cout << "exiting...\n";
    exit(0);
  }
  if (initial_states.size() != static_cast<std::size_t>(ntraj)) {
    cout << "ERROR in ids: the sizes of the input variables Coeff and "
            "initial_states are inconsistent\n";
    cout << "Coeff.num_of_cols = = " << ntraj << "\n";
  ...
  // ID-A
    ...
    for (traj = 0; traj < ntraj; traj++) {
      if (proposed_states[traj] != initial_states[traj]) {
        // Only apply ID-A, if the proposed states are different from the
        // original ones
  ...
  // ID-F, ID on the failed transition, experimental
    ...
    for (traj = 0; traj < ntraj; traj++) {
      if (proposed_states[traj] != initial_states[traj]) {
        // Only apply ID-F, if the proposed states are different from the
        // original ones

        if (accepted_states[traj] != proposed_states[traj]) {
          // Proposed hop is rejected - project out the proposed states, but
          // don't collapse onto the accepted hop
          project_out(Coeff, traj, proposed_states[traj]);
        }
      }
    } // traj
...
void instantaneous_decoherence_dia(CMATRIX &Coeff, nHamiltonian &ham,
  ...
  /**
    This is an experimental ID function which does all the same as the normal
    one does, but in the diabatic basis:
    ...
    Two options of the algorithm are available:
    ID-S: wavefunction amplitudes are collapsed only during the successful hops
    (onto the new state) ID-A: wavefunction amplitudes are collapsed at every
    attempted hop to the new state, if successful to the old state, if now
...
CMATRIX afssh_dzdt(CMATRIX &dz, CMATRIX &Hvib, CMATRIX &F, CMATRIX &C,
                   double mass, int act_state) {
  /**
    ...
    dz = (           )
      ...
      Args:
          dz is a block-diagonal matrix of dR and dP, which are:
            dR ( CMATRIX(nstates, nstates) ): moment matrix for position, for
    just 1 DOF and 1 trajectory dP ( CMATRIX(nstates, nstates) ): moment matrix
    for momentum, for just 1 DOF and 1 trajectory Hvib ( CMATRIX(nstates,
    nstates) ): vibronic Hamiltonian matrix, for 1 trajectory F (
    CMATRIX(nstates, nstates) ): diagonal matrix of forces for all states, for a
    given DOF and for 1 trajectory C ( CMATRIX(nstates, 1) ): amplitudes of
    electronic states, for 1 trajectory mass ( double ): mass of the given DOF
          act_state ( int ): index of the currently active state for given
    trajectory
...
void integrate_afssh_moments(CMATRIX &dR, CMATRIX &dP, CMATRIX &Hvib,
                             CMATRIX &F, CMATRIX &C, double mass, int act_state,
                             double dt, int nsteps) {
  /**

     RK4 integration for 1 DOF and 1 trajectory but the matrices are for all
     states

      Args:
          dR ( CMATRIX(nstates, nstates) ): moment matrix for position, for just
     1 DOF and 1 trajectory dP ( CMATRIX(nstates, nstates) ): moment matrix for
     momentum, for just 1 DOF and 1 trajectory Hvib ( CMATRIX(nstates, nstates)
     ): vibronic Hamiltonian matrix, for 1 trajectory F ( CMATRIX(nstates,
     nstates) ): diagonal matrix of forces for all states, for a given DOF and
     for 1 trajectory C ( CMATRIX(nstates, 1) ): amplitudes of electronic
     states, for 1 trajectory mass ( double ): mass of the given DOF act_state (
     int ): active electronic state index dt ( double ): integration timestep
...
//                           nHamiltonian& ham, vector<CMATRIX>& projectors,
//                           double dt){
void wp_reversal_events(dyn_variables &dyn_var, nHamiltonian &ham, double dt) {
  /**
     This function computes the flags to decide on whether WP on any given
     surface of every trajectory reflects (1) or not (0).

     This is according to:
     Xu, J.; Wang, L. "Branching corrected surface hopping: Resetting
     wavefunction coefficients based on judgement of wave packet reflection" J.
     Chem. Phys. 2019,  150,  164101
  ...
  */
  ...
  for (int itraj = 0; itraj < ntraj; itraj++) {
    ...
    double T_a = compute_kinetic_energy(
        pa, *dyn_var.iM); // kinetic energy on the active state
    ...
    for (int i = 0; i < nadi; i++) {
      if (i != a) {
        ...
        double T_i =
            T_a + E_a - E_i; // predicted kinetic energy on other state i

        // WP momenta on all surfaces
        if (T_i < 0) {
          pi = 0.0000001 * pa;
        } // infinitesimally small along pa
        else {
          pi = sqrt(T_i / T_a) * pa;
        } // just along pa, conserving energy
...
CMATRIX bcsh(CMATRIX &Coeff, double dt, vector<int> &act_states,
  ...
  Xu, J.; Wang, L. "Branching corrected surface hopping: Resetting wavefunction
  coefficients based on judgement of wave packet reflection" J. Chem. Phys.
  2019,  150,  164101
  ...
  \param[in]          dt [ float ] The integration timestep. Units = a.u. of
  time
  \param[in]      act_st [ list of integer ] The active state indices for all
  trajectories
  \param[in]      reversal_events [ MATRIX(nadi, ntraj) ]  > 0 - wp on that
  state reverses

  The function returns:
  C [ CMATRIX ] - the updated state of the electronic DOF, in the same data type
  as the input
  ...
  */
  ...
  for (int itraj = 0; itraj < ntraj; itraj++) {
    ...
    if (reversal_events.get(a, itraj) > 0) {
      collapse(C, itraj, a, 0);
    }

    else { // Otherwise, let's project out the all the nonactive states that
           // reflect

      for (int i = 0; i < nadi; i++) {
        if (i != a) {
          if (reversal_events.get(i, itraj) > 0) {
            project_out(C, itraj, i);
          }
        } // if
      } // for i
...
CMATRIX mfsd(MATRIX &p, CMATRIX &Coeff, MATRIX &invM, double dt,
             vector<int> &act_states, vector<MATRIX> &decoherence_rates,
             nHamiltonian &ham, Random &rnd, int isNBRA) {
  /**
  ...
  stochastic decoherence (MF-SD): A new algorithm for nonadiabatic mixed
  quantum/classical molecular-dynamics simulations with nuclear-induced
  decoherence" J. Chem. Phys. 123, 234106, 2005.
  ...
  \param[in]      act_st [ list of integer ] The active state indices for all
  trajectories
  \param[in]      decoherence_rates [ MATRIX(nadi, nadi) x ntraj ]  -
  decoherence rates (1/tau_i)
  ...
  */
  ...
  for (auto itraj = 0; itraj < ntraj; itraj++) {
    ...
    for (auto i = 0; i < nadi; i++) {

      complex<double> c_i = C.get(i, itraj);

      // Probability of decoherence on state i
      // Here, we assume that the state-only decoherence rates are on the
      // diagonal
      double P_i;
      if (isNBRA == 1) {
        P_i =
            (std::conj(c_i) * c_i).real() * decoherence_rates[0].get(i, i) * dt;
      } else {
        P_i = (std::conj(c_i) * c_i).real() *
              decoherence_rates[itraj].get(i, i) * dt;
      }
      if (ksi < P_i) {
    ...
    if (summ_prob > 0.0) {
      // Renormalize relative probabilities of the collapse on any of non-unique
      // states
      for (auto i = 0u; i < proposed_states.size(); i++) {
      ...
      // double E_new = ham.get_ham_adi(_id).get(decohered_state,
      // decohered_state).real();

      // cout<<"Decohered state = "<<decohered_state<<" E_old = "<<E_old<<"
      // E_new = "<<E_new<<endl;
        ...
        for (idof = 0; idof < ndof; idof++) {
          if (i != decohered_state) {
            nac_eff.add(
                idof, 0,
                P_i *
                    ham.get_dc1_adi(idof, _id).get(i, decohered_state).real());
          }
        } // idof
      } // i
      ...
      if (can_rescale_along_vector(E_old, E_new, p_i, invM, nac_eff) &&
        ...
        for (idof = 0; idof < ndof; idof++) {
          p.set(idof, itraj, p_i.get(idof, 0));
        }
...
CMATRIX mfsd(MATRIX &p, CMATRIX &Coeff, MATRIX &invM, double dt,
             vector<int> &act_states, vector<MATRIX> &decoherence_rates,
             nHamiltonian &ham, Random &rnd) {

  int is_nbra = 0;
  return mfsd(p, Coeff, invM, dt, act_states, decoherence_rates, ham, rnd,
              is_nbra);
}

void xf_init_AT(dyn_variables &dyn_var, int traj, int ist) {
  /**
  \brief Initialize the ist-th auxiliary trajectory. When ist==-1, initialize
  all auxiliary trajectories
...
void xf_destroy_AT(dyn_variables &dyn_var, double threshold) {
  /**
  \brief When the electronic state recovers to an adiabatic state, destroy
...
void xf_destroy_AT(dyn_variables &dyn_var, nHamiltonian &ham,
                   double threshold) {
  /**
  \brief When the electronic state recovers to an adiabatic state, destroy
  auxiliary trajectories Here, momentum rescaling is also performed.
  */
  int traj;
  int ntraj = dyn_var.ntraj;
  int nadi = dyn_var.nadi;
    ...
    for (int i = 0; i < nadi; i++) {
      ...
      if (is_mixed[i] == 1) {
        if (a_ii > upper_lim) {
          ...
          MATRIX p_real(dyn_var.ndof, 1);
          p_real = dyn_var.p->col(traj);
          double alpha =
              compute_kinetic_energy(p_real, *dyn_var.iM) + Epot_old - Epot;

          if (alpha > 0.0) {
            alpha /= compute_kinetic_energy(p_real, *dyn_var.iM);
          } else {
            alpha = 0.0;
            cout << "Total energy is drifted due to adiabatic recovery to a "
                    "classically forbidden state!"
                 << endl;
          }

          for (int idof = 0; idof < dyn_var.ndof; idof++) {
            dyn_var.p->set(idof, traj,
                           dyn_var.p->get(idof, traj) * sqrt(alpha));
          }
...
void xf_create_AT(dyn_variables &dyn_var, double threshold) {
  /**
  \brief When the electronic state is in a superposition between adiabatic
  states, create auxiliary trajectories
...
void xf_hop_reset(dyn_variables &dyn_var, vector<int> &accepted_states,
                  vector<int> &initial_states) {
  int traj;
  int ntraj = dyn_var.ntraj;
  ...
  for (traj = 0; traj < ntraj; traj++) {
    ...
    if (accepted_states[traj] != initial_states[traj]) {
      xf_init_AT(dyn_var, traj, -1);
      cout << "destroy auxiliary trajectories of traj " << traj
           << " due to a hop" << endl;
    }
  } // traj
}

void shxf(dyn_variables &dyn_var, nHamiltonian &ham, nHamiltonian &ham_prev,
          dyn_control_params &prms) {
  /**
  \brief The generic framework of the SHXF (Surface Hopping based on eXact
  Factorization) method of Ha, J.-K.; Lee, I. S.; Min, S. K. J. Phys. Chem.
  Lett. 2018, 9, 1097
  ...
  */
    ...
    for (int i = 0; i < nadi; i++) {
      if (is_mixed[i] == 1) {
        ...
        if (is_first[i] == 1) {
          ...
          for (int idof = 0; idof < ndof; idof++) {
            q_aux.set(i, idof, dyn_var.q->get(idof, traj));
          }
        } else {
          if (i == a) {
            for (int idof = 0; idof < ndof; idof++) {
            ...
            for (int idof = 0; idof < ndof; idof++) {
              q_aux.add(i, idof,
                        invM.get(idof, 0) * p_aux.get(i, idof) * prms.dt);
            }
          }
        }
      }
  ...
  for (int traj = 0; traj < ntraj; traj++) {
    ...
    for (int i = 0; i < nadi; i++) {
      if (is_mixed[i] == 1) {
        if (is_fixed[i] == 1 or is_keep[i] == 1) {
          continue;
        }
        ...
        if (i == a) {
          ...
          if (is_first[i] == 1) {
            alpha = compute_kinetic_energy(p_real, invM) +
                    ham_adi.get(a, a).real() - ham_adi.get(i, i).real();
          } else {
            p_aux_temp = p_aux_old.row(i).T();
            alpha = compute_kinetic_energy(p_aux_temp, invM) +
                    ham_adi_prev.get(i, i).real() - ham_adi.get(i, i).real();
          }
        }
        ...
        if (alpha < 0.0) {
          ...
          if (prms.project_out_aux == 1) {
            ...
            xf_init_AT(dyn_var, traj, -1);
            cout << "Project out a classically forbidden state " << i
                 << " on traj " << traj << endl;
            break;
          }
        }
    ...
    if (is_tp == 1) {
      if (prms.tp_algo == 1) {
        ...
        xf_init_AT(dyn_var, traj, -1);
        cout << "Collapse to the active state " << a
        ...
        cout << "Fix auxiliary trajectory except for the active state " << a
             << " at a classical turning point on traj " << traj << endl;
        ...
        cout << "Keep auxiliary momenta except for the active state " << a
             << " at a classical turning point on traj " << traj << endl;
      }
    ...
    for (int i = 0; i < nadi; i++) {
      if (is_mixed[i] == 1) {
        if (is_first[i] == 1) {
          ...
          for (int idof = 0; idof < ndof; idof++) {
            nab_phase.set(i, idof,
                          nab_phase_old.get(i, idof) + p_aux.get(i, idof) -
                              p_aux_old.get(i, idof));
          } // idof
        }
      }
    } // i
  ...
  // cout << "SHXF " << dyn_var.p_quant->get(0,0) << " " <<
  // dyn_var.q_aux[0]->get(0,0) << " " << dyn_var.q_aux[0]->get(1,0)
  //      << " " << dyn_var.p_aux[0]->get(0,0) << " " <<
  //      dyn_var.p_aux[0]->get(1,0) <<endl; // Debug
}

void mqcxf(dyn_variables &dyn_var, nHamiltonian &ham, nHamiltonian &ham_prev,
           dyn_control_params &prms) {
  /**
  \brief The generic framework of the MQCXF (Mixed Quantum-Classical based on
  eXact Factorization) method of Ha, J.-K.; Min, S. K. J. Chem. Phys. 2022, 156,
  174109
  ...
  */
  ...
  for (int traj = 0; traj < ntraj; traj++) {
    ...
    for (int i = 0; i < nadi; i++) {
      if (is_mixed[i] == 1) {
        ...
        if (is_first[i] == 1) {
          alpha = compute_kinetic_energy(p_real, invM) + Epot -
                  ham_adi.get(i, i).real();
        } else {
          // p_aux_temp = p_aux_old.row(i).T();
          // alpha = compute_kinetic_energy(p_aux_temp, invM) +
          // ham_adi_prev.get(i,i).real() - ham_adi.get(i,i).real();
    ...
    if (is_tp == 1) {
      if (prms.tp_algo == 1) {
        ...
        if (alpha > 0.0) {
          ...
          alpha = 0.0;
          cout << "Total energy is drifted due to the dynamics initialization "
                  "at a classical turning point"
               << endl;
        }
  ...
  // Propagate the spatial derivative of phases; the E-based approximation is
  // used
  for (int traj = 0; traj < ntraj; traj++) {
    vector<int> &is_mixed = dyn_var.is_mixed[traj];
    vector<int> &is_first = dyn_var.is_first[traj];
    ...
    for (int i = 0; i < nadi; i++) {
      if (is_mixed[i] == 1) {
        for (int idof = 0; idof < ndof; idof++) {
          nab_phase.set(i, idof,
                        -0.5 * E.get(i, i).real() * dyn_var.p->get(idof, traj) /
                            (Ekin + prms.e_mask));
        } // idof
      }
    } // i
...
void rotate_nab_phase(dyn_variables &dyn_var, nHamiltonian &ham,
                      dyn_control_params &prms) {
  int ntraj = dyn_var.ntraj;
  ...
  for (int traj = 0; traj < ntraj; traj++) {
    ...
    if (prms.assume_always_consistent) {
      T_new.identity();
    }
    ...
    for (int idof = 0; idof < ndof; idof++) {
      for (int i = 0; i < nst; i++) {
        if (is_mixed[i] == 1) {
          phi.set(i, i,
                  complex<double>(dyn_var.nab_phase[traj]->get(i, idof), 0.0));
        }
      }
    ...
    if (prms.decoherence_algo == 5) {
      for (int i = 0; i < nst; i++) {
        if (is_mixed[i] == 1) {
          for (int idof = 0; idof < ndof; idof++) {
            dyn_var.nab_phase_old[traj]->set(
                i, idof, dyn_var.nab_phase[traj]->get(i, idof));
          }
        }
      }
    }
...
void update_ham_xf(dyn_variables &dyn_var) {
  ...
  for (int traj = 0; traj < ntraj; traj++) {
    ...
    for (int i = 0; i < nst; i++) {
      if (is_mixed[i] == 1) {
        for (int idof = 0; idof < ndof; idof++) {
          dyn_var.p_quant->add(idof, traj,
                               0.5 /
                                   pow(dyn_var.wp_width->get(idof, traj), 2.0) *
                                   RHO.get(i, i).real() *
                                   (dyn_var.q_aux[traj]->get(a, idof) -
                                    dyn_var.q_aux[traj]->get(i, idof)));
    ...
    for (int idof = 0; idof < ndof; idof++) {
      ...
      for (int i = 0; i < nst; i++) {
        if (is_mixed[i] == 1) {
          iphi.set(i, i,
                   complex<double>(0.0, dyn_var.nab_phase[traj]->get(i, idof)));
        }
      }

      Ham_xf += -invM.get(idof, 0) * dyn_var.p_quant->get(idof, traj) *
                (RHO * iphi - iphi * RHO);
    }
...
void update_forces_xf(dyn_variables &dyn_var, nHamiltonian &ham,
  ...
  for (int traj = 0; traj < ntraj; traj++) {
    ...
    for (int idof = 0; idof < ndof; idof++) {
      for (int jdof = 0; jdof < ndof; jdof++) {
        ...
        dyn_var.f_xf->add(
            idof, traj,
            -2.0 * invM.get(jdof, 0) * dyn_var.p_quant->get(jdof, traj) *
                (dyn_var.VP->get(jdof, traj) * dyn_var.VP->get(idof, traj) -
                 temp.get(0, 0).real()));
      }
    }
...
void propagate_half_xf(dyn_variables &dyn_var, nHamiltonian &Ham,
                       dyn_control_params &prms) {
  int itraj, i, j;
