...
/**
  ...
  \brief The file implements functions for computing various properties of the
  wavefunctions
...
double Wfcgrid2::norm(int rep) {
  ...
  */
  ...
  //  else if(rep==1){  nrm = (lin_PSI_adi->H() *
  //  (*lin_PSI_adi)).get(0,0).real(); }
...
double Wfcgrid2::e_kin(vector<double> &mass, int rep) {
  ...
  */
  ...
  for (int npt1 = 0; npt1 < Npts; npt1++) {
    ...
    if (rep == 0) {
      res += kfactor *
             (reciPSI_dia[npt1].H() * reciPSI_dia[npt1]).get(0, 0).real();
      // nrm += (PSI_dia[npt1].H() * PSI_dia[npt1]).get(0,0).real();
      nrm += (reciPSI_dia[npt1].H() * reciPSI_dia[npt1]).get(0, 0).real();
    } else if (rep == 1) {
      res += kfactor *
             (reciPSI_adi[npt1].H() * reciPSI_adi[npt1]).get(0, 0).real();
      nrm += (PSI_adi[npt1].H() * PSI_adi[npt1]).get(0, 0).real();
    }
...
CMATRIX Wfcgrid2::get_pow_q(int rep, int n) {
  /**
    Compute the expectation value of coordinates of a nd-D wavefunction:
    <psi|r^n |psi> / <psi|psi>
...
CMATRIX Wfcgrid2::get_pow_p(int rep, int n) {
  /**
    Compute the expectation value of momentum of a nd-D wavefunction:
    <psi|(-i*hbar*d/dr)^n |psi> / <psi|psi>
  ...
  for (int npt1 = 0; npt1 < Npts; npt1++) {
    ...
    if (rep == 0) {
      ...
      for (istate = 0; istate < nstates; istate++) {
        pop_i = (PSI_dia[npt1].get(istate, 0) *
                 std::conj(PSI_dia[npt1].get(istate, 0)))
                    .real();
      ...
      for (istate = 0; istate < nstates; istate++) {
        pop_i = (PSI_adi[npt1].get(istate, 0) *
                 std::conj(PSI_adi[npt1].get(istate, 0)))
                    .real();
        res.add(istate, 0, pop_i);
      }
    }
...
MATRIX Wfcgrid2::get_pops(int rep, vector<double> &bmin, vector<double> &bmax) {
  /**
    ...
    Only the points that belong to a multidimensional box given by bmin and bmax
    parameters are included in the corresponding populations
  ...
  */
  ...
  int maxdim = ndof;
  if (bmin.size() < static_cast<std::size_t>(maxdim)) {
    maxdim = bmin.size();
  }
...
MATRIX Wfcgrid2::get_coherences(int rep) {
  ...
  */
  ...
  if (rep == 0) {
    for (npt1 = 0; npt1 < Npts; npt1++) {
      for (istate = 0; istate < nstates; istate++) {
        rho_i = (PSI_dia[npt1].get(istate, 0) *
                 std::conj(PSI_dia[npt1].get(istate, 0)))
                    .real();
        for (jstate = 0; jstate < nstates; jstate++) {
          rho_j = (PSI_dia[npt1].get(jstate, 0) *
                   std::conj(PSI_dia[npt1].get(jstate, 0)))
                      .real();
    ...
    for (npt1 = 0; npt1 < Npts; npt1++) {
      for (istate = 0; istate < nstates; istate++) {
        rho_i = (PSI_adi[npt1].get(istate, 0) *
                 std::conj(PSI_adi[npt1].get(istate, 0)))
                    .real();
        for (jstate = 0; jstate < nstates; jstate++) {
          rho_j = (PSI_adi[npt1].get(jstate, 0) *
                   std::conj(PSI_adi[npt1].get(jstate, 0)))
                      .real();
          coh_ij = rho_i * rho_j / temp.get(npt1, 0);
          res.add(istate, jstate, coh_ij);
        } // for jstate
...
projection of the wfc)
  ...
  MATRIX res(nstates, 1);
  if(xf < rmin[fdim]) {
    cout<<"The flux point is outside the grid boundaries xf = "<<xf<<" rmin =
"<<rmin[fdim]<<"  for fdim = "<< fdim <<endl; cout<<"Exiting\n"; exit(0);
  }
  if(xf > rmax[fdim]) {
    cout<<"The flux point is outside the grid boundaries xf = "<<xf<<" rmax =
"<<rmax[fdim]<<"  for fdim = "<< fdim <<endl; cout<<"Exiting\n"; exit(0);
  }
