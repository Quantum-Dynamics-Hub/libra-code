...
/**
  ...
  \brief The file implements the TD-SE integrators based on SOFT
  (Split-Operator-Fourier-Transform) method of Kosloff and Kosloff
...
void Wfcgrid2::update_propagator_H(double dt) {
  /**
    ...
    exp(-i*dt*E) = exp(-i * dt * [Re(E)+i*Im(E)]) = exp(-i*dt*Re(E)) *
    exp(dt*Im(E))
                                                    ----- ex -------   ---- scl
    ----
  ...
  */
  ...
  CMATRIX S(nstates, nstates);
  S.Init_Unit_Matrix(1.0);
  ...
  if (ndof == 1) {
    ...
    cout << "WARNING: Beware of the phase problem when initializing the "
            "dynamics!\n";
    cout << "for problems with ndof > 1, no phase consistency correction "
            "applied yet,\n";
    cout << "which may result in wrong kinetic energy and overall dynamics "
            "problems\n";
  }
  ...
  for (int npt1 = 0; npt1 < Npts; npt1++) {
    ...
    for (int nst = 0; nst < nstates; nst++) {
      cs = std::cos(dt * (Hadi[npt1].get(nst, nst).real() +
                          Vcomplex[npt1].get(nst, nst).real()));
      si = std::sin(dt * (Hadi[npt1].get(nst, nst).real() +
                          Vcomplex[npt1].get(nst, nst).real()));
      scl = std::exp(dt * (Hadi[npt1].get(nst, nst).imag() +
                           Vcomplex[npt1].get(nst, nst).imag()));
...
void Wfcgrid2::update_propagator_H_lin(double dt) {
  ...
  */
  ...
  for (i = 0; i < dim; i++) {
    for (j = 0; j < dim; j++) {
      if (std::abs(U.get(i, j)) > 1e-5) {
        cnt += 1;
      }
    }
  }
  cout << "number of non-zero elements = " << cnt << " out of " << (dim * dim)
       << endl;
  ...
  *lin_U = U;
  ...
  for (i = 0; i < dim; i++) {
    for (j = 0; j < dim; j++) {
      ...
      sum = 0.0;
      for (a = 0; a < dim; a++) {
        sum += U.get(i, a) * expE[a] * Udag.get(a, j);
      }
...
void Wfcgrid2::update_propagator_K(double dt, vector<double> &mass) {
  /**
    ...
    \param[in] mass Masses of the particle in all dimensions (effective DOF)
    [a.u.]
  ...
  */
  ...
  for (int npt1 = 0; npt1 < Npts; npt1++) {
    ...
    for (int nst = 0; nst < nstates; nst++) {
      expK[npt1].set(nst, nst,
                     complex<double>(std::cos(kfactor), std::sin(kfactor)));
    } // for nst
...
void Wfcgrid2::SOFT_propagate() {
  ...
  */
  ...
  for (npt1 = 0; npt1 < Npts; npt1++) {
    PSI_dia[npt1] = expH[npt1] * PSI_dia[npt1];
  }
  /*
  ...
  */
  ...
  for (npt1 = 0; npt1 < Npts; npt1++) {
    reciPSI_dia[npt1] = expK[npt1] * reciPSI_dia[npt1];
  }
