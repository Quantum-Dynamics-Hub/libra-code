...
/**
  ...
  \brief The file implements the TD-SE integrators based on the Colbert-Miller
  DVR method
*/
  ...
  for (i = 0; i < npts; i++) {
    for (j = 0; j < npts; j++) {
      ...
      if (i == j) {
        ...
        T_ij =
            2.0 / double((i - j) * (i - j)) - 2.0 / double((i + j) * (i + j));
      }
...
vector<CMATRIX> Wfcgrid2::T_PSI(vector<CMATRIX> &inp_psi, vector<int> &bc_type,
                                vector<double> &mass, complex<double> scaling) {
  /**
    \brief Application of the kinetic energy operator to the wavefunction,
    according to Colbert-Miller formula
    ...
    0 - finite (the most general case) - (a, b) - use rmin and rmax for that
    dof, in this case 1 - (-inf, +inf) 2 - (0, +inf)

  */

  int idof, ipt1, ipt2, istate, i, j;
  ...
  for (idof = 0; idof < ndof; idof++) {
    ...
    if (bc_type[idof] == 0) {
      dvr0(reT[idof], npts[idof], rmin[idof], rmax[idof], mass[idof]);
    } else if (bc_type[idof] == 1) {
      dvr1(reT[idof], npts[idof], dr[idof], mass[idof]);
    } else if (bc_type[idof] == 2) {
      dvr2(reT[idof], npts[idof], dr[idof], mass[idof]);
    }
  }
    ...
    for (idof = 0; idof < ndof; idof++) {
      ...
      for (j = 0; j < npts[idof]; j++) {
        ...
        for (istate = 0; istate < nstates; istate++) {
          Tpsi[ipt1].add(istate, 0,
                         reT[idof].get(i, j) * inp_psi[ipt2].get(istate, 0));
        } // for istate
...
vector<CMATRIX> Wfcgrid2::T_PSI_adi(vector<int> &bc_type, vector<double> &mass,
                                    complex<double> scaling) {

  return T_PSI(PSI_adi, bc_type, mass, scaling);
}

vector<CMATRIX> Wfcgrid2::T_PSI_dia(vector<int> &bc_type, vector<double> &mass,
                                    complex<double> scaling) {

  return T_PSI(PSI_dia, bc_type, mass, scaling);
}

CMATRIX Wfcgrid2::operator_T(vector<int> &bc_type, vector<double> &mass,
                             complex<double> scaling) {
  /**
...
vector<CMATRIX> Wfcgrid2::expT_PSI(double dt, vector<double> &mass,
                                   int nterms) {
  ...
  double nrm = 0.0;
  for (i = 0; i < Npts; i++) {
    nrm += (res[i].H() * res[i]).get(0, 0).real();
  }

  nrm = sqrt(2.0 * Npts / nrm);

  for (i = 0; i < Npts; i++) {
    res[i] *= complex<double>(nrm, 0.0);
  }

  return res;
}

void Wfcgrid2::Colbert_Miller_SOFT(CMATRIX &expT, vector<CMATRIX> &expV,
                                   int opt) {
  /**
  ...
  */
  ...
  if (opt == 0) {
    for (i = 0; i < Npts; i++) {
      PSI_dia[i] = expV[i] * PSI_dia[i];
    }

    for (istate = 0; istate < nstates; istate++) {

      for (i = 0; i < Npts; i++) {
        psi_tmp.set(i, 0, PSI_dia[i].get(istate, 0));
      }
      psi_tmp = expT * psi_tmp;
      for (i = 0; i < Npts; i++) {
        PSI_dia[i].set(istate, 0, psi_tmp.get(i, 0));
      }
...
vector<CMATRIX> Wfcgrid2::nubla_PSI(int idof, vector<CMATRIX> &inp_psi,
                                    complex<double> scaling) {
...
vector<CMATRIX> Wfcgrid2::V_PSI(vector<CMATRIX> &V, vector<CMATRIX> &inp_psi,
                                complex<double> scaling) {
  /**
...
vector<CMATRIX> Wfcgrid2::H_PSI_adi(vector<double> &mass) {
  /*  Compute the action of the H(effective) on the adiabatic wfc
    ...
    i * hbar * psi_a * d(xi_a)/dt  = (T xi * psi - sum_{alp} {  nubla_{alp} xi /
    m_{alp} } * nubla_{alp} psi +  psi * T + (V - nac2/2*m) * xi * psi )
  */
  ...
  for (idof = 0; idof < ndof; idof++) {
    ...
    for (ipt = 0; ipt < Npts; ipt++) {
      res[ipt] += NAC1[ipt][idof] * tmp_psi[ipt];
    }
  }
  ...
  for (ipt = 0; ipt < Npts; ipt++) {
    ...
    for (idof = 0; idof < ndof; idof++) {
      tmp += 0.5 * NAC2[ipt][idof] / mass[idof];
    }
...
void Wfcgrid2::Colbert_Miller_propagate_adi2(double dt, vector<double> &mass) {
  ...
  */
  ...
  for (npt1 = 0; npt1 < Npts; npt1++) {
    PSI_tmp[npt1] -= 2.0 * eye * dt * Hpsi[npt1];
  }
...
void Wfcgrid2::Colbert_Miller_propagate_dia1(double dt, vector<double> &mass) {
  ...
  */
  ...
  /// Copy the current wavefunction into the previous one
  for (npt1 = 0; npt1 < Npts; npt1++) {
    PSI_dia_past[npt1] = PSI_dia[npt1];
  }

  /// Copy the temporary results into the current wavefunction
  for (npt1 = 0; npt1 < Npts; npt1++) {
    PSI_dia[npt1] = PSI_tmp[npt1];
  }
}
