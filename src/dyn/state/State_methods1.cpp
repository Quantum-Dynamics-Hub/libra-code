...
//  if(md->ensemble=="NVE"){is_thermostat = 0; is_barostat = 0; }
  ...
  if(is_thermostat){
    thermostat->set_Nf_t(system->Nf_t); 
    thermostat->set_Nf_r(system->Nf_r);
    if(is_barostat){  thermostat->set_Nf_b(Nf_b); }
    thermostat->init_nhc();
    init_velocities(thermostat->Temperature);
  }
  if(is_barostat){   
    barostat->set_Nf_t(system->Nf_t);
    barostat->set_Nf_r(system->Nf_r);
  ...
  if(md->integrator=="Terec"||md->integrator=="qTerec"){
    for(int i=0;i<system->Number_of_fragments;i++){
      system->Fragments[i].Group_RB.initialize_terec(md->terec_exp_size);
    }
  }
  is_md_initialized = 1;
...
void State::run_md(){
  ...
  while(md->curr_step<md->max_step){
    ...
    if(is_thermostat){
      ...
      thermostat->propagate_nhc(dt_half,system->ekin_tr(),system->ekin_rot(),ekin_baro);
    }
...
//    }
    ...
    if(is_barostat){ 
      if(system->is_Box) {
        ...
        system->Box  =  barostat->pos_scale(dt) * system->Box;
/*
