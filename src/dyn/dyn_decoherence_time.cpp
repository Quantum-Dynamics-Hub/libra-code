...
/**
  ...
  \brief The file implements the methods to compute dephasing rates and
...
MATRIX edc_rates(CMATRIX &Hvib, double Ekin, double C_param, double eps_param,
  ...
  /**
      ...
      \param[in]        Ekin [ float ] The classical kinetic energy of nuclei.
     Units = Ha
      \param[in]     C_param [ float ] The method parameter, typically set
     to 1.0 Ha
      \param[in]   eps_param [ float ] The method parameter, typically set to
     0.1 Ha
      \param[in]      isNBRA [ int   ] The method for considering NBRA-type
     calculations
  */
  ...
  for (i = 0; i < nst; i++) {
    for (j = 0; j < nst; j++) {
      ...
      double itau = fabs(Hvib.get(i, i).real() - Hvib.get(j, j).real()) /
                    (C_param + (eps_param / Ekin));
...
vector<MATRIX> edc_rates(vector<CMATRIX> &Hvib, vector<double> &Ekin,
                         double C_param, double eps_param, int isNBRA) {
  ...
  if (isNBRA == 1) {
    if (Ekin.size() != static_cast<std::size_t>(ntraj)) {
      cout << "ERROR in edc_rates: the sizes of the input variables Hvib and "
              "Ekin are inconsistent\n";
...
vector<MATRIX> edc_rates(vector<CMATRIX> &Hvib, vector<double> &Ekin,
                         double C_param, double eps_param) {
  int is_nbra = 0;
  return edc_rates(Hvib, Ekin, C_param, eps_param, is_nbra);
}

void dephasing_informed_correction(MATRIX &decoh_rates, CMATRIX &Hvib,
                                   MATRIX &ave_gaps, int isNBRA) {
  /**
      This function computes the corrected dephasing rates
      The correction is based on the instnataneous energy levels and on the
     average of absolute values of the energy gaps

      Reference: Sifain, A. E.; Wang, L.; Teritiak, S.; Prezhdo, O. V. J. Chem.
     Phys. 2019, 150, 194104

      \param[in, out]  decoh_rates [ MATRIX ] uncorrected decoherence rates
     [units: a.u.t.^-1]
      \param[in]             Hvib  [ CMATRIX ] Instantaneous vibronic
     Hamiltonian [units: Ha]
      \param[in]          ave_gaps [ MATRIX ] time-averaged module of the energy
     level gaps: <|E_i - E_j|>  [units: Ha]
      \param[in]            isNBRA [ int    ] The method for considering
     NBRA-type calculations
...
void dephasing_informed_correction(vector<MATRIX> &decoh_rates,
                                   vector<CMATRIX> &Hvib, MATRIX &ave_gaps,
                                   int isNBRA) {
  ...
  if (isNBRA == 1) {
    ...
    if (decoh_rates.size() != static_cast<std::size_t>(ntraj)) {
      cout
          << "ERROR in dephasing_informed_correction: the sizes of the input variables \
...
MATRIX coherence_intervals(CMATRIX &Coeff, MATRIX &rates) {
  /**
    This function computes the time-dependent (and population-dependent)
    coherence intervals (the time after which different states should experience
    a decoherence event) as described by Eq. 11 in: Jaeger, H. M.; Fischer, S.;
    Prezhdo, O. V. Decoherence-Induced Surface Hopping. J. Chem. Phys. 2012,
    137, 22A545.
...
MATRIX coherence_intervals(CMATRIX &Coeff, vector<MATRIX> &rates) {
  /**
    ...
    \param[in] Coeff - CMATRIX(nstates, ntraj) Amplitudes of the electronic
    states
    \param[in] rates - ntraj matrices of nstates x nstates - Matrices containing
    the decoherence rates (inverse of the decoherence time for each given pair
    of states) for each trajectory

    Returns: A matrix of the coherence intervals for each state for each
    trajectory
  ...
  */
  ...
  if (static_cast<std::size_t>(ntraj) != rates.size()) {
    cout << "ERROR in coherence_intervals: the ntraj dimensions do not agree "
            "for Coeff and rates\n";
...
vector<MATRIX> schwartz_1(dyn_control_params &prms, CMATRIX &amplitudes,
                          nHamiltonian &ham, MATRIX &inv_alp) {
  /**
    Compute decoherence rates 1/tau_i for all states and all trajectories
    according to Schwartz prescription
    ...
    MATRIX(nstates, ntraj) - 1/tau - decoherence rates for all states and
    trajectories
  */
  ...
  /// F_mf = ham.Ehrenfest_forces_adi(amplitudes, 1).real();
  /// //aux_get_forces(prms_mf, amplitudes, projectors, act_states, ham);
  ...
  for (int i = 0; i < nstates; i++) {
    ...
    F_st = ham.forces_adi(act_states)
               .real(); // aux_get_forces(prms_mf, amplitudes, projectors,
                        // act_states, ham);
...
vector<MATRIX> schwartz_1(dyn_control_params &prms, CMATRIX &amplitudes,
                          MATRIX &p, nHamiltonian &ham, MATRIX &w) {
  /**
    Compute decoherence rates 1/tau_i for all states and all trajectories
    according to Schwartz prescription
  ...
  for (int i = 0; i < nstates; i++) {
    ...
    for (int itraj = 0; itraj < ntraj; itraj++) {
      ...
      for (int idof = 0; idof < ndof; idof++) {
        ...
        tau_inv2 +=
            0.25 * pow(4 * M_PI / (w2 * p.get(idof, itraj)), 2.0) * dF * dF;
      }
...
vector<MATRIX> schwartz_2(dyn_control_params &prms, nHamiltonian &ham,
                          MATRIX &inv_alp) {
  /**
    Compute decoherence rates 1/tau_ij for all pairs of states and all
    trajectories according to Schwartz state-pair prescription

    inv_alp - MATRIX(ndof, 1)

    Return:

    MATRIX(nstates, nstates) x ntraj  - 1/tau_ij - decoherence rates for all
    pairs of states and trajectories
  */
  ...
  //  dyn_control_params prms_st(prms);  prms_st.force_method = 1;
  //  prms_st.rep_force = 1; /// adiabatic, state-resolved force
  ...
  for (int i = 0; i < nstates; i++) {
    ...
    F[i] = ham.forces_adi(act_states_i)
               .real(); //  aux_get_forces(prms_st, amplitudes, projectors,
                        //  act_states_i, ham);
...
vector<MATRIX> Gu_Franco(dyn_control_params &prms, CMATRIX &amplitudes) {
  /**
    Compute decoherence rates 1/tau_i for all states and all trajectories
    according to Eq. 25 in:

    (1) Gu, B.; Franco, I. Generalized Theory for the Timescale of Molecular
    Electronic Decoherence in the Condensed Phase. J. Phys. Chem. Lett. 2018, 9
    (4), 773–778. https://doi.org/10.1021/acs.jpclett.7b03322.

    also notice this correction:

    (2) Gu, B.; Franco, I. Correction to "Generalized Theory for the Timescale
    of Molecular Electronic Decoherence in the Condensed Phase." J. Phys. Chem.
    Lett. 2020, 11 (6), 2044–2045. https://doi.org/10.1021/acs.jpclett.0c00437.
