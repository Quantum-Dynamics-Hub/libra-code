...
/**
  \file NList.cpp
  \brief The file implements auxiliary and main functions for the neighbor list
  construction
...
MATRIX fold_coords(MATRIX &R, MATRIX3x3 &Box, std::string pbc_type) {
  /**
    ...
    \param[in] pbc_type The parameter controlling the periodicity (when and if
    folding) of the unit cell Can take values: "a", "b", "c", "ab", "ac", "bc",
    and "abc"
  ...
  */
  ...
  for (int step = 0; step < nsteps; step++) {
    ...
    for (int i = 0; i < nat; i++) {
      ...
      VECTOR r;
      r.x = R.get(3 * i, step);
      r.y = R.get(3 * i + 1, step);
      r.z = R.get(3 * i + 2, step);
      r = invBox * r;

      if ((pbc_type == "a") || (pbc_type == "ab") || (pbc_type == "ac") ||
          (pbc_type == "abc")) {
        r.x = r.x - floor(r.x);
      }
      if ((pbc_type == "b") || (pbc_type == "ab") || (pbc_type == "bc") ||
          (pbc_type == "abc")) {
        r.y = r.y - floor(r.y);
      }
      if ((pbc_type == "c") || (pbc_type == "ac") || (pbc_type == "bc") ||
          (pbc_type == "abc")) {
        r.z = r.z - floor(r.z);
      }
      r = Box * r;

      res.set(3 * i, step, r.x);
      res.set(3 * i + 1, step, r.y);
      res.set(3 * i + 2, step, r.z);
...
void max_vector(VECTOR &t1, VECTOR &t2, VECTOR &t3, VECTOR &T) {
  /**
    ...
    \param[out] T the resulting vector containing the projections of the
    maximal-length vector
...
void apply_pbc(MATRIX3x3 &H, int sz, VECTOR *in, VECTOR *out,
               vector<quartet> &T) {
  /**
    ...
    \param[in] in The pointer to the array of input atomic coordinates
    (unfolded)
    \param[out] out The pointer to the array of the output atomic coordinates
    (folded)
    \param[out] the array of integer vectors(in units of the cell vectors) that
    fold each atom into the central unit cell
  */
...
boost::python::list apply_pbc(MATRIX3x3 H, boost::python::list in,
                              boost::python::list t) {
  int i;
  vector<quartet> T;
  ...
  for (i = 0; i < sz2; i++) {
    r[i] = boost::python::extract<VECTOR>(in[i]);
  }
...
void serial_to_vector(int c, int Nx, int Ny, int Nz, int &nx, int &ny,
                      int &nz) {
  /**
    ...
    \param[in] Nx maximal number of cell replicas in x dimension (along a
    vector, actually)
    \param[in] Ny maximal number of cell replicas in y dimension (along b
    vector, actually)
    \param[in] Nz maximal number of cell replicas in z dimension (along c
    vector, actually)
    \param[out] nx the x (first) component of the integer translation vector for
    given cell
    \param[out] ny the y (second) component of the integer translation vector
    for given cell
    \param[out] nz the z (third) component of the integer translation vector for
    given cell
...
void serial_to_vector_symm(int c, int Nx, int Ny, int Nz, int &nx, int &ny,
                           int &nz) {
  /**
    \brief Mapping of a single integer index (serial) into 3 integers (vector)
...
void form_neibc(int c, vector<int> &neibc, int Nx, int Ny, int Nz, double cellx,
                double celly, double cellz, double Roff) {
  /**
    ...
    \param[in] c Index of the cell for which we want to construct neighbor cells
    list
    \param[out] neibc Indices (serial) of the cells that are neighbor to the
    cell c
    ...
    \param[in] Roff cutoff distance that is used to determine of the atoms are
    neighbor
  ...
  */
  ...
  int ax, ay, az; // how many cells in each direction are necessary to exceed
                  // Roff in corresponding direction
                  /*
                    ax = floor(Roff/cellx);
                    ay = floor(Roff/celly);
                    az = floor(Roff/cellz);
...
boost::python::list form_neibc(int c, int Nx, int Ny, int Nz, double cellx,
                               double celly, double cellz, double Roff) {
...
void find_min_shell(VECTOR &t1, VECTOR &t2, VECTOR &t3, VECTOR &g1, VECTOR &g2,
                    VECTOR &g3, double Roff, triple &minb, triple &maxb) {
  /**
    ...
    This is basically the way to construct the all the replicas of the original
    cell around it such that the most far distant point of the replicated cells
    is not closer than by Roff to any point (even the closest one) of the
    original cell.
    ...
    \param[out] minb is the triple containing the integer translations of the
    unit cell to form the minimal boundary of the super-cell
    \param[out] maxb is the triple containing the integer translations of the
    unit cell to form the maximal boundary of the super-cell
...
void make_nlist(int Nat, VECTOR *r, MATRIX3x3 &H, int maxa, int maxb, int maxc,
                double cellx, double celly, double cellz, double Roff,
  ...
  /**
    ...
    \param[in] r The pointer to the array containing the coordinates of all
    atoms
    \param[in] H is the matrix describing the shape and size of the unit cell
    \param[in] maxa The maximal number of periodic translation of the unit cell
    along a axis
    \param[in] maxb The maximal number of periodic translation of the unit cell
    along b axis
    \param[in] maxc The maximal number of periodic translation of the unit cell
    along c axis
    ...
    \param[in] Roff The cutoff distance which controls the formation of the
    neighbor list
  ...
  */
  ...
  apply_pbc(H, Nat, r, tmp,
            initT); // now all tmp in simulation cell, no outside
  ...
  vector<vector<int>> cell2at(
      Ncells,
      dummy); // cell2at[i] - contains complex indexes of all atoms in cell i
  ...
  vector<vector<int>>
      neibc; // indexes of neighboring cells for given cell index
  for (c = 0; c < Ncells; c++) {
    vector<int> neibc_c;
    form_neibc(c, neibc_c, Nx, Ny, Nz, cellx, celly, cellz, Roff);
    neibc.push_back(neibc_c);
    // Check indexes of neighbor cells
    for (auto nc = 0u; nc < neibc_c.size(); nc++) {
      if (neibc_c[nc] >= Ncells) {
        cout << "Hey! Potential problem\n";
      }
    }
  }
  ...
  int cc = (2 * maxc + 1) * (2 * maxb + 1) * maxa + (2 * maxc + 1) * maxb +
           maxc; // index of central cell
  ...
  for (int at_indx1 = 0; at_indx1 < Nat; at_indx1++) {
    ...
    for (c = 0; c < neibc[ci1].size(); c++) {
      ...
      for (a = 0; a < cell2at[ci2].size();
           a++) { // iterations over atoms in cell ci2
...
void make_nlist_auto(int Nat, VECTOR *r, MATRIX3x3 &H, double cellx,
                     double celly, double cellz, double Roff,
                     vector<vector<quartet>> &nlist) {
  /**
  ...
  ************************************************************************/
  ...
  // Calculate neighbors of each cell (sub-cell)
  // we use serial indexes of both central cell and its neighbors
  vector<vector<int>> neibc(
      Ncells, dummy); // indexes of neighboring cells for given cell index
  for (c = 0; c < Ncells; c++) {
    //    vector<int> neibc_c;
  ...
  int cc = (maxb.n3 - minb.n3 + 1) * (maxb.n2 - minb.n2 + 1) * (-minb.n1) +
           (maxb.n3 - minb.n3 + 1) * (-minb.n2) +
           (-minb.n3); // index of central cell
  ...
  for (int at_indx1 = 0; at_indx1 < Nat; at_indx1++) {
    ...
    int sz1 =
        neibc[ci1]
            .size(); // number of neighbor cells of the cell with index ci1

    int newsize = 0;
    for (c = 0; c < sz1; c++) {
      int ci2 = neibc[ci1][c]; // one of the neighboring cell of cell l
      int sz2 =
          cell2at[ci2].size(); // number of atoms in the cell with index ci2
      newsize += sz2; // total possible(max) number of neighbor complex atoms
    }
    if (nlist[at_indx1].capacity() <= static_cast<std::size_t>(newsize)) {
      nlist[at_indx1].reserve(newsize);
    }
    ...
    for (c = 0; c < sz1; c++) {
      ...
      for (a = 0; a < sz2; a++) { // iterations over atoms in cell ci2
        ...
        int at_indx2 = i2 % Nat;  // real index of atom, corresponding to the
                                  // atom with complex index i2
...
double energy(int Nat, VECTOR *r, MATRIX3x3 &H,
              vector<vector<quartet>> &nlist) {
  /**
    \brief Auxiliary function to test neighbor list

    The function computes the sum of distances from all atoms to all their
    replicas
    ...
    The function returs the computed sum - to serve a the quantitative
    descriptor of summation
  */
  ...
  for (int i = 0; i < Nat; i++) {
    ...
    for (int k = 0; k < sz; k++) {
      int j = nlist[i][k].j;
      res += (r[i] - (r[j] + nlist[i][k].n1 * t1 + nlist[i][k].n2 * t2 +
                      nlist[i][k].n3 * t3))
                 .length();
      cnt += 1.0;
    } // for k
...
void bruteforce(int Nat, VECTOR *r, MATRIX3x3 &H, int maxa, int maxb, int maxc,
