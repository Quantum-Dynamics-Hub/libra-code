...
DATA::DATA(const DATA &d) {
  ...
  if (d.is_scale_factor) {
    scale_factor = d.scale_factor;
    is_scale_factor = 1;
  }
  if (d.is_shift_amount) {
    shift_amount = d.shift_amount;
    is_shift_amount = 1;
  }
}
...
int operator!=(const DATA &d1, const DATA &d2) {
  ...
  if (!res) {
    for (auto i = 0u; i < d1.Data.size(); i++) {
      if (d1.Data[i] != d2.Data[i]) {
        res = 1;
      }
    }
...
int DATA::Calculate_Estimators(double &Ave, double &Var, double &Sd, double &Se,
  ...
  if (sz > 1) {
    ...
    std::cout << "Warning: Data size is less than 2. Can not calculate "
                 "statistics for such data\n";
  }
...
int DATA::Calculate_MiniMax(double &Min_Val, int &Min_Indx, double &Max_Val,
                            int &Max_Indx) {
...
int DATA::Calculate_Distribution(vector<double> &Interval,
                                 vector<double> &Density,
                                 vector<double> &Cumulative) {
  // Cumulative(x) - is the probability to find the point in the interval
  // (Intrval[0], x)
  int i, j;
  int sz = Data.size();
  ...
  for (j = 1; j < szi; j++) {
    Density[j] =
        (Cumulative[j] - Cumulative[j - 1]) / (Interval[j] - Interval[j - 1]);
  }
...
/*
int DATA::Lin_Regression(int flag,double& a,double& b, double& erra, double&
errb){
