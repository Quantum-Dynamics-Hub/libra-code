...
double Random::Gamma(double a) { return fact((int)(a - 1.0)); }

void Random::bin(vector<double> &in, double minx, double maxx, double dx,
...
double Random::p_uniform(double a, double b) { return (1.0 / (b - a)); }
...
double Random::normal() {
  ...
  if (i == 0) { // Tail
    i = 6;
    A = a[31]; // here I used 31 instead of 32 as in paper - this fixes the
               // center-tail gap
    do {
      u = 2 * u;
      A = A + d[i];
...
id1:
  ...
  if (u <= 0.009572265238289) {
    ...
    x = b * (1.0 + s / d);
    u = uniform(0.0, 1.0);
    if (log(u) > mu * (2.0 + log(x / mu) - x / b) + 3.7203284924588 - b -
                     log(sigma * d / b)) {
      goto id1;
    } else {
      return x;
    ...
    if (x < 0 || x > b) {
      ...
      if (u <= 1.0 - S * ((1.0 - 2.0 * s / V) * W - 1.0)) {
        return x;
      } else {
        if (log(u) > mu * (1.0 + log(x / mu)) - x + S) {
          goto id1;
        } else {
          return x;
        }
...
double Random::p_beta(double x, double a, double b) {
  ...
  return pow(x, (a - 1)) * pow((1 - x), (b - 1)) * Gamma(a + b) /
         (Gamma(a) * Gamma(b));
}
...
// Definition of the poisson processes is given by Gurevich (Books/Math/Teorija
// sluchainyx prozessov) This is in fact a Poisson process with given time t
...
// Definition of the poisson processes is given by Gurevich (Books/Math/Teorija
// sluchainyx prozessov) This is in fact a complete Poisson trajectory of length
// maxT and time steps dt
void Random::poiss(double lambda, double maxT, double dt,
  ...
  if (w >= c) {
    ...
    if (x > w) {
      ...
      do {
        ...
        if (u <= p) {
          k++;
        } // maybe u not n? <- that's right: must be u, not n!
        n = n - 1;
      } while (n > 1);
...
vector<double> Random::voron(int nstates, int active_state) {
  ...
  //  cout<<"In voron with nstates = "<<nstates<<" and active_state =
  //  "<<active_state<<endl;
