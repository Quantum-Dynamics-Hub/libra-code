...
DIIS::DIIS(int _N_diis_max, int Norb) {
  /**
    ...
    \param[in] _N_diis_max The maximal length of DIIS history - how many
    matrixes to store
    \param[in] Norb The size of the DIIS matrices. Well the name is a bit
    misleading. This is because I initially implemented it with the SCF
    convergence in mind, but in reality this is pretty general algorithm, so
    Norb is just the size of the problem
  */
...
void DIIS::update_diis_coefficients() {
  /**
    General case - holds true for both N_diis<N_diis_max and for
    N_diis==N_diis_max (after preliminary corrections ) Starting at this point
    we compute the extrapolation coefficients: Solving Ax = b, where b - are the
    errors, x - are the changes of the parameter space, A contain the
  ...
  */
  ...
  int debug_flag = 0;
  double diis_damp =
      0.0; // see [Hamilton,Pulay, JCP 84, 5728 (1986) ] - scale diagonal
           // element of B matrix by (1+diis_damp) to avoid numerical problems
           // associated with large diis coefficents
  ...
  if (debug_flag) {
    cout << "*diis_err[N_diis]= " << *diis_err[N_diis] << endl;
  }

  // Actual size of DIIS matrices at given call
  // We save this number, because N_diis can reduce in the following run, to
  // give full-rank matrices
  N_diis_curr = N_diis;
  ...
  if (rank == (N_diis + 2)) {
    if (debug_flag) {
      cout << "Matrix is well-conditioned\n" << *A << endl;
    }
  } else {
    if (debug_flag) {
      cout << "Matrix is ill-conditioned\n" << *A << endl;
    }

    int min_indx = 0;

    // Reduce DIIS matrix by removing older iterates untill it is
    // well-conditioned
    while (rank != (N_diis + 2)) {

      N_diis--;
      min_indx++; // this will hide first min_indx diis_err matrices from
                  // consideration
      ...
      for (i = 0; i <= N_diis; i++) {
        for (j = 0; j <= N_diis; j++) {
          tempA(i, j) =
              ((*diis_err[min_indx + i]).T() * (*diis_err[min_indx + j])).tr();

          if (i == j) {
            tempA(i, j) *= (1.0 + diis_damp);
          }
        }
  ...
  *x = A->lu().solve(*b);
  ...
  N_diis_eff = (N_diis_curr -
                N_diis); // keep in mind that N_diis is a current one (updated)
  N_diis = N_diis_curr;
...
void DIIS::add_diis_matrices(MATRIX *X, MATRIX *err) {
  /**
    This function adds information about new iteration, so it updates the DIIS
    input matrices

    \param[in] X the matrix of parameters change
    \param[in] err the matrix of the corresponding errors

    Note that this function will be only accumulating the matrices until the
    DIIS history is filled. Then, it will be rotating matrices, adding these
    matrices to the end and removing very first set of matrices (queue
    mechanism)
  ...
  */
  ...
  if (N_diis == 0) { // The very first iterate (most likely this will be used
                     // right after guess)
    ...
    *diis_err[0] = *err;
    ...
    if (N_diis >= N_diis_max) { // this happens because last call of
                                // add_diis_matrices incremented N_diis

      N_diis = N_diis_max - 1; // so we set N_diis to maximal valid index,
                               // pointing to the last entries in the arrays
...
void DIIS::add_diis_matrices(MATRIX &X, MATRIX &err) {
  add_diis_matrices(&X, &err);
}

void DIIS::extrapolate_matrix(MATRIX *X_ext) {
  /**
    Extrapolate X matrix
    Note the Fock matrix constructed below (extrapolated) will only be used to
    obtain density It will not be stored in diis_Fao_... (timing/sequence of
    function calls in scf() procedure is very important!!! )

    //!!!!!!!!!! Assume it is called just after add_diis_matrices !!!!!!!!!!!
    so we need to used decremented N_diis value!!!!!

    \param[out] X_ext The extrapolated input matrix (in context of SCF - this is
    an extrapolted Fock or density matrix)
...
boost::python::list DIIS::get_diis_X() {
  /**
    ...
    The returned objects are brand-new objects (constructed here), so don't
    worry about references
  */

  boost::python::list res;
  for (auto i = 0u; i < diis_X.size(); i++) {
    res.append(*diis_X[i]);
  }
  return res;
}
...
boost::python::list DIIS::get_diis_err() {
  ...
  */
  ...
  boost::python::list res;
  for (auto i = 0u; i < diis_err.size(); i++) {
    res.append(*diis_err[i]);
  }
  return res;
}
