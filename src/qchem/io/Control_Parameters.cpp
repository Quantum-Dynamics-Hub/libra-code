  ...
  for(int i=0;i<f_sz;i++){   // line
    if(file[i].size()>0){
      ...
      if(file[i][0]=="<calculation>"){
        ...
        for(int i1=i+1;i1<end_i;i1++){
          if(file[i1].size()>2){ 
            if(file[i1][0]=="charge_density_prefix"){  prms.charge_density_prefix = file[i1][2];  } 
...
//        cell = Cell(t1,t2,t3);
      ...
      else if(file[i][0]=="<coordinates>"){
        // search for end of this group
        int end_i = i;
        for(int i1=i+1;i1<f_sz;i1++){
          if(file[i1].size()>0){  if(file[i1][0]=="</coordinates>"){ end_i = i1; break; }    }// non-empty line
        }// for i1


        // Number of atoms and coordinate system
        if(file[i+1].size()>=3){
          prms.Natoms = atoi(file[i+1][0].c_str());
          prms.charge = atof(file[i+1][1].c_str());
          prms.spin = atoi(file[i+1][2].c_str());
        }else{
          cout<<"Error in input: First line following <coordinates> should have at least 3 entries: number_of_atoms  total_charge spin_multiplicity\n";
          exit(0);
        }

        prms.coordinates = file[i+2][0]; // Direct or Cartesian

        if(prms.coordinates=="Cartesian"||prms.coordinates=="Direct"){ ;; }
        else{  cout<<"Error: Line 2 or <coordinates> section must be either Cartisian or Direct\n"; exit(0); }
  
        // Actual coordinates and atom types
        for(int n=0;n<prms.Natoms;n++){
          std::string typ;
          VECTOR r, fract; 
          typ = file[i+3+n][0]; r  = VECTOR(atof(file[i+3+n][1].c_str()),atof(file[i+3+n][2].c_str()),atof(file[i+3+n][3].c_str()));
 
          // Convert coordinates
          if(prms.coordinates=="Cartesian"){ r *= Angst; } // convert to Bohr
          else if(prms.coordinates=="Crystal"||prms.coordinates=="Direct"){  
            fract = r;
            r = fract.x * prms.t1 + fract.y * prms.t2 + fract.z * prms.t3; // Unit cell vectors t1,t2,t3 are already converted to Bohr units
          }
          Atom at(typ,r);
          atoms.push_back(at);
        }// for n
