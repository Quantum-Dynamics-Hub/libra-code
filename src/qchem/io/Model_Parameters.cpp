...
void mEHT_K::set_mapping1(EHT_K& k, Nuclear& mol){
// This is similar to set_mapping, but we now assume atomic (not orbital) arrays
  ...
  for(int n=0;n<nat;n++){ // create a map of distinct types of atoms
    ...
    it_type = at_types.find(mol.at_type[n]);

    if(it_type==at_types.end()){  // new type
      at_types.insert(std::pair<std::string,int>(mol.at_type[n],ind)); ind++;
    }
  ...
  for(int n=0;n<nat;n++){
    ...
    int i1 = at_types[mol.at_type[n]]; // index of given atomic type within the set of the atom types present in this system
...
void set_parameters_eht_mapping1(Model_Parameters& modprms, Nuclear& mol){
  ...
  modprms.meht_k.set_mapping1(modprms.eht_k, mol);
}
