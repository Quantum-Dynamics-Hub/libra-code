...
int init_numbers(vector<int>& fragment,Electronic* el,vector<AO>& basis_ao, Model_Parameters& modprms, Nuclear& mol,double charge){
  ...
  for(i=0;i<fragment.size();i++){
    int at_indx = fragment[i];

    el->Nelec += modprms.PT[mol.at_type[at_indx]].Nval;  //num_valence_elec(mol.Z[at_indx]);

  }// for i


  // Cange the total number of electrons according to total charge
  ...
  // By this time, the number of (active) electrons should be known from bas_ function
