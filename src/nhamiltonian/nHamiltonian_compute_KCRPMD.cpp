...
/**
  ...
  \brief The file implements the calculations of the KC-RPMD (Kinetically
  Constrained Ring Polymer Molecular Dynamics) terms
...
vector<MATRIX> nHamiltonian::generate_m_matrices(double beta) {
  ...
  */
  ...
  if (ham_dia_mem_status == 0) {
    cout
        << "Error in generate_m_matrices(): the diabatic Hamiltonian matrix is not allocated \
  but it is needed for the calculations\n";
    exit(0);
  }

  if (ndia != 2) {
    cout << "Error in generate_m_matrices(): implementation only for ndia=2\n";
    exit(0);
  }
  ...
  if (children.size() == 0) {
    ...
    for (auto traj = 0u; traj < children.size(); traj++) {
      ...
      res[traj].set(0, 1,
                    -beta / children.size() * K *
                        exp(-beta / children.size() * V0));
      res[traj].set(1, 0,
                    -beta / children.size() * K *
                        exp(-beta / children.size() * V1));
...
double nHamiltonian::kcrpmd_effective_potential(vector<double> &y_aux_var,
                                                const MATRIX &q,
  ...
  /**
    ...
    beta - the inverse temperature Boltzmann factor in atomic units
    eta - geometric parameter conserving free energy of kinked pair formation ad
    defined in second KC-RPMD paper a - is the kinetic constraint ad-hoc
    parameter b - is the heavyside functional limit parameter c - is the
    constraint switching parameter d - is the free energy conservation switching
    parameter
  */

  if (ham_dia_mem_status == 0) {
    cout
        << "Error in kcrpmd_effective_potential(): the diabatic Hamiltonian matrix is not allocated \
  but it is needed for the calculations\n";
    exit(0);
  }

  if (ndia != 2) {
    cout << "Error in kcrpmd_effective_potential(): implementation only for "
            "ndia=2\n";
    exit(0);
  }
  ...
  if (children.size() == 1 and ntraj == 1) {
    ...
    if (beta * K > 1e-3) {
      VKP = Vg - log(1 + exp(-beta * (Ve - Vg)) - exp(-beta * (V0 - Vg)) -
                     exp(-beta * (V1 - Vg))) /
                     beta;
    } else if (beta * abs(V0 - V1) > 1e-7) {
      VKP = 0.5 * (V0 + V1) -
            log(pow(beta * K, 2) * sinh(0.5 * beta * (V0 - V1)) /
                (0.5 * beta * (V0 - V1))) /
                beta;
    } else {
    ...
    if (abs(y_aux_var[0] + 1) < 0.5) {
      ...
      V0 += (b * (2 * abs(y_aux_var[0] + 1) - 1) -
             log(1 / (1 + exp(-b * (2 * abs(y_aux_var[0] + 1) - 1))))) /
            beta;
    }
    if (abs(y_aux_var[0] - 1) < 0.5) {
      V1 += -log(1 / (1 + exp(b * (2 * abs(y_aux_var[0] - 1) - 1)))) / beta;
    } else {
      V1 += (b * (2 * abs(y_aux_var[0] - 1) - 1) -
             log(1 / (1 + exp(-b * (2 * abs(y_aux_var[0] - 1) - 1))))) /
            beta;
    }
    if (abs(y_aux_var[0]) < 0.5) {
      ...
      VKP += (b * (2 * abs(y_aux_var[0]) - 1) -
              log(1 / (1 + exp(-b * (2 * abs(y_aux_var[0]) - 1))))) /
             beta;
    }
  } else if (children.size() == static_cast<std::size_t>(ntraj)) {
    cout << "Error in kcrpmd_effective_potential() not implemented for quantum "
            "nuclei\n";
    exit(0);
  } else {
    cout << "ERROR: the size of the input is different from the number of "
            "children\n";
    exit(0);
  }
  ...
  res = RPMD_internal_potential(q, invM, beta) + Vshift -
        log(exp(-beta * (V0 - Vshift)) + exp(-beta * (VKP - Vshift)) +
            exp(-beta * (V1 - Vshift))) /
            beta;
...
MATRIX nHamiltonian::kcrpmd_effective_force(vector<double> &y_aux_var,
  ...
  */
  ...
  if (children.size() == 1 and ntraj == 1) {
    ...
    Fg = 0.5 * (F0 + F1) - ((V0 - V1) * (F0 - F1) + 4 * K * FK) /
                               (2 * sqrt(pow(V0 - V1, 2) + pow(2 * K, 2)));
    Fe = 0.5 * (F0 + F1) + ((V0 - V1) * (F0 - F1) + 4 * K * FK) /
                               (2 * sqrt(pow(V0 - V1, 2) + pow(2 * K, 2)));
    if (beta * K > 1e-3) {
      FKP = (Fg + exp(-beta * (Ve - Vg)) * Fe - exp(-beta * (V0 - Vg)) * F0 -
             exp(-beta * (V1 - Vg)) * F1) /
            (1 + exp(-beta * (Ve - Vg)) - exp(-beta * (V0 - Vg)) -
             exp(-beta * (V1 - Vg)));
    } else if (beta * abs(V0 - V1) > 1e-7) {
      FKP = 0.5 * (F0 + F1) +
            (F0 - F1) *
                (1 / (beta * (V0 - V1)) - 0.5 * cosh(0.5 * beta * (V0 - V1)) /
                                              sinh(0.5 * beta * (V0 - V1))) -
            2 * FK / (beta * K);
    } else {
    ...
    Fw = (F0 - F1 - w * FK) / K;
    if ((beta * K - 1) < 0.0) {
      FA = -2 * beta * a * c * exp(2 * c * (beta * K - 1)) /
           pow((1 + exp(2 * c * (beta * K - 1))), 2) * FK;
      FC = -beta *
           (eta * sqrt(a / 3.1415) * c * exp(2 * c * (beta * K - 1)) /
                (pow(sqrt(1 + exp(2 * c * (beta * K - 1))), 3) *
                 (1 + exp(2 * d * (beta * K - 1)))) +
            (eta * sqrt(a / 3.1415) / sqrt(1 + exp(2 * c * (beta * K - 1))) -
             1) *
                2 * d * exp(2 * d * (beta * K - 1)) /
                pow((1 + exp(2 * d * (beta * K - 1))), 2)) *
           FK;
    } else {
      FA = -2 * beta * a * c * exp(-2 * c * (beta * K - 1)) /
           pow((1 + exp(-2 * c * (beta * K - 1))), 2) * FK;
      FC = -beta *
           (eta * sqrt(a / 3.1415) * c * exp(-(c + 2 * d) * (beta * K - 1)) /
                (pow(sqrt(1 + exp(-2 * c * (beta * K - 1))), 3) *
                 (1 + exp(-2 * d * (beta * K - 1)))) +
            (eta * sqrt(a / 3.1415) * exp(-c * (beta * K - 1)) /
                 sqrt(1 + exp(-2 * c * (beta * K - 1))) -
             1) *
                2 * d * exp(-2 * d * (beta * K - 1)) /
                pow((1 + exp(-2 * d * (beta * K - 1))), 2)) *
  ...
  res = RPMD_internal_force(q, invM, beta) +
        (exp(-beta * (V0 - Vshift)) * F0 + exp(-beta * (VKP - Vshift)) * FKP +
         exp(-beta * (V1 - Vshift)) * F1) /
            (exp(-beta * (V0 - Vshift)) + exp(-beta * (VKP - Vshift)) +
             exp(-beta * (V1 - Vshift)));
...
vector<double> nHamiltonian::kcrpmd_effective_auxiliary_force(
  ...
  */
  ...
  if (ntraj == 1) {
    ...
    cout << "Error in kcrpmd_effective_auxiliary_force() not implemented for "
            "quantum nuclei\n";
    exit(0);
  }
  ...
  res = vector<double>(
      1, (exp(-beta * (V0 - Vshift)) * F0 + exp(-beta * (VKP - Vshift)) * FKP +
          exp(-beta * (V1 - Vshift)) * F1) /
             (exp(-beta * (V0 - Vshift)) + exp(-beta * (VKP - Vshift)) +
              exp(-beta * (V1 - Vshift))));
