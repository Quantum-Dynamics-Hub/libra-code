...
/**
  ...
  \brief The file implements computations of the Nonadiabatic couplings (NACs)
  and vibronic Hamiltonians
...
void nHamiltonian::compute_nac_dia(MATRIX &p, const MATRIX &invM) {
  /**
    Function to compute nonadiabatic coupling (nac) in the diabatic
    representation This coupiling is also known as the time-derivative coupling
    and it is a scalar
  ...
  */
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    if (dc1_dia_mem_status[n] == 0) {
      cout
          << "Error in compute_nac_dia(): the derivatives couplings matrix in the adiabatic \
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      ...
      cout << "ERROR in compute_nac_dia: No Hamiltonian matching the requested "
              "id\n";
      exit(0);
    }
  } else {
...
void nHamiltonian::compute_nac_dia(MATRIX &p, const MATRIX &invM, int lvl,
                                   int split) {
  /**
    \brief Function to compute nonadiabatic coupling (nac) in the diabatic
    representation This coupiling is also known as the time-derivative coupling
    and it is a scalar.
    ...
    column by column, with column being used by a children Hamiltonian (next
    level relative to the input parameter lvl). This is only allowed if the
    number of columns in p is equal to the number of children of the
    present-level Hamiltonian.

    There are 4 possible use cases:
    a) p is a ndof x 1 matrix (one trajectory) and is meant to be used by the
    current Hamiltonian - this is already implemented above b) p is a ndof x 1
    matrix (one trajectory) but is meant to be used by the Hamiltonian of a
    different level - call the appropriate level children. c) p is a ndof x
    ntraj matrix containing momenta for multiple trajectories and they are meant
    to be used by the children sub-Hamiltonians of the current level Hamiltonian
    one by one (this is activated by the split=1 option)
    d) Same as "c", but relative to a different level of the Hamiltonians
    hierarchy.
  ...
  */
  ...
  if (lvl == level) { // Case "a"
    if (split ==
      ...
      if (children.size() != static_cast<std::size_t>(p.n_cols)) {
        cout << "ERROR in void nHamiltonian::compute_nac_dia(const MATRIX& p, "
                "const MATRIX& invM, int lvl, int split):\n";
...
void nHamiltonian::compute_nac_adi(MATRIX &p, const MATRIX &invM) {
  /**
    Function to compute nonadiabatic coupling (nac) in the adiabatic
    representation This coupiling is also known as the time-derivative coupling
    and it is a scalar
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      ...
      cout << "ERROR in compute_nac_adi: No Hamiltonian matching the requested "
              "id\n";
      exit(0);
    }
  } else {
...
void nHamiltonian::compute_nac_adi(MATRIX &p, const MATRIX &invM, int lvl,
                                   int split) {
  /**
    \brief Function to compute nonadiabatic coupling (nac) in the adiabatic
    representation This coupiling is also known as the time-derivative coupling
    and it is a scalar.
  ...
  */
  ...
  if (lvl == level) { // Case "a"
    if (split ==
      ...
      // Check whether we have enough sub-Hamiltonians
      if (children.size() != static_cast<std::size_t>(p.n_cols)) {
        cout << "ERROR in void nHamiltonian::compute_nac_adi(const MATRIX& p, "
                "const MATRIX& invM, int lvl, int split):\n";
        cout << "The number of columns of the p (" << p.n_cols << ")";
        cout << " should be equal to the number of children Hamiltonians ("
             << children.size() << ")\n";
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      ...
      cout << "ERROR in compute_hvib_dia: No Hamiltonian matching the "
              "requested id\n";
      exit(0);
    }
  } else {
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      ...
      cout << "ERROR in compute_hvib_adi: No Hamiltonian matching the "
              "requested id\n";
      exit(0);
    }
  } else {
