...
/**
  ...
  \brief The file implements the generic methods of the nHamiltonian class:
  getters and setters
...
void nHamiltonian::show_memory_status(vector<int> &id_) {
  ...
  if (id_.size() == 1) {
    ...
    if (id_[0] == id) {
      ...
      cout << "dc1_dia_mem_status = ";
      for (auto n = 0u; n < dc1_dia_mem_status.size(); n++) {
        cout << dc1_dia_mem_status[n] << " ";
      }
      cout << endl;
      ...
      cout << "d1ham_dia_mem_status = ";
      for (auto n = 0u; n < d1ham_dia_mem_status.size(); n++) {
        cout << d1ham_dia_mem_status[n] << " ";
      }
      cout << endl;
      cout << "d2ham_dia_mem_status = ";
      for (auto n = 0u; n < d2ham_dia_mem_status.size(); n++) {
        cout << d2ham_dia_mem_status[n] << " ";
      }
      cout << endl;
      cout << "dc1_adi_mem_status = ";
      for (auto n = 0u; n < dc1_adi_mem_status.size(); n++) {
        cout << dc1_adi_mem_status[n] << " ";
      }
      cout << endl;
      ...
      cout << "d1ham_adi_mem_status = ";
      for (auto n = 0u; n < d1ham_adi_mem_status.size(); n++) {
        cout << d1ham_adi_mem_status[n] << " ";
      }
      cout << endl;
      cout << "d2ham_adi_mem_status = ";
      for (auto n = 0u; n < d2ham_adi_mem_status.size(); n++) {
        cout << d2ham_adi_mem_status[n] << " ";
      }
      cout << endl;
      cout << "basis_transform_mem_status = " << basis_transform_mem_status
           << endl;
      cout << "time_overlap_adi_mem_status = " << time_overlap_adi_mem_status
           << endl;
      cout << "time_overlap_dia_mem_status = " << time_overlap_dia_mem_status
           << endl;
      cout << "cum_phase_corr_mem_status = " << cum_phase_corr_mem_status
           << endl;
    } else {
      cout << "ERROR in nHamiltonian::show_memory_status: No Hamiltonian "
...
nHamiltonian::nHamiltonian(const nHamiltonian &src) {
  ...
  if (src.d1ham_dia_mem_status.size() > 0) {
    if (src.d1ham_dia_mem_status[0] != 0) {
      der_lvl = 1;
    }
  }
  if (src.d2ham_dia_mem_status.size() > 0) {
    if (src.d2ham_dia_mem_status[0] != 0) {
      der_lvl = 2;
    }
  }
...
void nHamiltonian::init_mem_status(int ndia_, int nadi_, int nnucl_) {
  ...
  ovlp_dia = nullptr;
  ovlp_dia_mem_status = 0;

  ham_dia = nullptr;
  ham_dia_mem_status = 0;
  nac_dia = nullptr;
  nac_dia_mem_status = 0;
  hvib_dia = nullptr;
  hvib_dia_mem_status = 0;

  ham_adi = nullptr;
  ham_adi_mem_status = 0;
  nac_adi = nullptr;
  nac_adi_mem_status = 0;
  hvib_adi = nullptr;
  hvib_adi_mem_status = 0;

  basis_transform = nullptr;
  basis_transform_mem_status = 0;

  time_overlap_adi = nullptr;
  time_overlap_adi_mem_status = 0;
  time_overlap_dia = nullptr;
  time_overlap_dia_mem_status = 0;

  cum_phase_corr = nullptr;
  cum_phase_corr_mem_status = 0;
...
nHamiltonian::~nHamiltonian() {
  ...
  */
  if (this != nullptr) {
    ...
    // cout<<"nHamiltonian destructor at level "<<level<<" address
    // "<<this<<endl;
    ...
    delete ordering_adi;
    ordering_adi = nullptr;

    if (ovlp_dia_mem_status == 1) {
      delete ovlp_dia;
      ovlp_dia = nullptr;
      ovlp_dia_mem_status = 0;
    }

    for (n = 0u; n < dc1_dia.size(); n++) {
      if (dc1_dia_mem_status[n] == 1) {
        delete dc1_dia[n];
        dc1_dia[n] = nullptr;
        dc1_dia_mem_status[n] = 0;
      }
    }
    dc1_dia.clear();
    dc1_dia_mem_status.clear();

    if (ham_dia_mem_status == 1) {
      delete ham_dia;
      ham_dia = nullptr;
      ham_dia_mem_status = 0;
    }
    if (nac_dia_mem_status == 1) {
      delete nac_dia;
      nac_dia = nullptr;
      nac_dia_mem_status = 0;
    }
    if (hvib_dia_mem_status == 1) {
      delete hvib_dia;
      hvib_dia = nullptr;
      hvib_dia_mem_status = 0;
    }

    for (n = 0u; n < d1ham_dia.size(); n++) {
      if (d1ham_dia_mem_status[n] == 1) {
        delete d1ham_dia[n];
        d1ham_dia[n] = nullptr;
        d1ham_dia_mem_status[n] = 0;
      }
    }
    d1ham_dia.clear();
    d1ham_dia_mem_status.clear();

    for (n = 0u; n < d2ham_dia.size(); n++) {
      if (d2ham_dia_mem_status[n] == 1) {
        delete d2ham_dia[n];
        d2ham_dia[n] = nullptr;
        d2ham_dia_mem_status[n] = 0;
      }
    }
    d2ham_dia.clear();
    d2ham_dia_mem_status.clear();

    for (n = 0u; n < dc1_adi.size(); n++) {
      if (dc1_adi_mem_status[n] == 1) {
        delete dc1_adi[n];
        dc1_adi[n] = nullptr;
        dc1_adi_mem_status[n] = 0;
      }
    }
    dc1_adi.clear();
    dc1_adi_mem_status.clear();

    if (ham_adi_mem_status == 1) {
      delete ham_adi;
      ham_adi = nullptr;
      ham_adi_mem_status = 0;
    }
    if (nac_adi_mem_status == 1) {
      delete nac_adi;
      nac_adi = nullptr;
      nac_adi_mem_status = 0;
    }
    if (hvib_adi_mem_status == 1) {
      delete hvib_adi;
      hvib_adi = nullptr;
      hvib_adi_mem_status = 0;
    }

    for (n = 0u; n < d1ham_adi.size(); n++) {
      if (d1ham_adi_mem_status[n] == 1) {
        delete d1ham_adi[n];
        d1ham_adi[n] = nullptr;
        d1ham_adi_mem_status[n] = 0;
      }
    }
    d1ham_adi.clear();
    d1ham_adi_mem_status.clear();

    for (n = 0u; n < d2ham_adi.size(); n++) {
      if (d2ham_adi_mem_status[n] == 1) {
        delete d2ham_adi[n];
        d2ham_adi[n] = nullptr;
        d2ham_adi_mem_status[n] = 0;
      }
    }
    d2ham_adi.clear();
    d2ham_adi_mem_status.clear();

    if (basis_transform_mem_status == 1) {
      delete basis_transform;
      basis_transform = nullptr;
      basis_transform_mem_status = 0;
    }

    if (time_overlap_adi_mem_status == 1) {
      delete time_overlap_adi;
      time_overlap_adi = nullptr;
      time_overlap_adi_mem_status = 0;
    }

    if (time_overlap_dia_mem_status == 1) {
      delete time_overlap_dia;
      time_overlap_dia = nullptr;
      time_overlap_dia_mem_status = 0;
    }

    if (cum_phase_corr_mem_status == 1) {
      delete cum_phase_corr;
      cum_phase_corr = nullptr;
      cum_phase_corr_mem_status = 0;
    }

    /// Deallocate children if they aren't deallocated yet
    for (n = 0u; n < children.size(); n++) {
      if (children[n] != nullptr) {
        children[n]->~nHamiltonian();
      }
    } // for n
    children.clear();
...
void nHamiltonian::copy_content(nHamiltonian *src) {
  ...
  for (auto n = 0u; n < children.size(); n++) {
    // cout<<"  child "<<n<<endl;
    if (children[n] != nullptr) {
      children[n]->copy_content(src->children[n]);
    }
  } // for n
}
...
void nHamiltonian::copy_content(const nHamiltonian &src) {
  ...
  copy_content(const_cast<nHamiltonian *>(&src));
}

void nHamiltonian::copy_level_content(nHamiltonian *src) {
  /**
    Copy allocated variables only if they are allocated in the source and target
    (this) objects The copying is applied only to the current level, and not to
    any children
  */
  ...
  if (ovlp_dia_mem_status != 0 && src->ovlp_dia_mem_status != 0) {
    *ovlp_dia = *(src->ovlp_dia);
  }
  if (ham_dia_mem_status != 0 && src->ham_dia_mem_status != 0) {
    *ham_dia = *(src->ham_dia);
  }
  if (nac_dia_mem_status != 0 && src->nac_dia_mem_status != 0) {
    *nac_dia = *(src->nac_dia);
  }
  if (hvib_dia_mem_status != 0 && src->hvib_dia_mem_status != 0) {
    *hvib_dia = *(src->hvib_dia);
  }

  for (n = 0u; n < dc1_dia.size(); ++n) {
    if (dc1_dia_mem_status[n] != 0 && src->dc1_dia_mem_status[n] != 0) {
      *dc1_dia[n] = *(src->dc1_dia[n]);
    }
  }
  for (n = 0u; n < d1ham_dia.size(); ++n) {
    if (d1ham_dia_mem_status[n] != 0 && src->d1ham_dia_mem_status[n] != 0) {
      *d1ham_dia[n] = *(src->d1ham_dia[n]);
    }
  }
  for (n = 0u; n < d2ham_dia.size(); ++n) {
    if (d2ham_dia_mem_status[n] != 0 && src->d2ham_dia_mem_status[n] != 0) {
      *d2ham_dia[n] = *(src->d2ham_dia[n]);
    }
  }

  //  if(ovlp_adi_mem_status != 0  && src->ovlp_adi_mem_status != 0 ){ *ovlp_dia
  //  = *(src->ovlp_dia);  }
  if (ham_adi_mem_status != 0 && src->ham_adi_mem_status != 0) {
    *ham_adi = *(src->ham_adi);
  }
  if (nac_adi_mem_status != 0 && src->nac_adi_mem_status != 0) {
    *nac_adi = *(src->nac_adi);
  }
  if (hvib_adi_mem_status != 0 && src->hvib_adi_mem_status != 0) {
    *hvib_adi = *(src->hvib_adi);
  }

  for (n = 0u; n < dc1_adi.size(); ++n) {
    if (dc1_adi_mem_status[n] != 0 && src->dc1_adi_mem_status[n] != 0) {
      *dc1_adi[n] = *(src->dc1_adi[n]);
    }
  }
  for (n = 0u; n < d1ham_adi.size(); ++n) {
    if (d1ham_adi_mem_status[n] != 0 && src->d1ham_adi_mem_status[n] != 0) {
      *d1ham_adi[n] = *(src->d1ham_adi[n]);
    }
  }
  for (n = 0u; n < d2ham_adi.size(); ++n) {
    if (d2ham_adi_mem_status[n] != 0 && src->d2ham_adi_mem_status[n] != 0) {
      *d2ham_adi[n] = *(src->d2ham_adi[n]);
    }
  }

  if (basis_transform_mem_status != 0 && src->basis_transform_mem_status != 0) {
    *basis_transform = *(src->basis_transform);
  }
  if (time_overlap_dia_mem_status != 0 &&
      src->time_overlap_dia_mem_status != 0) {
    *time_overlap_dia = *(src->time_overlap_dia);
  }
  if (time_overlap_adi_mem_status != 0 &&
      src->time_overlap_adi_mem_status != 0) {
    *time_overlap_adi = *(src->time_overlap_adi);
  }
  if (cum_phase_corr_mem_status != 0 && src->cum_phase_corr_mem_status != 0) {
    *cum_phase_corr = *(src->cum_phase_corr);
  }
...
void nHamiltonian::init_all(int der_lvl) { init_all(der_lvl, 0); }

void nHamiltonian::init_all(int der_lvl, int target_lvl) {
  /**
    This function will allocate memory for all the variables - in case, we don't
    need to set them up in Python. This may lead to a simpler and cleaner Python
    code and may also be a bit more efficient in terms of performance
  */
  ...
  if (level < target_lvl) {
    ...
    /**
    If we haven't reached the target level yet, we need to propagate the
    initialization to the children nodes
    */
  ...
  //   c out<<"Can not run evaluation of function in the parent Hamiltonian from
  //   the\
  //   child node\n";
  // }
}
...
void nHamiltonian::add_child(nHamiltonian &child) {
  /**
    ...
    such that the <child> becomes connected with <this> by a child-parent
    relationship.
  */
...
void nHamiltonian::add_new_children(int _ndia, int _nadi, int _nnucl,
                                    int nchildren) {
  /**
    This function creates the sub-Hamiltonians of a given one and makes them
    children of the current-level Hamiltonian
  */
...
void nHamiltonian::set_ovlp_dia_by_ref(CMATRIX &ovlp_dia_) {
  ...
  */
  ...
  if (ovlp_dia_mem_status == 0) {
    ovlp_dia = &ovlp_dia_;
  } /// Not allocated - not a problem
  else if (ovlp_dia_mem_status == 1) {
    delete ovlp_dia;
    ovlp_dia = &ovlp_dia_;
  } else if (ovlp_dia_mem_status == 2) {
    ovlp_dia = &ovlp_dia_;
  }
...
void nHamiltonian::set_ovlp_dia_by_val(CMATRIX &ovlp_dia_) {
  ...
  */
  ...
  if (ovlp_dia_mem_status == 0) {
    ...
    check_mat_dimensions(ovlp_dia, ndia, ndia);
  } else if (ovlp_dia_mem_status == 2) {
    ovlp_dia = new CMATRIX(ndia, ndia);
  }
  ...
  */
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    if (dc1_dia_mem_status[n] == 0) {
      ...
      cout << "WARNING in init_dc1_dia: memory for element" << n
...
void nHamiltonian::set_dc1_dia_by_ref(vector<CMATRIX> &dc1_dia_) {
  /**
    Setup of the derivative coupling matrices in the diabatic basis w.r.t all
    nuclear DOFs
  */
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    } else if (dc1_dia_mem_status[n] == 1) {
      delete dc1_dia[n];
      dc1_dia[n] = &dc1_dia_[n];
    } else if (dc1_dia_mem_status[n] == 2) {
      dc1_dia[n] = &dc1_dia_[n];
    }
  ...
  */
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    if (dc1_dia_mem_status[n] == 0) {
      ...
      check_mat_dimensions(dc1_dia[n], ndia, ndia);
    } else if (dc1_dia_mem_status[n] == 2) {
      dc1_dia[n] = new CMATRIX(ndia, ndia);
    }
...
void nHamiltonian::set_ham_dia_by_ref(CMATRIX &ham_dia_) {
  ...
  */
  ...
  else if (ham_dia_mem_status == 1) {
    delete ham_dia;
    ham_dia = &ham_dia_;
  } else if (ham_dia_mem_status == 2) {
    ham_dia = &ham_dia_;
  }
...
void nHamiltonian::set_ham_dia_by_val(CMATRIX &ham_dia_) {
  ...
  */
  ...
  if (ham_dia_mem_status == 0) {
    ...
    check_mat_dimensions(ham_dia, ndia, ndia);
  } else if (ham_dia_mem_status == 2) {
    ham_dia = new CMATRIX(ndia, ndia);
  }
...
void nHamiltonian::set_nac_dia_by_ref(CMATRIX &nac_dia_) {
  /**
    Setup of the nonadiabatic (time-derivative) coupling matrix in the diabatic
    basis
  */
  ...
  else if (nac_dia_mem_status == 1) {
    delete nac_dia;
    nac_dia = &nac_dia_;
  } else if (nac_dia_mem_status == 2) {
    nac_dia = &nac_dia_;
  }
...
void nHamiltonian::set_nac_dia_by_val(CMATRIX &nac_dia_) {
  ...
  */
  ...
  if (nac_dia_mem_status == 0) {
    ...
    check_mat_dimensions(nac_dia, ndia, ndia);
  } else if (nac_dia_mem_status == 2) {
    nac_dia = new CMATRIX(ndia, ndia);
  }
...
void nHamiltonian::set_hvib_dia_by_ref(CMATRIX &hvib_dia_) {
  ...
  */
  ...
  else if (hvib_dia_mem_status == 1) {
    delete hvib_dia;
    hvib_dia = &hvib_dia_;
  } else if (hvib_dia_mem_status == 2) {
    hvib_dia = &hvib_dia_;
  }
...
void nHamiltonian::set_hvib_dia_by_val(CMATRIX &hvib_dia_) {
  ...
  */
  ...
  if (hvib_dia_mem_status == 0) {
    ...
    check_mat_dimensions(hvib_dia, ndia, ndia);
  } else if (hvib_dia_mem_status == 2) {
    hvib_dia = new CMATRIX(ndia, ndia);
  }
...
void nHamiltonian::init_d1ham_dia() {
  /**
    Allocate memory for the 1-st derivatives of the Hamiltonian matrix in the
    diabatic basis w.r.t. all nuclear DOFs
  */
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    if (d1ham_dia_mem_status[n] == 0) {
      ...
      cout << "WARNING in init_d1ham_dia: memory for element" << n
           << " is already allocated\n";
    }
  }
}
...
void nHamiltonian::set_d1ham_dia_by_ref(vector<CMATRIX> &d1ham_dia_) {
  /**
    Setup of the 1-st derivatives of the Hamiltonian matrix in the diabatic
    basis w.r.t all nuclear DOFs
  */

  //  set_X2_by_ref(d1ham_dia, d1ham_dia_, d1ham_dia_mem_status, ndia, ndia,
  //  nnucl);
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    } else if (d1ham_dia_mem_status[n] == 1) {
      delete d1ham_dia[n];
      d1ham_dia[n] = &d1ham_dia_[n];
    } else if (d1ham_dia_mem_status[n] == 2) {
      d1ham_dia[n] = &d1ham_dia_[n];
    }
...
void nHamiltonian::set_d1ham_dia_by_val(vector<CMATRIX> &d1ham_dia_) {
  ...
  */
  ...
  //  set_X2_by_val(d1ham_dia, d1ham_dia_, d1ham_dia_mem_status, ndia, ndia,
  //  nnucl);
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    if (d1ham_dia_mem_status[n] == 0) {
      ...
      check_mat_dimensions(d1ham_dia[n], ndia, ndia);
    } else if (d1ham_dia_mem_status[n] == 2) {
      d1ham_dia[n] = new CMATRIX(ndia, ndia);
    }
...
void nHamiltonian::init_d2ham_dia() {
  /**
    Allocate memory for the 2-nd derivatives of the Hamiltonian matrix in the
    diabatic basis w.r.t. all nuclear DOFs
  */
  ...
  for (int n = 0; n < nnucl * nnucl; n++) {
    ...
    if (d2ham_dia_mem_status[n] == 0) {
      ...
      cout << "WARNING in init_d2ham_dia: memory for element" << n
           << " is already allocated\n";
    }
  }
}
...
void nHamiltonian::set_d2ham_dia_by_ref(vector<CMATRIX> &d2ham_dia_) {
  /**
    Setup of the 2-nd derivatives of the Hamiltonian matrix in the diabatic
    basis w.r.t all nuclear DOFs
  */

  //  set_X2_by_ref(d2ham_dia, d2ham_dia_, d2ham_dia_mem_status, ndia, ndia,
  //  nnucl);

  check_vector_dimensions(d2ham_dia, d2ham_dia_, d2ham_dia_mem_status,
                          nnucl * nnucl);
  ...
  for (int n = 0; n < nnucl * nnucl; n++) {
    ...
    } else if (d2ham_dia_mem_status[n] == 1) {
      delete d2ham_dia[n];
      d2ham_dia[n] = &d2ham_dia_[n];
    } else if (d2ham_dia_mem_status[n] == 2) {
      d2ham_dia[n] = &d2ham_dia_[n];
    }
...
void nHamiltonian::set_d2ham_dia_by_val(vector<CMATRIX> &d2ham_dia_) {
  ...
  */
  ...
  //  set_X2_by_val(d2ham_dia, d2ham_dia_, d2ham_dia_mem_status, ndia, ndia,
  //  nnucl);
  ...
  for (int n = 0; n < nnucl * nnucl; n++) {
    ...
    if (d2ham_dia_mem_status[n] == 0) {
      ...
      check_mat_dimensions(d2ham_dia[n], ndia, ndia);
    } else if (d2ham_dia_mem_status[n] == 2) {
      d2ham_dia[n] = new CMATRIX(ndia, ndia);
    }
...
void nHamiltonian::init_dc1_adi() {
  /**
    Allocate memory for the derivative coupling matrices in the adiabatic basis
    w.r.t. all nuclear DOFs
  */
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    if (dc1_adi_mem_status[n] == 0) {
      ...
      cout << "WARNING in init_dc1_adi: memory for element" << n
           << " is already allocated\n";
    }
  }
}
...
void nHamiltonian::set_dc1_adi_by_ref(vector<CMATRIX> &dc1_adi_) {
  /**
    Setup of the derivative coupling matrices in the adiabatic basis w.r.t all
    nuclear DOFs
  */
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    } else if (dc1_adi_mem_status[n] == 1) {
      delete dc1_adi[n];
      dc1_adi[n] = &dc1_adi_[n];
    } else if (dc1_adi_mem_status[n] == 2) {
      dc1_adi[n] = &dc1_adi_[n];
    }
  ...
  */
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    if (dc1_adi_mem_status[n] == 0) {
      ...
      check_mat_dimensions(dc1_adi[n], nadi, nadi);
    } else if (dc1_adi_mem_status[n] == 2) {
      dc1_adi[n] = new CMATRIX(nadi, nadi);
    }
...
void nHamiltonian::set_ham_adi_by_ref(CMATRIX &ham_adi_) {
  ...
  */
  ...
  if (ham_adi_mem_status == 0) {
    ham_adi = &ham_adi_;
  } else if (ham_adi_mem_status == 1) {
    delete ham_adi;
    ham_adi = &ham_adi_;
  } else if (ham_adi_mem_status == 2) {
    ham_adi = &ham_adi_;
  }
...
void nHamiltonian::set_ham_adi_by_val(CMATRIX &ham_adi_) {
  ...
  */
  ...
  if (ham_adi_mem_status == 0) {
    ...
    check_mat_dimensions(ham_adi, nadi, nadi);
  } else if (ham_adi_mem_status == 2) {
    ham_adi = new CMATRIX(nadi, nadi);
  }
...
void nHamiltonian::init_nac_adi() {
  /**
    Allocate memory for the nonadiabatic (time-derivative) coupling matrix in
    the adiabatic basis
  */
...
void nHamiltonian::set_nac_adi_by_ref(CMATRIX &nac_adi_) {
  /**
    Setup of the nonadiabatic (time-derivative) coupling matrix in the adiabatic
    basis
  */
  ...
  else if (nac_adi_mem_status == 1) {
    delete nac_adi;
    nac_adi = &nac_adi_;
  } else if (nac_adi_mem_status == 2) {
    nac_adi = &nac_adi_;
  }
...
void nHamiltonian::set_nac_adi_by_val(CMATRIX &nac_adi_) {
  ...
  */
  ...
  if (nac_adi_mem_status == 0) {
    ...
    check_mat_dimensions(nac_adi, nadi, nadi);
  } else if (nac_adi_mem_status == 2) {
    nac_adi = new CMATRIX(nadi, nadi);
  }
...
void nHamiltonian::set_hvib_adi_by_ref(CMATRIX &hvib_adi_) {
  ...
  */
  ...
  else if (hvib_adi_mem_status == 1) {
    delete hvib_adi;
    hvib_adi = &hvib_adi_;
  } else if (hvib_adi_mem_status == 2) {
    hvib_adi = &hvib_adi_;
  }
...
void nHamiltonian::set_hvib_adi_by_val(CMATRIX &hvib_adi_) {
  ...
  */
  ...
  if (hvib_adi_mem_status == 0) {
    ...
    check_mat_dimensions(hvib_adi, nadi, nadi);
  } else if (hvib_adi_mem_status == 2) {
    hvib_adi = new CMATRIX(nadi, nadi);
  }
...
void nHamiltonian::init_d1ham_adi() {
  /**
    Allocate memory for the 1-st derivatives of the Hamiltonian matrix w.r.t.
    all nuclear DOFs
  */
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    if (d1ham_adi_mem_status[n] == 0) {
      ...
      cout << "WARNING in init_d1ham_adi: memory for element" << n
           << " is already allocated\n";
    }
  }
}
...
void nHamiltonian::set_d1ham_adi_by_ref(vector<CMATRIX> &d1ham_adi_) {
  /**
    Setup of the 1-st derivatives of the Hamiltonian matrix in the adiabatic
    basis w.r.t all nuclear DOFs
  */

  //  set_X2_by_ref(d1ham_adi, d1ham_adi_, d1ham_adi_mem_status, nadi, nadi,
  //  nnucl);
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    } else if (d1ham_adi_mem_status[n] == 1) {
      delete d1ham_adi[n];
      d1ham_adi[n] = &d1ham_adi_[n];
    } else if (d1ham_adi_mem_status[n] == 2) {
      d1ham_adi[n] = &d1ham_adi_[n];
    }
...
void nHamiltonian::set_d1ham_adi_by_val(vector<CMATRIX> &d1ham_adi_) {
  ...
  */
  ...
  //  set_X2_by_val(d1ham_adi, d1ham_adi_, d1ham_adi_mem_status, nadi, nadi,
  //  nnucl);
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    if (d1ham_adi_mem_status[n] == 0) {
      ...
      check_mat_dimensions(d1ham_adi[n], nadi, nadi);
    } else if (d1ham_adi_mem_status[n] == 2) {
      d1ham_adi[n] = new CMATRIX(nadi, nadi);
    }
...
void nHamiltonian::init_d2ham_adi() {
  /**
    Allocate memory for the 2-nd derivatives of the Hamiltonian matrix w.r.t.
    all nuclear DOFs
  */
  ...
  for (int n = 0; n < nnucl * nnucl; n++) {
    ...
    if (d2ham_adi_mem_status[n] == 0) {
      ...
      cout << "WARNING in init_d2ham_adi: memory for element" << n
           << " is already allocated\n";
    }
  }
}
...
void nHamiltonian::set_d2ham_adi_by_ref(vector<CMATRIX> &d2ham_adi_) {
  /**
    Setup of the 2-nd derivatives of the Hamiltonian matrix in the adiabatic
    basis w.r.t all nuclear DOFs
  */

  //  set_X2_by_ref(d2ham_adi, d2ham_adi_, d2ham_adi_mem_status, nadi, nadi,
  //  nnucl);

  check_vector_dimensions(d2ham_adi, d2ham_adi_, d2ham_adi_mem_status,
                          nnucl * nnucl);
  ...
  for (int n = 0; n < nnucl * nnucl; n++) {
    ...
    } else if (d2ham_adi_mem_status[n] == 1) {
      delete d2ham_adi[n];
      d2ham_adi[n] = &d2ham_adi_[n];
    } else if (d2ham_adi_mem_status[n] == 2) {
      d2ham_adi[n] = &d2ham_adi_[n];
    }
...
void nHamiltonian::set_d2ham_adi_by_val(vector<CMATRIX> &d2ham_adi_) {
  ...
  */
  ...
  //  set_X2_by_val(d2ham_adi, d2ham_adi_, d2ham_adi_mem_status, nadi, nadi,
  //  nnucl);
  ...
  for (int n = 0; n < nnucl * nnucl; n++) {
    ...
    if (d2ham_adi_mem_status[n] == 0) {
      ...
      check_mat_dimensions(d2ham_adi[n], nadi, nadi);
    } else if (d2ham_adi_mem_status[n] == 2) {
      d2ham_adi[n] = new CMATRIX(nadi, nadi);
    }
...
void nHamiltonian::set_basis_transform_by_ref(CMATRIX &basis_transform_) {
  ...
  */
  ...
  //  set_X1_by_ref(basis_transform, basis_transform_,
  //  basis_transform_mem_status, ndia, nadi);
  check_mat_dimensions(&basis_transform_, ndia, nadi);

  if (basis_transform_mem_status == 0) {
    basis_transform = &basis_transform_;
  } else if (basis_transform_mem_status == 1) {
    delete basis_transform;
    basis_transform = &basis_transform_;
  } else if (basis_transform_mem_status == 2) {
    basis_transform = &basis_transform_;
  }
...
void nHamiltonian::set_basis_transform_by_val(CMATRIX &basis_transform_) {
  ...
  */
  ...
  //  set_X1_by_val(basis_transform, basis_transform_,
  //  basis_transform_mem_status, ndia, nadi);
  ...
  if (basis_transform_mem_status == 0) {
    ...
    check_mat_dimensions(basis_transform, ndia, nadi);
  } else if (basis_transform_mem_status == 2) {
    basis_transform = new CMATRIX(ndia, nadi);
  }
...
void nHamiltonian::set_time_overlap_adi_by_ref(CMATRIX &time_overlap_adi_) {
  ...
  */
  ...
  } else if (time_overlap_adi_mem_status == 1) {
    delete time_overlap_adi;
    time_overlap_adi = &time_overlap_adi_;
  } else if (time_overlap_adi_mem_status == 2) {
    time_overlap_adi = &time_overlap_adi_;
  }
...
void nHamiltonian::set_time_overlap_adi_by_val(CMATRIX &time_overlap_adi_) {
  ...
  */
  ...
  if (time_overlap_adi_mem_status == 0) {
    ...
    check_mat_dimensions(time_overlap_adi, nadi, nadi);
  } else if (time_overlap_adi_mem_status == 2) {
    time_overlap_adi = new CMATRIX(nadi, nadi);
  }
...
void nHamiltonian::set_time_overlap_dia_by_ref(CMATRIX &time_overlap_dia_) {
  ...
  */
  ...
  } else if (time_overlap_dia_mem_status == 1) {
    delete time_overlap_dia;
    time_overlap_dia = &time_overlap_dia_;
  } else if (time_overlap_dia_mem_status == 2) {
    time_overlap_dia = &time_overlap_dia_;
  }
...
void nHamiltonian::set_time_overlap_dia_by_val(CMATRIX &time_overlap_dia_) {
  ...
  */
  ...
  if (time_overlap_dia_mem_status == 0) {
    ...
    check_mat_dimensions(time_overlap_dia, ndia, ndia);
  } else if (time_overlap_dia_mem_status == 2) {
    time_overlap_dia = new CMATRIX(ndia, ndia);
  }
...
void nHamiltonian::set_ordering_adi_by_ref(vector<int> &ordering_adi_) {
  /**
    Setup of the variable that takes care of the ordering of the adiabatic
    states
  */

  if (static_cast<int>(ordering_adi_.size()) != nadi) {
    cout
        << "ERROR in void nHamiltonian::set_ordering_adi_by_ref(vector<int>& ordering_adi_): \
          The external variable must be allocated and its size should be = "
        << nadi << ". The \
          current size is = "
        << ordering_adi_.size() << "\nExiting...\n";
    exit(0);
...
void nHamiltonian::init_cum_phase_corr() {
  ...
  */
  ...
  if (cum_phase_corr_mem_status == 0) {
    ...
    cout << "WARNING in void nHamiltonian::init_cum_phase_corr(): memory is "
            "already allocated\n";
  }
}
...
void nHamiltonian::set_cum_phase_corr_by_ref(CMATRIX &cum_phase_corr_) {
  ...
  */
  ...
  } else if (cum_phase_corr_mem_status == 1) {
    delete cum_phase_corr;
    cum_phase_corr = &cum_phase_corr_;
  } else if (cum_phase_corr_mem_status == 2) {
    cum_phase_corr = &cum_phase_corr_;
  }
...
void nHamiltonian::set_cum_phase_corr_by_val(CMATRIX &cum_phase_corr_) {
  ...
  */
  ...
  if (cum_phase_corr_mem_status == 0) {
    ...
    check_mat_dimensions(cum_phase_corr, nadi, 1);
  } else if (cum_phase_corr_mem_status == 2) {
    cum_phase_corr = new CMATRIX(nadi, 1);
  }
...
CMATRIX nHamiltonian::get_ovlp_dia() {
  ...
  */
  if (ovlp_dia_mem_status == 0) {
    cout << "Error in get_ovlp_dia: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      ...
      cout << "ERROR in get_ovlp_dia: No Hamiltonian matching the requested "
              "id\n";
      exit(0);
    }
  } else {
...
CMATRIX nHamiltonian::get_dc1_dia(int i) {
  ...
  */
  if (dc1_dia_mem_status[i] == 0) {
    cout << "Error in get_dc1_dia: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
...
CMATRIX nHamiltonian::get_dc1_dia(int i, vector<int> &id_) {
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return get_dc1_dia(i);
    } else {
      cout
          << "ERROR in get_dc1_dia: No Hamiltonian matching the requested id\n";
      exit(0);
    }
  } else {
    vector<int> next(id_.begin() + 1, id_.end());
...
CMATRIX nHamiltonian::get_ham_dia() {
  ...
  */
  if (ham_dia_mem_status == 0) {
    cout << "Error in get_ham_dia: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
  ...
  */
  ...
  if (id_.size() == 1) {
    if (id_[0] == id) {
      ...
      cout
          << "ERROR in get_ham_dia: No Hamiltonian matching the requested id\n";
      exit(0);
    }
  } else {
    vector<int> next(id_.begin() + 1, id_.end());
...
CMATRIX nHamiltonian::get_nac_dia() {
  /**
    Return the nonadiabatic (time-derivative) coupling matrix in the diabatic
    basis
  */
  if (nac_dia_mem_status == 0) {
    cout << "Error in get_nac_dia: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      ...
      cout
          << "ERROR in get_nac_dia: No Hamiltonian matching the requested id\n";
      exit(0);
    }
  } else {
    vector<int> next(id_.begin() + 1, id_.end());
...
CMATRIX nHamiltonian::get_hvib_dia() {
  ...
  */
  if (hvib_dia_mem_status == 0) {
    cout << "Error in get_hvib_dia: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      ...
      cout << "ERROR in get_hvib_dia: No Hamiltonian matching the requested "
              "id\n";
      exit(0);
    }
  } else {
...
CMATRIX nHamiltonian::get_d1ham_dia(int i) {
  /**
    Return the 1-st derivative of the Hamiltonian matrix in the diabatic basis
    w.r.t. nuclear DOFs
  */
  if (d1ham_dia_mem_status[i] == 0) {
    cout << "Error in get_d1ham_dia: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
...
CMATRIX nHamiltonian::get_d1ham_dia(int i, vector<int> &id_) {
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return get_d1ham_dia(i);
    } else {
      cout << "ERROR in get_d1ham_dia: No Hamiltonian matching the requested "
              "id\n";
      exit(0);
    }
  } else {
...
CMATRIX nHamiltonian::get_d2ham_dia(int i) {
  /**
    Return the 2-nd derivative of the Hamiltonian matrix in the diabatic basis
    w.r.t. nuclear DOFs
  */
  if (d2ham_dia_mem_status[i] == 0) {
    cout << "Error in get_d2ham_dia: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
...
CMATRIX nHamiltonian::get_d2ham_dia(int i, vector<int> &id_) {
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return get_d2ham_dia(i);
    } else {
      cout << "ERROR in get_d2ham_dia: No Hamiltonian matching the requested "
              "id\n";
      exit(0);
    }
  } else {
...
CMATRIX nHamiltonian::get_dc1_adi(int i) {
  ...
  */
  if (dc1_adi_mem_status[i] == 0) {
    cout << "Error in get_dc1_adi: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
...
CMATRIX nHamiltonian::get_dc1_adi(int i, vector<int> &id_) {
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return get_dc1_adi(i);
    } else {
      cout
          << "ERROR in get_dc1_adi: No Hamiltonian matching the requested id\n";
      exit(0);
    }
  } else {
    vector<int> next(id_.begin() + 1, id_.end());
...
CMATRIX nHamiltonian::get_ham_adi() {
  ...
  */
  if (ham_adi_mem_status == 0) {
    cout << "Error in get_ham_adi: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      ...
      cout
          << "ERROR in get_ham_adi: No Hamiltonian matching the requested id\n";
      exit(0);
    }
  } else {
    vector<int> next(id_.begin() + 1, id_.end());
...
CMATRIX nHamiltonian::get_nac_adi() {
  /**
    Return the nonadiabatic (time-derivative) coupling matrix in the adiabatic
    basis
  */
  if (nac_adi_mem_status == 0) {
    cout << "Error in get_nac_adi: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      ...
      cout
          << "ERROR in get_nac_adi: No Hamiltonian matching the requested id\n";
      exit(0);
    }
  } else {
    vector<int> next(id_.begin() + 1, id_.end());
...
CMATRIX nHamiltonian::get_hvib_adi() {
  ...
  */
  if (hvib_adi_mem_status == 0) {
    cout << "Error in get_hvib_adi: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      ...
      cout << "ERROR in get_hvib_adi: No Hamiltonian matching the requested "
              "id\n";
      exit(0);
    }
  } else {
...
CMATRIX nHamiltonian::get_d1ham_adi(int i) {
  /**
    Return the 1-st derivative of the Hamiltonian matrix in the adiabatic basis
    w.r.t. nuclear DOFs
  */
  if (d1ham_adi_mem_status[i] == 0) {
    cout << "Error in get_d1ham_adi: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
...
CMATRIX nHamiltonian::get_d1ham_adi(int i, vector<int> &id_) {
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return get_d1ham_adi(i);
    } else {
      cout << "ERROR in get_d1ham_adi: No Hamiltonian matching the requested "
              "id\n";
      exit(0);
    }
  } else {
...
CMATRIX nHamiltonian::get_d2ham_adi(int i) {
  /**
    Return the 2-nd derivative of the Hamiltonian matrix in the adiabatic basis
    w.r.t. nuclear DOFs
  */
  if (d2ham_adi_mem_status[i] == 0) {
    cout << "Error in get_d2ham_adi: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
...
CMATRIX nHamiltonian::get_d2ham_adi(int i, vector<int> &id_) {
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return get_d2ham_adi(i);
    } else {
      cout << "ERROR in get_d2ham_adi: No Hamiltonian matching the requested "
              "id\n";
      exit(0);
    }
  } else {
...
CMATRIX nHamiltonian::get_basis_transform() {
  ...
  */
  if (basis_transform_mem_status == 0) {
    cout << "Error in get_basis_transform: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
...
CMATRIX nHamiltonian::get_basis_transform(vector<int> &id_) {
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return get_basis_transform();
    } else {
      cout << "ERROR in get_basis_transform: No Hamiltonian matching the "
              "requested id\n";
      exit(0);
    }
  } else {
...
CMATRIX nHamiltonian::get_time_overlap_adi() {
  ...
  */
  if (time_overlap_adi_mem_status == 0) {
    cout << "Error in get_time_overlap_adi: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
...
CMATRIX nHamiltonian::get_time_overlap_adi(vector<int> &id_) {
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return get_time_overlap_adi();
    } else {
      cout << "ERROR in get_time_overlap_adi: No Hamiltonian matching the "
              "requested id\n";
      exit(0);
    }
  } else {
...
CMATRIX nHamiltonian::get_time_overlap_dia() {
  ...
  */
  if (time_overlap_dia_mem_status == 0) {
    cout << "Error in get_time_overlap_dia: The matrix is not allocated "
            "anywhere\nExiting...\n";
    exit(0);
  }
...
CMATRIX nHamiltonian::get_time_overlap_dia(vector<int> &id_) {
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return get_time_overlap_dia();
    } else {
      cout << "ERROR in get_time_overlap_dia: No Hamiltonian matching the "
              "requested id\n";
      exit(0);
    }
  } else {
...
vector<int> nHamiltonian::get_ordering_adi(vector<int> &id_) {
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return get_ordering_adi();
    } else {
      cout
          << "ERROR in vector<int> nHamiltonian::get_ordering_adi(vector<int>& id_): \
...
CMATRIX nHamiltonian::get_cum_phase_corr() {
  ...
  */
  if (cum_phase_corr_mem_status == 0) {
    cout << "ERROR in CMATRIX nHamiltonian::get_cum_phase_corr(): The matrix "
...
CMATRIX nHamiltonian::get_cum_phase_corr(vector<int> &id_) {
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return get_cum_phase_corr();
    } else {
      cout
          << "ERROR in CMATRIX nHamiltonian::get_cum_phase_corr(vector<int>& id_):\
                 No Hamiltonian matching the requested id\n";
      exit(0);
    }
  } else {
...
void nHamiltonian::transform_all(vector<CMATRIX *> &T, int option) {
  /**
   changes all the properties of all the children Hamiltonians by the matrices
   in T
  */

  int ntraj = children.size();
  for (int i = 0; i < ntraj; i++) {
    children[i]->transform_all(T[i], option);
  }
}
...
void nHamiltonian::transform_basis(vector<CMATRIX *> &T, int option) {
  ...
  */
  ...
  int ntraj = children.size();
  for (int i = 0; i < ntraj; i++) {
    children[i]->transform_basis(T[i], option);
  }
