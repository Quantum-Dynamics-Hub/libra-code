...
/**
  ...
  \brief The file implements the calculations of the ETHD (Entangled
  Trajectories Hamiltonian Dynamics) terms
  ...
  */
  ...
  for (dof = 0; dof < ndof; dof++) {
    ...
    for (traj = 0; traj < ntraj; traj++) {
      ...
      //      cout<<"dof = "<<dof<<" traj = "<<traj<<pref * (q.get(dof, traj) -
      //      q_ave.get(dof, 0) )<<endl;
...
void nHamiltonian::add_ethd_dia(const MATRIX &q, const MATRIX &invM,
                                int der_lvl) {
  /**
    Add ETHD energies and (optionally) forces to all the children Hamiltonians
    ...
    *ham_dia = ethd_en;
    ...
    if (der_lvl >= 1) {
      ...
      for (auto traj = 0u; traj < children.size(); traj++) {
        for (auto dof = 0; dof < nnucl; dof++) {
          for (auto st = 0; st < ndia; st++) {
            children[traj]->d1ham_dia[dof]->add(
                st, st, ethd_frcs.get(dof, traj) * minus_one);
          } // for st
        } // dof
      } // traj
...
void nHamiltonian::add_ethd_adi(const MATRIX &q, const MATRIX &invM,
                                int der_lvl) {
  /**
    Add ETHD energies and (optionally) forces to all the children Hamiltonians
  ...
  if (der_lvl >= 0) {
    ...
    *ham_adi = ethd_en;
    ...
    if (der_lvl >= 1) {
      ...
      for (auto traj = 0u; traj < children.size(); traj++) {
        for (auto dof = 0; dof < nnucl; dof++) {
          for (auto st = 0; st < nadi; st++) {
            children[traj]->d1ham_adi[dof]->add(
                st, st, ethd_frcs.get(dof, traj) * minus_one);
          } // for st
        } // dof
      } // traj
