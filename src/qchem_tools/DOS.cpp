...
/**
  \file DOS.cpp
  \brief This file implements function for computing Densities of States
  (atomically and orbitally-resolved)
*/
...
void compute_dos(Electronic_Structure &el, vector<AO> &basis_ao,
                 Control_Parameters &prms, vector<int> &fragment,
  ...
  /** Calculation of the total density of states based on the density matrix
    ...
    \param[in] basis_ao AO basis for the entire system (the one used to produce
    electronic structure)
    \param[in] prms The parameters controlling DOS calculations
    \param[in] fragment The list of integers - the atom indices (not the IDs!)
    onto which we project wavefunction
    \param[in] atom_to_ao_map The mapping between global atomic and AO indices,
    so atom_to_ao_map[a][i] is the global index of i-th orbital of the atom with
    index a
  */
  ...
  for (auto a = 0u; a < fragment.size();
       a++) { // loop over all atoms in the given fragment
    ...
    FILE *fpa;
    std::string filename;
    filename = prms.dos_prefix + "_alpha_wfc_atom" + out;
    ...
    fprintf(fpa, "   Energy(alpha)   Total(alpha)    DOS(s,alpha)   "
                 "DOS(p,alpha)   DOS(d,alpha)\n");
    fprintf(fpb, "   Energy(beta)    Total(beta)     DOS(s,beta)    "
                 "DOS(p,beta)    DOS(d,beta)\n");
    ...
    for (int k = 0; k < el.Norb;
         k++) { // loop over all energies (ordered bands)

      int kk_a = el.bands_alp[k].first; // local index of the MO with this
                                        // energy (usually it is just "k")
      int kk_b = el.bands_bet[k].first; // local index of the MO with this
                                        // energy (usually it is just "k")
      ...
      for (auto n = 0u; n < atom_to_ao_map[A].size();
           n++) { // Loop over all AOs on atom A
        int I =
            atom_to_ao_map[A][n]; // global index of n-th AO centered on atom A
        ...
        for (int j = 0; j < el.Norb; j++) {
          ...
          tmp += el.C_alp->M[ii * el.Norb + kk_a] *
                 el.C_alp->M[jj * el.Norb + kk_a] *
                 el.Sao->M[jj * el.Norb + ii];
        ...
        if (basis_ao[I].ao_shell_type == "s") {
          ...
          popp_a += tmp;
        } else if (basis_ao[I].ao_shell_type == "d") {
          popd_a += tmp;
        }
        ...
        for (int j = 0; j < el.Norb; j++) {
          ...
          tmp += el.C_bet->M[ii * el.Norb + kk_b] *
                 el.C_bet->M[jj * el.Norb + kk_b] *
                 el.Sao->M[jj * el.Norb + ii];
      ...
      fprintf(fpa, " %8.3f  %8.3f %8.3f %8.3f %8.3f   \n",
              el.bands_alp[k].second, pops_a + popp_a + popd_a, pops_a, popp_a,
              popd_a);
      fprintf(fpb, " %8.3f  %8.3f %8.3f %8.3f %8.3f   \n",
              el.bands_bet[k].second, pops_b + popp_b + popd_b, pops_b, popp_b,
              popd_b);
...
void compute_dos(Electronic_Structure &el, vector<AO> &basis_ao,
                 Control_Parameters &prms, boost::python::list fragment,
                 vector<vector<int>> &atom_to_ao_map) {
  /** Calculation of the total density of states based on the density matrix
  ...
  */
  ...
  for (int i = 0; i < sz; i++) {
    fragm[i] = boost::python::extract<double>(fragment[i]);
  }

  compute_dos(el, basis_ao, prms, fragm, atom_to_ao_map);
}

void compute_dos(Electronic_Structure &el, System &syst, vector<AO> &basis_ao,
  ...
  /** Calculation of the total density of states based on the density matrix
    ...
    \param[in] el Variable containig the electronic variables
    \param[in] syst Variable containig nuclear variables - the chemical
    structure
    \param[in] basis_ao AO basis for the entire system (the one used to produce
    electronic structure)
    \param[in] prms The parameters controlling DOS calculations
